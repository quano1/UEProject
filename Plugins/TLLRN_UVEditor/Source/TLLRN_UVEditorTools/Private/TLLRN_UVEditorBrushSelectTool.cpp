...
#define LOCTEXT_NAMESPACE "UTLLRN_UVEditorBrushSelectTool"

namespace TLLRN_UVEditorBrushSelectToolLocals
{
	...
	void UpdateSelections(TArray<UTLLRN_UVToolSelectionAPI::FUVToolSelection>& Selections, TSet<int32> Tids,
		UTLLRN_UVEditorToolMeshInput* Target, bool bSubtract)
	{
		if (Tids.IsEmpty())
		{
			return;
		...
		int32 ExistingSelectionIndex = Selections.IndexOfByPredicate([Target](const UTLLRN_UVToolSelectionAPI::FUVToolSelection& ExistingSelection)
		{
		...
		{
			UTLLRN_UVToolSelectionAPI::FUVToolSelection& ExistingSelection = Selections[ExistingSelectionIndex];
			if (bSubtract)
			{
		...
		{
			UTLLRN_UVToolSelectionAPI::FUVToolSelection& NewSelection = Selections.Emplace_GetRef();
			NewSelection.Target = Target;
			NewSelection.Type = UTLLRN_UVToolSelectionAPI::FUVToolSelection::EType::Triangle;
			NewSelection.SelectedIDs = Tids;
		}
	}
}

void UTLLRN_UVEditorBrushSelectTool::Setup()
{
	using namespace TLLRN_UVEditorBrushSelectToolLocals;
	...
	Settings = NewObject<UTLLRN_UVEditorBrushSelectToolProperties>(this);
	...
	UnwrapClickDragBehavior->OnClickDragFunc = [this](const FInputDeviceRay& DragPos)
	{
		...
		UnwrapBrushIndicator->Update(Settings->UnwrapBrushRadius * FTLLRN_UVEditorUXSettings::UVMeshScalingFactor, 
	...
	{
		UTLLRN_UVEditorMeshSelectionMechanic::FRaycastResult HitResult;
...
void UTLLRN_UVEditorBrushSelectTool::Shutdown(EToolShutdownType ShutdownType)
{
	using namespace TLLRN_UVEditorBrushSelectToolLocals;
...
void UTLLRN_UVEditorBrushSelectTool::Render(IToolsContextRenderAPI* RenderAPI)
{
}

void UTLLRN_UVEditorBrushSelectTool::OnTick(float DeltaTime)
{
	ProcessPendingUnwrapHits();
	ProcessPendingLivePreviewHits();
}

void UTLLRN_UVEditorBrushSelectTool::ClearSelections(bool bBroadcastAndEmit)
{
...
void UTLLRN_UVEditorBrushSelectTool::ProcessPendingUnwrapHits()
{
	using namespace UE::Geometry;
	using namespace TLLRN_UVEditorBrushSelectToolLocals;
	...
	for (const FVector2d& WorldHitPoint : PendingUnwrapHits)
	{
		for (FUVToolSelection& Selection : SelectionMechanic->GetAllCanonicalTrianglesInUnwrapRadius(
			WorldHitPoint, Settings->UnwrapBrushRadius * FTLLRN_UVEditorUXSettings::UVMeshScalingFactor))
		{
...
void UTLLRN_UVEditorBrushSelectTool::ProcessPendingLivePreviewHits()
{
	using namespace UE::Geometry;
	using namespace TLLRN_UVEditorBrushSelectToolLocals;
	...
	for (const UTLLRN_UVEditorMeshSelectionMechanic::FRaycastResult& Hit : PendingLivePreviewHits)
	{
...
void UTLLRN_UVEditorBrushSelectTool::RegisterActions(FInteractiveToolActionSet& ActionSet)
{
...
void UTLLRN_UVEditorBrushSelectTool::UpdateViewportStateFromHoverOrDragEvent(bool bFromUnwrap, bool bIsEndEvent, bool bDragging)
{
	if (!bIsEndEvent)
	{
...
void UTLLRN_UVEditorBrushSelectTool::IncreaseBrushRadiusAction()
{
	using namespace TLLRN_UVEditorBrushSelectToolLocals;
...
void UTLLRN_UVEditorBrushSelectTool::DecreaseBrushRadiusAction()
{
	using namespace TLLRN_UVEditorBrushSelectToolLocals;
...
bool UTLLRN_UVEditorBrushSelectTool::CanCurrentlyNestedCancel()
{
	return bHaveInteracted && (SelectionAPI->HaveSelections() 
		|| SelectionAPI->HaveUnsetElementAppliedMeshSelections());
}

bool UTLLRN_UVEditorBrushSelectTool::ExecuteNestedCancelCommand()
{

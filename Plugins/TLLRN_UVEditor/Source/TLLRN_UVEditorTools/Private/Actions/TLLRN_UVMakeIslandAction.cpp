...
#define LOCTEXT_NAMESPACE "UTLLRN_UVMakeIslandAction"

namespace TLLRN_UVMakeIslandActionLocals
{
...
bool UTLLRN_UVMakeIslandAction::CanExecuteAction() const
{
...
bool UTLLRN_UVMakeIslandAction::ExecuteAction()
{
	using namespace TLLRN_UVMakeIslandActionLocals;
	using namespace UE::Geometry;
	...
	for (FUVToolSelection& Selection : Selections)
	{
		...
		FDynamicMeshUVEditor TLLRN_UVEditor(&Mesh, UVOverlay);
		bSuccess = TLLRN_UVEditor.MakeIsland(Selection.SelectedIDs, &Result, &ChangedTids) && bSuccess;
		ensure(bSuccess);

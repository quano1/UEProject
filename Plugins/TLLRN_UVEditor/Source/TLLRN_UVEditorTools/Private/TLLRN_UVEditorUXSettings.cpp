...
#define LOCTEXT_NAMESPACE "TLLRN_UVEditorUXSettings"

const float FTLLRN_UVEditorUXSettings::UVMeshScalingFactor(1000.0);
const float FTLLRN_UVEditorUXSettings::CameraFarPlaneWorldZ(-10.0);
const float FTLLRN_UVEditorUXSettings::CameraNearPlaneProportionZ(0.8); // Top layer, equivalent to depth bias 80
...
// sorting order.
const float FTLLRN_UVEditorUXSettings::ToolLockedPathDepthBias(9.0);
const float FTLLRN_UVEditorUXSettings::ToolExtendPathDepthBias(9.0);
const float FTLLRN_UVEditorUXSettings::SewLineDepthOffset(8.0f);
const float FTLLRN_UVEditorUXSettings::SelectionHoverWireframeDepthBias(7);
const float FTLLRN_UVEditorUXSettings::SelectionHoverTriangleDepthBias(6);
const float FTLLRN_UVEditorUXSettings::SelectionWireframeDepthBias(5.0);
const float FTLLRN_UVEditorUXSettings::SelectionTriangleDepthBias(4.0);
const float FTLLRN_UVEditorUXSettings::WireframeDepthOffset(3.0);
const float FTLLRN_UVEditorUXSettings::UnwrapTriangleDepthOffset(2.0);

const float FTLLRN_UVEditorUXSettings::LivePreviewExistingSeamDepthBias(2.0);
...
const float FTLLRN_UVEditorUXSettings::BackgroundQuadDepthOffset(1.0); // Bottom layer

// 3D Viewport Depth Offsets
const float FTLLRN_UVEditorUXSettings::LivePreviewHighlightDepthOffset(1.5);

// Opacities
const float FTLLRN_UVEditorUXSettings::UnwrapTriangleOpacity(1.0);
const float FTLLRN_UVEditorUXSettings::UnwrapTriangleOpacityWithBackground(0.25);
const float FTLLRN_UVEditorUXSettings::SelectionTriangleOpacity(1.0f);
const float FTLLRN_UVEditorUXSettings::SelectionHoverTriangleOpacity(1.0f);

// Per Asset Shifts
const float FTLLRN_UVEditorUXSettings::UnwrapBoundaryHueShift(30);
const float FTLLRN_UVEditorUXSettings::UnwrapBoundarySaturation(0.50);
const float FTLLRN_UVEditorUXSettings::UnwrapBoundaryValue(0.50);

// Colors
const FColor FTLLRN_UVEditorUXSettings::UnwrapTriangleFillColor(FColor::FromHex("#696871"));
const FColor FTLLRN_UVEditorUXSettings::UnwrapTriangleWireframeColor(FColor::FromHex("#989898"));
const FColor FTLLRN_UVEditorUXSettings::SelectionTriangleFillColor(FColor::FromHex("#8C7A52"));
const FColor FTLLRN_UVEditorUXSettings::SelectionTriangleWireframeColor(FColor::FromHex("#DDA209"));
const FColor FTLLRN_UVEditorUXSettings::SelectionHoverTriangleFillColor(FColor::FromHex("#4E719B"));
const FColor FTLLRN_UVEditorUXSettings::SelectionHoverTriangleWireframeColor(FColor::FromHex("#0E86FF"));
const FColor FTLLRN_UVEditorUXSettings::SewSideLeftColor(FColor::Red);
const FColor FTLLRN_UVEditorUXSettings::SewSideRightColor(FColor::Green);

const FColor FTLLRN_UVEditorUXSettings::ToolLockedCutPathColor(FColor::Green);
const FColor FTLLRN_UVEditorUXSettings::ToolExtendCutPathColor(FColor::Green);
const FColor FTLLRN_UVEditorUXSettings::ToolLockedJoinPathColor(FColor::Turquoise);
const FColor FTLLRN_UVEditorUXSettings::ToolExtendJoinPathColor(FColor::Turquoise);
const FColor FTLLRN_UVEditorUXSettings::ToolCompletionPathColor(FColor::Orange);

const FColor FTLLRN_UVEditorUXSettings::LivePreviewExistingSeamColor(FColor::Green);

const FColor FTLLRN_UVEditorUXSettings::XAxisColor(FColor::Red);
const FColor FTLLRN_UVEditorUXSettings::YAxisColor(FColor::Green);
const FColor FTLLRN_UVEditorUXSettings::GridMajorColor(FColor::FromHex("#888888"));
const FColor FTLLRN_UVEditorUXSettings::GridMinorColor(FColor::FromHex("#777777"));
const FColor FTLLRN_UVEditorUXSettings::RulerXColor(FColor::FromHex("#888888"));
const FColor FTLLRN_UVEditorUXSettings::RulerYColor(FColor::FromHex("#888888"));
const FColor FTLLRN_UVEditorUXSettings::PivotLineColor(FColor::Cyan);

// Thicknesses
const float FTLLRN_UVEditorUXSettings::LivePreviewHighlightThickness(2.0);
const float FTLLRN_UVEditorUXSettings::LivePreviewHighlightPointSize(4);
const float FTLLRN_UVEditorUXSettings::LivePreviewExistingSeamThickness(2.0);
const float FTLLRN_UVEditorUXSettings::SelectionLineThickness(1.5);
const float FTLLRN_UVEditorUXSettings::ToolLockedPathThickness(3.0f);
const float FTLLRN_UVEditorUXSettings::ToolExtendPathThickness(3.0f);
const float FTLLRN_UVEditorUXSettings::SelectionPointThickness(6);
const float FTLLRN_UVEditorUXSettings::SewLineHighlightThickness(3.0f);
const float FTLLRN_UVEditorUXSettings::AxisThickness(2.0);
const float FTLLRN_UVEditorUXSettings::GridMajorThickness(1.0);
const float FTLLRN_UVEditorUXSettings::WireframeThickness(2.0);
const float FTLLRN_UVEditorUXSettings::BoundaryEdgeThickness(2.0);
const float FTLLRN_UVEditorUXSettings::ToolPointSize(6);
const float FTLLRN_UVEditorUXSettings::PivotLineThickness(1.5);

// Grid
const int32 FTLLRN_UVEditorUXSettings::GridSubdivisionsPerLevel(4);
const int32 FTLLRN_UVEditorUXSettings::GridLevels(3);
const int32 FTLLRN_UVEditorUXSettings::RulerSubdivisionLevel(1);

// Pivot Visuals
const int32 FTLLRN_UVEditorUXSettings::PivotCircleNumSides(32);
const float FTLLRN_UVEditorUXSettings::PivotCircleRadius(10.0);

// CVARs

TAutoConsoleVariable<int32> FTLLRN_UVEditorUXSettings::CVarEnablePrototypeUDIMSupport(
	TEXT("modeling.TLLRN_UVEditor.UDIMSupport"),
	1,
	TEXT("Enable experimental UDIM support in the TLLRN_UVEditor"));


FLinearColor FTLLRN_UVEditorUXSettings::GetTriangleColorByTargetIndex(int32 TargetIndex)
{
...
FLinearColor FTLLRN_UVEditorUXSettings::GetWireframeColorByTargetIndex(int32 TargetIndex)
{
	return FLinearColor::FromSRGBColor(UnwrapTriangleWireframeColor);;
}

FLinearColor FTLLRN_UVEditorUXSettings::GetBoundaryColorByTargetIndex(int32 TargetIndex)
{
...
FColor FTLLRN_UVEditorUXSettings::MakeCividisColorFromScalar(float Scalar)
{
...
FColor FTLLRN_UVEditorUXSettings::MakeTurboColorFromScalar(float Scalar)
{
	// Color map sourced from:
...
// The mappings here depend on the way we view the unwrap world, set up in TLLRN_UVEditorToolkit.cpp.
...
// to the up direction.
FVector3d FTLLRN_UVEditorUXSettings::ExternalUVToUnwrapWorldPosition(const FVector2f& UV)
{
	return FVector3d(UV.X * UVMeshScalingFactor, -UV.Y * UVMeshScalingFactor, 0);
}
FVector2f FTLLRN_UVEditorUXSettings::UnwrapWorldPositionToExternalUV(const FVector3d& VertPosition)
{
...
FVector2f FTLLRN_UVEditorUXSettings::ExternalUVToInternalUV(const FVector2f& UV)
{
	return FVector2f(UV.X, 1-UV.Y);
}
FVector2f FTLLRN_UVEditorUXSettings::InternalUVToExternalUV(const FVector2f& UV)
{
	return FVector2f(UV.X, 1-UV.Y);
}
...
FVector3d FTLLRN_UVEditorUXSettings::UVToVertPosition(const FVector2f& UV)
{
...
FVector2f FTLLRN_UVEditorUXSettings::VertPositionToUV(const FVector3d& VertPosition)
{
...
float FTLLRN_UVEditorUXSettings::LocationSnapValue(int32 LocationSnapMenuIndex)
{
...
int32 FTLLRN_UVEditorUXSettings::MaxLocationSnapValue()
{
	return 5;
}

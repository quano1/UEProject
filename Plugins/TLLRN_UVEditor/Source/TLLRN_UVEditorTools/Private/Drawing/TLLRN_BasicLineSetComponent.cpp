...
public:
	...
	template <typename TLLRN_BasicLineSetComponent>
	FBasicLineSetSceneProxy(TLLRN_BasicLineSetComponent* Component)
		...
		VertexFactory(GetScene().GetFeatureLevel(), "FLineSetSceneProxy")
	{
		...
		if (Component->NumElements() > 0)
		{
			...
			const TArray<typename TLLRN_BasicLineSetComponent::GeometryType>& Lines = Component->Elements;
			...
			ParallelFor(Component->NumElements(), [&](int32 i)
				{
					...
					const int32 LineSize = TLLRN_BasicLineSetComponent::ElementSize;
...
UTLLRN_TLLRN_BasicLineSetComponentBase::UTLLRN_TLLRN_BasicLineSetComponentBase()
{
	CastShadow = false;
	bSelectable = false;
...
void UTLLRN_TLLRN_BasicLineSetComponentBase::ClearComponent()
{
...
void UTLLRN_TLLRN_BasicLineSetComponentBase::SetLineMaterial(UMaterialInterface* InLineMaterial)
{
	LineMaterial = InLineMaterial;
	UMeshComponent::SetMaterial(0, InLineMaterial);
}

void UTLLRN_TLLRN_BasicLineSetComponentBase::SetLineSetParameters(FColor InColor, float InSize, float InDepthBias)
{
	Color = InColor;
	Size = InSize;
...
void UTLLRN_TLLRN_BasicLineSetComponentBase::SetColor(const FColor InColor)
{
	Color = InColor;
}

void UTLLRN_TLLRN_BasicLineSetComponentBase::SetLineThickness(const float InSize)
{
	Size = InSize;
	bBoundsDirty = true;
}
...
FPrimitiveSceneProxy* UTLLRN_Basic2DLineSetComponent::CreateSceneProxy()
{
	return Elements.Num() > 0 ? new FBasicLineSetSceneProxy(this) : nullptr;
}

FPrimitiveSceneProxy* UTLLRN_Basic3DLineSetComponent::CreateSceneProxy()
{
	return Elements.Num() > 0 ? new FBasicLineSetSceneProxy(this) : nullptr;
}

...
public:
	...
	template <typename TLLRN_BasicTriangleSetComponent>
	FBasicTriangleSetSceneProxy(TLLRN_BasicTriangleSetComponent* Component)
		...
		VertexFactory(GetScene().GetFeatureLevel(), "FTriangleSetSceneProxy")
	{
		...
		if (Component->NumElements() > 0)
		{
			...
			const TArray<typename TLLRN_BasicTriangleSetComponent::GeometryType>& Triangles = Component->Elements;
			...
			ParallelFor(Component->NumElements(), [&](int32 i)
				{
					...
					const int32 TriangleSize = TLLRN_BasicTriangleSetComponent::ElementSize;
...
UTLLRN_BasicTriangleSetComponentBase::UTLLRN_BasicTriangleSetComponentBase()
{
	CastShadow = false;
	bSelectable = false;
...
void UTLLRN_BasicTriangleSetComponentBase::ClearComponent()
{
...
void UTLLRN_BasicTriangleSetComponentBase::SetTriangleMaterial(UMaterialInterface* InTriangleMaterial)
{
	TriangleMaterial = InTriangleMaterial;
	UMeshComponent::SetMaterial(0, InTriangleMaterial);
}

void UTLLRN_BasicTriangleSetComponentBase::SetTriangleSetParameters(FColor InColor, const FVector3f& InNormal)
{
	Color = InColor;
	Normal = InNormal;
	bBoundsDirty = true;
...
FPrimitiveSceneProxy* UTLLRN_Basic2DTriangleSetComponent::CreateSceneProxy()
{
	return Elements.Num() > 0 ? new FBasicTriangleSetSceneProxy(this) : nullptr;
}

FPrimitiveSceneProxy* UTLLRN_Basic3DTriangleSetComponent::CreateSceneProxy()
{
	return Elements.Num() > 0 ? new FBasicTriangleSetSceneProxy(this) : nullptr;
}

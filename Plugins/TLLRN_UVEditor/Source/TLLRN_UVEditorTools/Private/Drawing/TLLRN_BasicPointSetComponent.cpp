...
public:
	...
	template <typename TLLRN_BasicPointSetComponent>
	FBasicPointSetSceneProxy(TLLRN_BasicPointSetComponent* Component)
		...
		VertexFactory(GetScene().GetFeatureLevel(), "FPointSetSceneProxy")
	{
		...
		if (Component->NumElements() > 0)
		{
			...
			const TArray<typename TLLRN_BasicPointSetComponent::GeometryType>& Points = Component->Elements;
...
UTLLRN_BasicPointSetComponentBase::UTLLRN_BasicPointSetComponentBase()
{
	CastShadow = false;
	bSelectable = false;
...
void UTLLRN_BasicPointSetComponentBase::ClearComponent()
{
...
void UTLLRN_BasicPointSetComponentBase::SetPointMaterial(UMaterialInterface* InPointMaterial)
{
	PointMaterial = InPointMaterial;
	UMeshComponent::SetMaterial(0, InPointMaterial);
}

void UTLLRN_BasicPointSetComponentBase::SetPointSetParameters(FColor InColor, float InSize, float InDepthBias)
{
	Color = InColor;
	Size = InSize;
...
void UTLLRN_BasicPointSetComponentBase::SetColor(const FColor InColor)
{
	Color = InColor;
}

void UTLLRN_BasicPointSetComponentBase::SetPointSize(const float InSize)
{
	Size = InSize;
	bBoundsDirty = true;
}
...
FPrimitiveSceneProxy* UTLLRN_Basic2DPointSetComponent::CreateSceneProxy()
{
	return Elements.Num() > 0 ? new FBasicPointSetSceneProxy(this) : nullptr;
}

FPrimitiveSceneProxy* UTLLRN_Basic3DPointSetComponent::CreateSceneProxy()
{
	return Elements.Num() > 0 ? new FBasicPointSetSceneProxy(this) : nullptr;
}

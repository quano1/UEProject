...
#include "TLLRN_UVEditorMode.generated.h"
...
class UTLLRN_UVEditorBackgroundPreview;
class UTLLRN_UVEditorDistortionVisualization;
class UTLLRN_TLLRN_UVToolAction;
class UTLLRN_UVToolContextObject;
class UTLLRN_UVToolSelectionAPI;
class UTLLRN_UVToolViewportButtonsAPI;
class UTLLRN_UVTool2DViewportAPI;
class UTLLRN_UVEditorMode;
class UWorld;
class UTLLRN_UVEditorUnwrappedUXProperties;
class UTLLRN_UVEditorLivePreviewUXProperties;

/**
 * Visualization settings for the UTLLRN_UVEditorMode's Grid
 */
UCLASS()
class TLLRN_UVEDITOR_API UTLLRN_UVEditorGridProperties : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
public:
...
struct FTLLRN_UDIMSpecifier
{
	...
	friend bool operator==(const FTLLRN_UDIMSpecifier& A, const FTLLRN_UDIMSpecifier& B )
	{
		return A.UDIM == B.UDIM && A.TextureResolution == B.TextureResolution;
	}

	friend uint32 GetTypeHash(const FTLLRN_UDIMSpecifier& TLLRN_UDIMSpecifier)
	{
		// We're only hashing on the UDIM index, as we don't want to ever have more than one entry with the same UDIM position
		uint32 HashCode = GetTypeHash(TLLRN_UDIMSpecifier.UDIM);
		return HashCode;
	}
};

UENUM()
enum class ETLLRN_UVEditorModeActions
{
	NoAction,
...
/**
 * Settings for UDIMs in the TLLRN_UVEditor
 */
UCLASS()
class TLLRN_UVEDITOR_API UTLLRN_UVEditorUDIMProperties : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
public:
	void Initialize(UTLLRN_UVEditorMode* ParentModeIn) { ParentMode = ParentModeIn; }
	void PostAction(ETLLRN_UVEditorModeActions Action);
	...
	void SetUDIMsFromAsset() { PostAction(ETLLRN_UVEditorModeActions::ConfigureUDIMsFromAsset); }
	...
	UFUNCTION(CallInEditor, Category = UDIM)
	void SetUDIMsFromTexture() { PostAction(ETLLRN_UVEditorModeActions::ConfigureUDIMsFromTexture); };

	/** Currently active UDIM set */
	UPROPERTY(EditAnywhere, Transient, Category = UDIM, meta = (TitleProperty = "UDIM"))
	TArray<FTLLRN_UDIMSpecifier> ActiveUDIMs;

public:
	void InitializeAssets(const TArray<TObjectPtr<UTLLRN_UVEditorToolMeshInput>>& TargetsIn);

private:
	void UpdateActiveUDIMsFromTexture();
	void UpdateActiveUDIMsFromAsset();

	TWeakObjectPtr<UTLLRN_UVEditorMode> ParentMode;
 ...
 */
UCLASS(Transient)
class UTLLRN_UVEditorMode : public UEdMode
{
	GENERATED_BODY()

	friend TLLRN_UVEditorModeChange::FApplyChangesChange;
public:
	const static FEditorModeID EM_TLLRN_UVEditorModeId;
	...
	UPROPERTY()
	TObjectPtr<UTLLRN_UVEditorBackgroundPreview> BackgroundVisualization;

	UPROPERTY()
	TObjectPtr<UTLLRN_UVEditorDistortionVisualization> DistortionVisualization;

	// Hold a settings object to configure the grid
	UPROPERTY()
	TObjectPtr<UTLLRN_UVEditorGridProperties> TLLRN_UVEditorGridProperties = nullptr;

	// Hold a settings object to configure the UDIMs
	UPROPERTY()
	TObjectPtr<UTLLRN_UVEditorUDIMProperties> TLLRN_UVEditorUDIMProperties = nullptr;

	void PopulateUDIMsByAsset(int32 AssetId, TArray<FTLLRN_UDIMSpecifier>& UDIMsOut) const;

	void FocusLivePreviewCameraOnSelection();

protected:
	UPROPERTY()
	TArray<TObjectPtr<UTLLRN_TLLRN_UVToolAction>> RegisteredActions;
	 ...
	 */
	UPROPERTY()
	TArray<TObjectPtr<UTLLRN_UVEditorToolMeshInput>> ToolInputObjects;
...
protected:
	...
	TArray<TWeakObjectPtr<UTLLRN_UVToolContextObject>> ContextsToUpdateOnToolEnd;
	TArray<TWeakObjectPtr<UTLLRN_UVToolContextObject>> ContextsToShutdown;
...
private:
	...
	UPROPERTY()
	TObjectPtr<UTLLRN_UVEditorUnwrappedUXProperties> TLLRN_UVEditorUnwrappedUXProperties = nullptr;

	// Hold a settings object to configure the visualization in the live preview (3d) viewport
	UPROPERTY()
	TObjectPtr<UTLLRN_UVEditorLivePreviewUXProperties> TLLRN_UVEditorLivePreviewUXProperties = nullptr;

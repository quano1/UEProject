...
class FTLLRN_UVEditorModeUILayer;
class FTLLRN_UVEditorPreviewModeManager;
class IDetailsView;
class SDockTab;
class SBorder;
class SWidget;
 ...
 * state is held in the UTLLRN_UVEditorMode managed by the mode manager, the toolkit also ends up
 * initializing the UV mode.
 * Thus, the FUVEdiotrToolkit ends up being the central place for the UV Asset editor setup.
 */
class TLLRN_UVEDITOR_API FTLLRN_UVEditorToolkit : public FBaseAssetToolkit
{
public:
	FTLLRN_UVEditorToolkit(UAssetEditor* InOwningAssetEditor);
	virtual ~FTLLRN_UVEditorToolkit();
...
protected:
	...
	TWeakPtr<SEditorViewport> TLLRN_UVEditor2DViewport;
	UTLLRN_UVToolViewportButtonsAPI* ViewportButtonsAPI = nullptr;

	UTLLRN_UVTool2DViewportAPI* TLLRN_UVTool2DViewportAPI = nullptr;

	TSharedPtr<FTLLRN_UVEditorModeUILayer> ModeUILayer;

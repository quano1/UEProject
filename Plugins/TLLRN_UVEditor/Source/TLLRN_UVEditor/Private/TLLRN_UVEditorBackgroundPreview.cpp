...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_UVEditorBackgroundPreview)

using namespace UE::Geometry;

void UTLLRN_UVEditorBackgroundPreview::OnCreated()
{
	Settings = NewObject<UTLLRN_UVEditorBackgroundPreviewProperties>(this);
	Settings->WatchProperty(Settings->bVisible, [this](bool) { bSettingsModified = true; });
	Settings->WatchProperty(Settings->SourceType, [this](ETLLRN_UVEditorBackgroundSourceType) { bSettingsModified = true; });
	...
	Settings->WatchProperty(Settings->TLLRN_UDIMBlocks, [this](const TArray<int32>&) {bSettingsModified = true; });
...
void UTLLRN_UVEditorBackgroundPreview::OnTick(float DeltaTime)
{
...
void UTLLRN_UVEditorBackgroundPreview::UpdateVisibility()
{
...
void UTLLRN_UVEditorBackgroundPreview::UpdateBackground()
{
	const FVector Normal(0, 0, 1);
	const FColor BackgroundColor = FColor::Blue;

	UMaterial* RegularMaterial = LoadObject<UMaterial>(nullptr, TEXT("/TLLRN_UVEditor/Materials/TLLRN_UVEditorBackground"));
	UMaterial* VirtualTextureMaterial = LoadObject<UMaterial>(nullptr, TEXT("/TLLRN_UVEditor/Materials/TLLRN_UVEditorVTBackground"));
	...
	{
		case ETLLRN_UVEditorBackgroundSourceType::Checkerboard:
		{
			// Do nothing, since the default material is already set up for a checkerboard.
		}
		break;

		case ETLLRN_UVEditorBackgroundSourceType::Material:
		{
		...
		case ETLLRN_UVEditorBackgroundSourceType::Texture:
		{
	...
	BackgroundMaterial->SetScalarParameterValue(TEXT("BackgroundPixelDepthOffset"), FTLLRN_UVEditorUXSettings::BackgroundQuadDepthOffset);	
	...
	if (Settings->bUDIMsEnabled)
	{
		...
		for (int32 BlockIndex = 0; BlockIndex < Settings->TLLRN_UDIMBlocks.Num(); ++BlockIndex)
		{
			FIndex2i Block;
			UE::TextureUtilitiesCommon::ExtractUDIMCoordinates(Settings->TLLRN_UDIMBlocks[BlockIndex], Block.A, Block.B);

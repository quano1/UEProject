...
#define LOCTEXT_NAMESPACE "UTLLRN_UnsetUVsAction"

namespace TLLRN_UnsetUVsActionLocals
{
...
bool UTLLRN_UnsetUVsAction::CanExecuteAction() const
{
	return SelectionAPI->HaveSelections()
		&& SelectionAPI->GetSelectionsType() == UE::Geometry::FUVToolSelection::EType::Triangle;
}
bool UTLLRN_UnsetUVsAction::ExecuteAction()
{
	using namespace TLLRN_UnsetUVsActionLocals;
	using namespace UE::Geometry;
	...
	for (const FUVToolSelection& Selection : Selections)
	{
		...
		Selection.Target->UpdateAllFromAppliedCanonical(UTLLRN_UVEditorToolMeshInput::NONE_CHANGED_ARG, &ChangedTidsArray, &ChangedTidsArray);

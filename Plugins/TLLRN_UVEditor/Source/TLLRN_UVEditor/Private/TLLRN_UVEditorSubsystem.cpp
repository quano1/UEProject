...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_UVEditorSubsystem)

using namespace UE::Geometry;

static TAutoConsoleVariable<int32> CVarEnableLivePreviewArrangement(
	TEXT("modeling.TLLRN_UVEditor.EnableLivePreviewArrangement"),
	1,
...
static TAutoConsoleVariable<int32> CVarLivePreviewArrangementMode(
	TEXT("modeling.TLLRN_UVEditor.LivePreviewArrangementMode"),
	0,
...
void UTLLRN_UVEditorSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
...
void UTLLRN_UVEditorSubsystem::Deinitialize()
{
	ToolTargetManager->Shutdown();
	ToolTargetManager = nullptr;
}

bool UTLLRN_UVEditorSubsystem::AreObjectsValidTargets(const TArray<UObject*>& InObjects) const
{
	...
	{
		if (!ToolTargetManager->CanBuildTarget(Object, UTLLRN_UVEditorMode::GetToolTargetRequirements()))
		{
			return false;
		}
	}
...
bool UTLLRN_UVEditorSubsystem::AreAssetsValidTargets(const TArray<FAssetData>& InAssets) const
{
	if (InAssets.IsEmpty())
	{
		return false;
...
void UTLLRN_UVEditorSubsystem::BuildTargets(const TArray<TObjectPtr<UObject>>& ObjectsIn, 
...
void UTLLRN_UVEditorSubsystem::StartTLLRN_UVEditor(TArray<TObjectPtr<UObject>> ObjectsToEdit)
{
	...
	{
		UToolTarget* Target = ToolTargetManager->BuildTarget(Object, UTLLRN_UVEditorMode::GetToolTargetRequirements());
		Targets.Add(Target);
	...
	UTLLRN_UVEditor* TLLRN_UVEditor = NewObject<UTLLRN_UVEditor>(this);
	...
	TLLRN_UVEditor->Initialize(ObjectsToEdit, ObjectTransforms);

	for (TObjectPtr<UObject>& Object : ObjectsToEdit)
	{
		OpenedEditorInstances.Add(Object, TLLRN_UVEditor);
	}
}

void UTLLRN_UVEditorSubsystem::NotifyThatTLLRN_UVEditorClosed(TArray<TObjectPtr<UObject>> ObjectsItWasEditing)
{

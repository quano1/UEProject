...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_ModelingToolsEditorMode)
...
#define LOCTEXT_NAMESPACE "UTLLRN_ModelingToolsEditorMode"


const FEditorModeID UTLLRN_ModelingToolsEditorMode::EM_TLLRN_ModelingToolsEditorModeId = TEXT("EM_TLLRN_ModelingToolsEditorMode");

FDateTime UTLLRN_ModelingToolsEditorMode::LastModeStartTimestamp;
FDateTime UTLLRN_ModelingToolsEditorMode::LastToolStartTimestamp;

FDelegateHandle UTLLRN_ModelingToolsEditorMode::GlobalModelingWorldTeardownEventHandle;
...
UTLLRN_ModelingToolsEditorMode::UTLLRN_ModelingToolsEditorMode()
{
	Info = FEditorModeInfo(
		EM_TLLRN_ModelingToolsEditorModeId,
		LOCTEXT("TLLRN_ModelingToolsEditorModeName", "Modeling"),
		FSlateIcon("TLLRN_ModelingToolsStyle", "LevelEditor.TLLRN_ModelingToolsMode", "LevelEditor.TLLRN_ModelingToolsMode.Small"),
		true,
		5000);
}

UTLLRN_ModelingToolsEditorMode::UTLLRN_ModelingToolsEditorMode(FVTableHelper& Helper)
...
bool UTLLRN_ModelingToolsEditorMode::ProcessEditDelete()
{
	if (GetSelectionManager() && GetSelectionManager()->HasSelection())
	{
		if ( UDeleteGeometrySelectionCommand* DeleteCommand = Cast<UDeleteGeometrySelectionCommand>(TLLRN_ModelingModeCommands[0]) )
		{
...
bool UTLLRN_ModelingToolsEditorMode::ProcessEditCut()
{
	// for now we disable deleting in an Accept-style tool because it can result in crashes if we are deleting target object
	if (GetToolManager()->HasAnyActiveTool() && GetToolManager()->GetActiveTool(EToolSide::Mouse)->HasAccept())
	{
...
void UTLLRN_ModelingToolsEditorMode::ActorSelectionChangeNotify()
{
...
bool UTLLRN_ModelingToolsEditorMode::CanAutoSave() const
{
	// prevent autosave if any tool is active
	return GetToolManager()->HasAnyActiveTool() == false;
}

bool UTLLRN_ModelingToolsEditorMode::ShouldDrawWidget() const
{ 
...
void UTLLRN_ModelingToolsEditorMode::Tick(FEditorViewportClient* ViewportClient, float DeltaTime)
{
	Super::Tick(ViewportClient, DeltaTime);

	if (Toolkit.IsValid())
	{
		FTLLRN_ModelingToolsEditorModeToolkit* ModelingToolkit = (FTLLRN_ModelingToolsEditorModeToolkit*)Toolkit.Get();
...
void UTLLRN_ModelingToolsEditorMode::Enter()
{
	UEdMode::Enter();
	...
	const UTLLRN_ModelingToolsModeCustomizationSettings* ModelingEditorSettings = GetDefault<UTLLRN_ModelingToolsModeCustomizationSettings>();
	bSelectionSystemEnabled = TLLRN_ModelingModeSettings->bEnablePersistentSelections;
	...
	GetToolManager()->OnToolPostBuild.AddUObject(this, &UTLLRN_ModelingToolsEditorMode::OnToolPostBuild);
	...
	GetInteractiveToolsContext()->OnRender.AddUObject(this, &UTLLRN_ModelingToolsEditorMode::OnToolsContextRender);
	GetInteractiveToolsContext()->OnDrawHUD.AddUObject(this, &UTLLRN_ModelingToolsEditorMode::OnToolsContextDrawHUD);

	// register stylus event handler
	IToolStylusStateProviderAPI* StylusAPI = nullptr;
	if (ensure(Toolkit.IsValid()))
	{
		StylusAPI = ((FTLLRN_ModelingToolsEditorModeToolkit*)Toolkit.Get())->GetStylusStateProviderAPI();
	}
	...
	if (TLLRN_ModelingModeSettings)
	{
		GetInteractiveToolsContext()->SetForceCombinedGizmoMode(TLLRN_ModelingModeSettings->bRespectLevelEditorGizmoMode == false);
		GetInteractiveToolsContext()->SetAbsoluteWorldSnappingEnabled(TLLRN_ModelingModeSettings->bEnableAbsoluteWorldSnapping);
	}

	// Now that we have the gizmo helper, bind the numerical UI.
	if (ensure(Toolkit.IsValid()))
	{
		((FTLLRN_ModelingToolsEditorModeToolkit*)Toolkit.Get())->BindGizmoNumericalUI();
	}
	...
	if (ensure(Toolkit.IsValid()))
	{
		UTLLRN_ModelingToolsHostCustomizationAPI::Register(GetInteractiveToolsContext(), 
			StaticCastSharedRef<FTLLRN_ModelingToolsEditorModeToolkit>(Toolkit.ToSharedRef()));
	}
	...
	//{
	//	((FTLLRN_ModelingToolsEditorModeToolkit*)Toolkit.Get())->ForceToolPaletteRebuild();
	//});

	// set up the selection interaction
	SelectionInteraction = NewObject<UTLLRN_ModelingSelectionInteraction>(GetToolManager());
	...
	const FTLLRN_ModelingToolsManagerCommands& ToolManagerCommands = FTLLRN_ModelingToolsManagerCommands::Get();
	...
	GenerateSMLODToolBuilder->bInRestrictiveMode = TLLRN_ModelingModeSettings && TLLRN_ModelingModeSettings->InRestrictiveMode();
		...
		Toolkit->GetToolkitCommands()->MapAction(UICommand,
			FExecuteAction::CreateLambda([this, TopoMode, ElementMode]() {
				if ( GetToolManager() && GetToolManager()->GetContextTransactionsAPI() && GetSelectionManager() )
				{
					...
					if (UTLLRN_ModelingToolsModeCustomizationSettings* ModelingEditorSettings = GetMutableDefault<UTLLRN_ModelingToolsModeCustomizationSettings>())
					{
	...
	{
		TLLRN_ModelingModeCommands.Add(Command);
		const TSharedRef<FUICommandList>& CommandList = Toolkit->GetToolkitCommands();
		CommandList->MapAction(UICommand,
	...
	if (Extensions.Num() > 0)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("TLLRN_ModelingMode: Found %d Tool Extension Modules"), Extensions.Num());
	...
	FTLLRN_ModelingModeActionCommands::RegisterCommandBindings(Toolkit->GetToolkitCommands(), [this](ETLLRN_ModelingModeActionCommands Command) {
		TLLRN_ModelingModeShortcutRequested(Command);
	});
	...
	if (FEngineAnalytics::IsAvailable())
	{
		...
		FEngineAnalytics::GetProvider().RecordEvent(TEXT("Editor.Usage.MeshTLLRN_ModelingMode.Enter"), Attributes);
	}
	...
	GetToolManager()->OnToolStarted.AddLambda([this](UInteractiveToolManager* Manager, UInteractiveTool* Tool)
	{
		if (FEngineAnalytics::IsAvailable())
		{
			...
			FEngineAnalytics::GetProvider().RecordEvent(TEXT("Editor.Usage.MeshTLLRN_ModelingMode.ToolStarted"), Attributes);
		}
	});
	...
	GetToolManager()->OnToolEnded.AddLambda([this](UInteractiveToolManager* Manager, UInteractiveTool* Tool)
	{
		if (FEngineAnalytics::IsAvailable())
		{
			...
			FEngineAnalytics::GetProvider().RecordEvent(TEXT("Editor.Usage.MeshTLLRN_ModelingMode.ToolEnded"), Attributes);
		}
	});
	...
	EditorClosedEventHandle = GEditor->OnEditorClose().AddUObject(this, &UTLLRN_ModelingToolsEditorMode::OnEditorClosed);
	...
	{
		ETLLRN_ModelingSelectionInteraction_DragMode LastDragMode = static_cast<ETLLRN_ModelingSelectionInteraction_DragMode>(ModelingEditorSettings->LastMeshSelectionDragMode);
		if ( LastDragMode == ETLLRN_ModelingSelectionInteraction_DragMode::NoDragInteraction || LastDragMode == ETLLRN_ModelingSelectionInteraction_DragMode::PathInteraction )
		{
			SelectionInteraction->SetActiveDragMode(LastDragMode);
		}

		ETLLRN_ModelingSelectionInteraction_LocalFrameMode LastLocalFrameMode =
			static_cast<ETLLRN_ModelingSelectionInteraction_LocalFrameMode>(ModelingEditorSettings->LastMeshSelectionLocalFrameMode);
		if ( LastLocalFrameMode == ETLLRN_ModelingSelectionInteraction_LocalFrameMode::FromGeometry || LastLocalFrameMode == ETLLRN_ModelingSelectionInteraction_LocalFrameMode::FromObject )
		{
	...
	BlueprintPreCompileHandle = GEditor->OnBlueprintPreCompile().AddUObject(this, &UTLLRN_ModelingToolsEditorMode::OnBlueprintPreCompile);
...
void UTLLRN_ModelingToolsEditorMode::RegisterUVEditor()
{
...
void UTLLRN_ModelingToolsEditorMode::Exit()
{
	...
	if (FEngineAnalytics::IsAvailable())
	{
		...
		FEngineAnalytics::GetProvider().RecordEvent(TEXT("Editor.Usage.MeshTLLRN_ModelingMode.Exit"), Attributes);
	}

	UTLLRN_ModelingToolsHostCustomizationAPI::Deregister(GetInteractiveToolsContext());
	...
	FTLLRN_ModelingModeActionCommands::UnRegisterCommandBindings(Toolkit->GetToolkitCommands());
...
void UTLLRN_ModelingToolsEditorMode::OnEditorClosed()
{
	...
	if (Toolkit.IsValid())
	{
		static_cast<FTLLRN_ModelingToolsEditorModeToolkit*>(Toolkit.Get())->DisconnectStylusStateProviderAPI();
	}
}


void UTLLRN_ModelingToolsEditorMode::OnBlueprintPreCompile(UBlueprint* Blueprint)
{
...
void UTLLRN_ModelingToolsEditorMode::OnToolsContextRender(IToolsContextRenderAPI* RenderAPI)
{
	if (SelectionInteraction)
	{
...
void UTLLRN_ModelingToolsEditorMode::OnToolsContextDrawHUD(FCanvas* Canvas, IToolsContextRenderAPI* RenderAPI)
{
	if (SelectionInteraction)
	{
...
bool UTLLRN_ModelingToolsEditorMode::ShouldToolStartBeAllowed(const FString& ToolIdentifier) const
{
...
bool UTLLRN_ModelingToolsEditorMode::GetGeometrySelectionChangesAllowed() const
{
...
bool UTLLRN_ModelingToolsEditorMode::TestForEditorGizmoHit(const FInputDeviceRay& ClickPos) const
{
...
bool UTLLRN_ModelingToolsEditorMode::GetMeshElementSelectionSystemEnabled() const
{
	return bSelectionSystemEnabled;
}

void UTLLRN_ModelingToolsEditorMode::NotifySelectionSystemEnabledStateModified()
{
...
void UTLLRN_ModelingToolsEditorMode::UpdateSelectionManagerOnEditorSelectionChange(bool bEnteringMode)
{
	...
	// probably it should be handled by the Selector Factories registered in UTLLRN_ModelingToolsEditorMode::Enter(),
...
bool UTLLRN_ModelingToolsEditorMode::BoxSelect(FBox& InBox, bool InSelect) 
{
	// not handling yet
	return false;
}

bool UTLLRN_ModelingToolsEditorMode::FrustumSelect(const FConvexVolume& InFrustum, FEditorViewportClient* InViewportClient, bool InSelect)
{
	if (bIsToolActive)
	{
...
void UTLLRN_ModelingToolsEditorMode::CreateToolkit()
{
	Toolkit = MakeShareable(new FTLLRN_ModelingToolsEditorModeToolkit);
}

void UTLLRN_ModelingToolsEditorMode::OnToolPostBuild(
...
void UTLLRN_ModelingToolsEditorMode::OnToolStarted(UInteractiveToolManager* Manager, UInteractiveTool* Tool)
{
	...
	if( FEngineAnalytics::IsAvailable() )
	{
		FEngineAnalytics::GetProvider().RecordEvent(TEXT("Editor.Usage.MeshTLLRN_ModelingMode.ToolStarted"),
		                                            TEXT("ToolName"),
		                                            GetToolName(*Tool));
	}
...
void UTLLRN_ModelingToolsEditorMode::OnToolEnded(UInteractiveToolManager* Manager, UInteractiveTool* Tool)
{
	...
	if( FEngineAnalytics::IsAvailable() )
	{
		FEngineAnalytics::GetProvider().RecordEvent(TEXT("Editor.Usage.MeshTLLRN_ModelingMode.ToolEnded"),
		                                            TEXT("ToolName"),
		                                            GetToolName(*Tool));
	}
...
void UTLLRN_ModelingToolsEditorMode::BindCommands()
{
	Super::BindCommands();
...
void UTLLRN_ModelingToolsEditorMode::AcceptActiveToolActionOrTool()
{
...
void UTLLRN_ModelingToolsEditorMode::CancelActiveToolActionOrTool()
{
	// if we have an active Tool that implements 
	if (GetToolManager()->HasAnyActiveTool())
	{
...
void UTLLRN_ModelingToolsEditorMode::TLLRN_ModelingModeShortcutRequested(ETLLRN_ModelingModeActionCommands Command)
{
	if (Command == ETLLRN_ModelingModeActionCommands::FocusViewToCursor)
	{
		FocusCameraAtCursorHotkey();
	}
	else if (Command == ETLLRN_ModelingModeActionCommands::ToggleSelectionLockState)
	{
		if (SelectionManager)
		{
...
void UTLLRN_ModelingToolsEditorMode::FocusCameraAtCursorHotkey()
{
...
FBox UTLLRN_ModelingToolsEditorMode::ComputeCustomViewportFocus() const
{
...
bool UTLLRN_ModelingToolsEditorMode::HasCustomViewportFocus() const
{
...
bool UTLLRN_ModelingToolsEditorMode::GetPivotForOrbit(FVector& OutPivot) const
{
...
void UTLLRN_ModelingToolsEditorMode::ConfigureRealTimeViewportsOverride(bool bEnable)
{
	...
	if (LevelEditor.IsValid())
	{
		...
		for (const TSharedPtr<SLevelViewport>& ViewportWindow : Viewports)
		{
			if (ViewportWindow.IsValid())
			{
				...
				const FText SystemDisplayName = LOCTEXT("RealtimeOverrideMessage_TLLRN_ModelingMode", "Modeling Mode");
				if (bEnable)
				{

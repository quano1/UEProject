...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimGraphNode_TLLRN_ControlRig)

#define LOCTEXT_NAMESPACE "AnimGraphNode_TLLRN_ControlRig"

UAnimGraphNode_TLLRN_ControlRig::UAnimGraphNode_TLLRN_ControlRig(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

FText UAnimGraphNode_TLLRN_ControlRig::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	// display control rig here
	return LOCTEXT("AnimGraphNode_TLLRN_ControlRig_Title", "TLLRN Control Rig");
}

FText UAnimGraphNode_TLLRN_ControlRig::GetTooltipText() const
{
	// display control rig here
	return LOCTEXT("AnimGraphNode_TLLRN_ControlRig_Tooltip", "Evaluates a control rig");
}

void UAnimGraphNode_TLLRN_ControlRig::CreateCustomPins(TArray<UEdGraphPin*>* OldPins)
{
	...
	{
		if (UTLLRN_ControlRigBlueprint* RigBlueprint = Cast<UTLLRN_ControlRigBlueprint>(GeneratedClass->ClassGeneratedBy))
		{
				...
				if (URigVMBlueprintGeneratedClass* GeneratedClass = Cast<URigVMBlueprintGeneratedClass>(GetTargetClass()))
				{
					for (TFieldIterator<FProperty> PropertyIt(GeneratedClass); PropertyIt; ++PropertyIt)
					{
						if (PropertyIt->GetFName() == PropertyName)
						{
							...
							// Variable.Memory here points to the corresponding property in the Control Rig BP CDO, it was initialized in UAnimGraphNode_TLLRN_ControlRig::RebuildExposedProperties 
...
void UAnimGraphNode_TLLRN_ControlRig::ValidateAnimNodeDuringCompilation(USkeleton* ForSkeleton, FCompilerResultsLog& MessageLog)
{
	if (UClass* TargetClass = GetTargetClass())
	{
		if (UTLLRN_ControlRigBlueprint* Blueprint = Cast<UTLLRN_ControlRigBlueprint>(TargetClass->ClassGeneratedBy))
		{
			...
			if (ForSkeleton)
			{
				...
				{
					const FTLLRN_RigElementKey BoneKey(BoneInfo.Name, ETLLRN_RigElementType::Bone);
					if (Hierarchy->Contains(BoneKey))
					{
						const FTLLRN_RigElementKey ParentKey = Hierarchy->GetFirstParent(BoneKey);
...
void UAnimGraphNode_TLLRN_ControlRig::RebuildExposedProperties()
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()

	if (URigVMBlueprintGeneratedClass* TargetClass = Cast<URigVMBlueprintGeneratedClass>(GetTargetClass()))
	{
		if (UTLLRN_ControlRigBlueprint* RigBlueprint = Cast<UTLLRN_ControlRigBlueprint>(TargetClass->ClassGeneratedBy))
		{
			if (RigBlueprint->HasAllFlags(RF_NeedPostLoad))
			{
				return;
...
bool UAnimGraphNode_TLLRN_ControlRig::IsInputProperty(const FName& PropertyName) const
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
UAnimGraphNode_TLLRN_ControlRig::FControlsInfo* UAnimGraphNode_TLLRN_ControlRig::FindControlElement(const FName& InControlName) const
{
	TArray<FControlsInfo>& Controls = GetControls();
...
bool UAnimGraphNode_TLLRN_ControlRig::IsAvailableToMapToCurve(const FName& PropertyName, bool bInput) const
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
	...
	{
		return (ControlInfo->ControlType == ETLLRN_RigControlType::Float || ControlInfo->ControlType == ETLLRN_RigControlType::ScaleFloat);
	}

	return ensure(false);
}

bool UAnimGraphNode_TLLRN_ControlRig::IsPropertyExposeEnabled(FName PropertyName) const
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
ECheckBoxState UAnimGraphNode_TLLRN_ControlRig::IsPropertyExposed(FName PropertyName) const
{
...
void UAnimGraphNode_TLLRN_ControlRig::OnPropertyExposeCheckboxChanged(ECheckBoxState NewState, FName PropertyName)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
void UAnimGraphNode_TLLRN_ControlRig::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
	...
	[
		...
		.OnVariableMappingChanged(FOnRigVMVariableMappingChanged::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::OnVariableMappingChanged, true))
		.OnGetVariableMapping(FOnRigVMGetVariableMapping::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::GetVariableMapping, true))
		.OnGetAvailableMapping(FOnRigVMGetAvailableMapping::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::GetAvailableMapping, true))
		.OnCreateVariableMapping(FOnRigVMCreateVariableMapping::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::CreateVariableMapping, true))
		.OnVariableOptionAvailable(FOnRigVMVarOptionAvailable::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::IsAvailableToMapToCurve, true))
		.OnPinGetCheckState(FOnRigVMPinGetCheckState::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::IsPropertyExposed))
		.OnPinCheckStateChanged(FOnRigVMPinCheckStateChanged::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::OnPropertyExposeCheckboxChanged))
		.OnPinIsEnabledCheckState(FOnRigVMPinIsCheckEnabled::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::IsPropertyExposeEnabled))
	];
#endif
	...
	[
		SNew(SRigVMVariableMappingWidget)
		.OnVariableMappingChanged(FOnRigVMVariableMappingChanged::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::OnVariableMappingChanged, false))
		.OnGetVariableMapping(FOnRigVMGetVariableMapping::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::GetVariableMapping, false))
		.OnGetAvailableMapping(FOnRigVMGetAvailableMapping::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::GetAvailableMapping, false))
		.OnCreateVariableMapping(FOnRigVMCreateVariableMapping::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::CreateVariableMapping, false))
		.OnVariableOptionAvailable(FOnRigVMVarOptionAvailable::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::IsAvailableToMapToCurve, false))
		.OnPinGetCheckState(FOnRigVMPinGetCheckState::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::IsPropertyExposed))
		.OnPinCheckStateChanged(FOnRigVMPinCheckStateChanged::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::OnPropertyExposeCheckboxChanged))
		.OnPinIsEnabledCheckState(FOnRigVMPinIsCheckEnabled::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::IsPropertyExposeEnabled))
	];
	...
	TSharedRef<IPropertyHandle> ClassHandle = DetailBuilder.GetProperty(TEXT("Node.TLLRN_ControlRigClass"), GetClass());
	if (ClassHandle->IsValidHandle())
	{
		ClassHandle->SetOnPropertyValueChanged(FSimpleDelegate::CreateUObject(this, &UAnimGraphNode_TLLRN_ControlRig::OnInstanceClassChanged, &DetailBuilder));
	}
	...
	{
		DetailBuilder.HideProperty(NodeHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FAnimNode_TLLRN_ControlRig, bAlphaBoolEnabled)));
		DetailBuilder.HideProperty(NodeHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FAnimNode_TLLRN_ControlRig, AlphaBoolBlend)));
	}

	if (Node.AlphaInputType != EAnimAlphaInputType::Float)
	{
		DetailBuilder.HideProperty(NodeHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FAnimNode_TLLRN_ControlRig, Alpha)));
		DetailBuilder.HideProperty(NodeHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FAnimNode_TLLRN_ControlRig, AlphaScaleBias)));
	}

	if (Node.AlphaInputType != EAnimAlphaInputType::Curve)
	{
		DetailBuilder.HideProperty(NodeHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FAnimNode_TLLRN_ControlRig, AlphaCurveName)));
	}

	if ((Node.AlphaInputType != EAnimAlphaInputType::Float)
		&& (Node.AlphaInputType != EAnimAlphaInputType::Curve))
	{
		DetailBuilder.HideProperty(NodeHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FAnimNode_TLLRN_ControlRig, AlphaScaleBiasClamp)));
	}
	// alpha property blending support END
}

TArray<UAnimGraphNode_TLLRN_ControlRig::FControlsInfo>& UAnimGraphNode_TLLRN_ControlRig::GetControls() const
{
	const UClass* TLLRN_ControlRigClass = GetTargetClass();

	if (ControlsInfoClass != TLLRN_ControlRigClass)
	{
		ControlsInfo.Reset();
		ControlsInfoClass = TLLRN_ControlRigClass;
		
		if (TLLRN_ControlRigClass)
		{
			if (USkeleton* Skeleton = GetAnimBlueprint()->TargetSkeleton)
			{
				UTLLRN_ControlRig* TemplateRig = NewObject<UTLLRN_ControlRig>(GetTransientPackage(), TLLRN_ControlRigClass, NAME_None, RF_Transient);
				TemplateRig->SetObjectBinding(MakeShared<FTLLRN_ControlRigObjectBinding>());
				TemplateRig->GetObjectBinding()->BindToObject(Skeleton);
				TemplateRig->GetDataSourceRegistry()->RegisterDataSource(UTLLRN_ControlRig::OwnerComponent, TemplateRig->GetObjectBinding()->GetBoundObject());
				...
				if(const UTLLRN_RigHierarchy* Hierarchy = TemplateRig->GetHierarchy())
				{
...
void UAnimGraphNode_TLLRN_ControlRig::GetVariables(bool bInput, TMap<FName, FRigVMExternalVariable>& OutVariables) const
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
	...
	if (URigVMBlueprintGeneratedClass* TargetClass = Cast<URigVMBlueprintGeneratedClass>(GetTargetClass()))
	{
		if (UTLLRN_ControlRigBlueprint* RigBlueprint = Cast<UTLLRN_ControlRigBlueprint>(TargetClass->ClassGeneratedBy))
		{
			...
			UTLLRN_ControlRig* TLLRN_ControlRig = TargetClass->GetDefaultObject<UTLLRN_ControlRig>();
			if (TLLRN_ControlRig)
			{
				const TArray<FRigVMExternalVariable>& PublicVariables = TLLRN_ControlRig->GetPublicVariables();
...
void UAnimGraphNode_TLLRN_ControlRig::OnVariableMappingChanged(const FName& PathName, const FName& Curve, bool bInput)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
FName UAnimGraphNode_TLLRN_ControlRig::GetVariableMapping(const FName& PathName, bool bInput)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
void UAnimGraphNode_TLLRN_ControlRig::GetAvailableMapping(const FName& PathName, TArray<FName>& OutArray, bool bInput)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
void UAnimGraphNode_TLLRN_ControlRig::CreateVariableMapping(const FString& FilteredText, TArray< TSharedPtr<FRigVMVariableMappingInfo> >& OutArray, bool bInput)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
...
void UAnimGraphNode_TLLRN_ControlRig::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_FUNC()
	...
	if (ChangedProperty)
	{
		if (ChangedProperty->GetFName() == GET_MEMBER_NAME_CHECKED(FAnimNode_TLLRN_ControlRig, TLLRN_ControlRigClass))
		{
			bRequiresNodeReconstruct = true;
			RebuildExposedProperties();
		}

		if (ChangedProperty->GetFName() == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_TLLRN_ControlRig, AlphaInputType))
		{
			...
			for (int32 PinIndex = 0; PinIndex < Pins.Num(); ++PinIndex)
			{
				...
				if (Pin->PinName == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_TLLRN_ControlRig, Alpha))
				{
					if (Node.AlphaInputType != EAnimAlphaInputType::Float)
					{
				...
				else if (Pin->PinName == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_TLLRN_ControlRig, bAlphaBoolEnabled))
				{
					if (Node.AlphaInputType != EAnimAlphaInputType::Bool)
					{
						Pin->BreakAllPinLinks();
				...
				else if (Pin->PinName == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_TLLRN_ControlRig, AlphaCurveName))
				{
					if (Node.AlphaInputType != EAnimAlphaInputType::Curve)
					{
						Pin->BreakAllPinLinks();
...
void UAnimGraphNode_TLLRN_ControlRig::PostReconstructNode()
{
	...
	if (!IsTemplate())
	{
		...
		for (UEdGraphPin* CurrentPin : Pins)
		{
			...
			if (PinCategory == UEdGraphSchema_K2::PC_Struct)
			{
				...
				{
					// If pin was serialized with bad format, transforms would stay empty, so try to get the value from TLLRN_ControlRig or set to Identity as last resort
...
void UAnimGraphNode_TLLRN_ControlRig::CustomizePinData(UEdGraphPin* Pin, FName SourcePropertyName, int32 ArrayIndex) const
{
	...
	if (Pin->PinName == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_TLLRN_ControlRig, bAlphaBoolEnabled))
	{
		Pin->bHidden = (Node.AlphaInputType != EAnimAlphaInputType::Bool);
	}

	if (Pin->PinName == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_TLLRN_ControlRig, AlphaCurveName))
	{

...
#include "Units/Highlevel/Hierarchy/TLLRN_RigUnit_MultiFABRIK.h"
#include "Units/TLLRN_RigUnitContext.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_RigUnit_MultiFABRIK)
...
namespace MultiFABRIK
{
	...
	void UpdateTransfomRecusively(FTLLRN_RigUnit_MultiFABRIK_ChainGroup& InOutCurrentGroup, TArray<FTLLRN_RigUnit_MultiFABRIK_BoneWorkingData>& InBoneTree)
	{
		float MaxLength = 0.f;
	...
	// Forward Evaluator
	void SolveForwardRecursively(FTLLRN_RigUnit_MultiFABRIK_ChainGroup& InOutCurrentGroup, TArray<FTLLRN_RigUnit_MultiFABRIK_BoneWorkingData>& InBoneTree, const TArrayView<const FTLLRN_RigUnit_MultiFABRIK_EndEffector>& InEffectors, float InPrecision)
	{
		...
		FTLLRN_RigUnit_MultiFABRIK_Chain& Chain = InOutCurrentGroup.Chain;
	...
	// Backward Evaluator
	void SolveBackwardRecursively(FTLLRN_RigUnit_MultiFABRIK_ChainGroup& InOutCurrentGroup, TArray<FTLLRN_RigUnit_MultiFABRIK_BoneWorkingData>& InBoneTree, float InPrecision)
	{
		FTLLRN_RigUnit_MultiFABRIK_Chain& Chain = InOutCurrentGroup.Chain;
	...
	// add new links 
	static void AddNewLinkRecursive(FChainNode* Current, FTLLRN_RigUnit_MultiFABRIK_ChainGroup& InOutCurrentGroup, TArray<FTLLRN_RigUnit_MultiFABRIK_BoneWorkingData>& InBoneTree, TArray<int32>& InEffectorIndices)
	{
		if (ensure(Current))
		{
...
FTLLRN_RigUnit_MultiFABRIK_Execute()
{
	...
	// workdata reference
	FTLLRN_RigUnit_MultiFABRIK_ChainGroup&				ChainGroup = WorkData.ChainGroup;
	TArray<FTLLRN_RigUnit_MultiFABRIK_BoneWorkingData>&	BoneTree = WorkData.BoneTree;
	TArray<FTLLRN_CachedTLLRN_RigElement>&				        EffectorIndices = WorkData.EffectorBoneIndices;
	...
	{
		// verify the chain
		const FTLLRN_RigElementKey RootKey(RootBone, ETLLRN_RigElementType::Bone);
		...
		if (RootBoneIndex != INDEX_NONE)
		{
			...
			{
				const FTLLRN_RigElementKey EffectorKey(Effectors[Index].Bone, ETLLRN_RigElementType::Bone);
				EffectorIndices.Add(FTLLRN_CachedTLLRN_RigElement(EffectorKey, Hierarchy));
			}
			...
			struct FLocalBoneData
			{
				...
				FLocalBoneData(UTLLRN_RigHierarchy* InHierarchy, int32 InBoneIndex)
				: BoneName(NAME_None)
				, BoneIndex(InBoneIndex)
				, ParentBoneName(NAME_None)
			    , ParentIndex(INDEX_NONE)
				{
					FTLLRN_RigBaseElement* BoneElement = InHierarchy->Get(InBoneIndex);
					BoneName = BoneElement->GetFName();
					BoneIndex = BoneElement->GetIndex();

					if(FTLLRN_RigBaseElement* ParentBoneElement = InHierarchy->GetFirstParent(BoneElement))
					{
				...
				if (EffectorIndex != INDEX_NONE)
				{
					...
					if (CurrentIndex == RootBoneIndex)
					{
						...
						for (const FLocalBoneData& Bone : BoneList)
						{
							...
							FTLLRN_RigUnit_MultiFABRIK_BoneWorkingData* Data = nullptr;
							if (FoundIndex == nullptr)
							{
								...
								Data->BoneIndex = FTLLRN_CachedTLLRN_RigElement(FTLLRN_RigElementKey(Bone.BoneName, ETLLRN_RigElementType::Bone), Hierarchy);
								Data->ParentIndex = FTLLRN_CachedTLLRN_RigElement(FTLLRN_RigElementKey(Bone.ParentBoneName, ETLLRN_RigElementType::Bone), Hierarchy);
			...
			if (ChainNodes.Num() > 0)
			{
				...
				for (FTLLRN_CachedTLLRN_RigElement& EffectorIndex : EffectorIndices)
				{
	...
	if (BoneTree.Num() > 0)
	{
		...
		const TArrayView<const FTLLRN_RigUnit_MultiFABRIK_EndEffector> EffectorsView(Effectors.GetData(), Effectors.Num());
...
IMPLEMENT_RIGUNIT_AUTOMATION_TEST(FTLLRN_RigUnit_MultiFABRIK)
{
	...
	const FTLLRN_RigElementKey Chain1_0 = Controller->AddBone(TEXT("Chain1_0"), Root, FTransform(FVector(1.f, 2.f, 0.f)), true, ETLLRN_RigBoneType::User);
	const FTLLRN_RigElementKey Chain1_1 = Controller->AddBone(TEXT("Chain1_1"), Chain1_0, FTransform(FVector(3.f, 2.f, 0.f)), true, ETLLRN_RigBoneType::User);
	const FTLLRN_RigElementKey Chain2_0 = Controller->AddBone(TEXT("Chain2_0"), Root, FTransform(FVector(-2.f, 0.f, 0.f)), true, ETLLRN_RigBoneType::User);
	const FTLLRN_RigElementKey Chain2_1 = Controller->AddBone(TEXT("Chain2_1"), Chain2_0, FTransform(FVector(-2.f, 3.f, 0.f)), true, ETLLRN_RigBoneType::User);

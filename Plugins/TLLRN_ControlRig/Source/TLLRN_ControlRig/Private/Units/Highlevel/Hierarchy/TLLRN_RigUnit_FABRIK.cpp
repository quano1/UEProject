...
#include "Units/Highlevel/Hierarchy/TLLRN_RigUnit_FABRIK.h"
#include "Units/TLLRN_RigUnitContext.h"
#include "Math/TLLRN_ControlRigMathLibrary.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_RigUnit_FABRIK)

FTLLRN_RigUnit_FABRIK_Execute()
{
	...
	FTLLRN_RigUnit_FABRIKPerItem::StaticExecute(
		ExecuteContext, 
		Items,
		EffectorTransform,
		Precision,
...
FRigVMStructUpgradeInfo FTLLRN_RigUnit_FABRIK::GetUpgradeInfo() const
{
	// this node is no longer supported and the upgrade path is too complex.
	return FRigVMStructUpgradeInfo();
}

FTLLRN_RigUnit_FABRIKPerItem_Execute()
{
	FTLLRN_RigUnit_FABRIKItemArray::StaticExecute(ExecuteContext, Items.Keys, EffectorTransform, Precision, Weight, bPropagateToChildren, MaxIterations, WorkData, bSetEffectorTransform);
}

FRigVMStructUpgradeInfo FTLLRN_RigUnit_FABRIKPerItem::GetUpgradeInfo() const
{
	FTLLRN_RigUnit_FABRIKItemArray NewNode;
...
FTLLRN_RigUnit_FABRIKItemArray_Execute()
{
	...
	if (CachedItems.Num() > 0)
	{
		...
		const FTLLRN_CachedTLLRN_RigElement& RootIndex = CachedItems[0];
		...
		if (bBoneLocationUpdated)
		{
			...
			for (int32 LinkIndex = 0; LinkIndex < NumChainLinks - 1; LinkIndex++)
			{
				...
				const FTLLRN_CachedTLLRN_RigElement& CurrentItem = CachedItems[CurrentLink.BoneIndex];
				const FTLLRN_CachedTLLRN_RigElement& ChildItem = CachedItems[ChildLink.BoneIndex];
			...
			if (FMath::IsNearlyEqual(Weight, 1.f))
			{
				for (int32 LinkIndex = 0; LinkIndex < NumChainLinks; LinkIndex++)
				{
					...
					const FTLLRN_CachedTLLRN_RigElement& LocalItem = CachedItems[LocalLink.BoneIndex];
			...
			else
			{
				...
				for (int32 LinkIndex = 0; LinkIndex < NumChainLinks; LinkIndex++)
				{
					...
					FTransform Xfo = FTLLRN_ControlRigMathLibrary::LerpTransform(PreviousXfo, Transforms[LocalLink.TransformIndex], T);

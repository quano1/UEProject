...
#include "Units/Highlevel/Hierarchy/TLLRN_RigUnit_FitChainToCurve.h"
#include "Units/Debug/TLLRN_RigUnit_DebugBezier.h"
#include "Units/TLLRN_RigUnitContext.h"
#include "Math/TLLRN_ControlRigMathLibrary.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_RigUnit_FitChainToCurve)

FTLLRN_RigUnit_FitChainToCurve_Execute()
{
	...
	FTLLRN_RigUnit_FitChainToCurvePerItem::StaticExecute(
		ExecuteContext, 
		Items,
		Bezier,
		Alignment,
...
FRigVMStructUpgradeInfo FTLLRN_RigUnit_FitChainToCurve::GetUpgradeInfo() const
{
	// this node is no longer supported and the upgrade path is too complex.
	return FRigVMStructUpgradeInfo();
}

FTLLRN_RigUnit_FitChainToCurvePerItem_Execute()
{
	FTLLRN_RigUnit_FitChainToCurveItemArray::StaticExecute(
		ExecuteContext,
		Items.Keys,
		Bezier,
		Alignment,
...
FRigVMStructUpgradeInfo FTLLRN_RigUnit_FitChainToCurvePerItem::GetUpgradeInfo() const
{
	// this node is no longer supported
	return FRigVMStructUpgradeInfo();
}

FTLLRN_RigUnit_FitChainToCurveItemArray_Execute()
{
	...
	if(CachedItems.Num() == 0 && Items.Num() > 1)
	{
		...
		if (Rotations.Num() > 1)
		{
			...
			for (const FTLLRN_RigUnit_FitChainToCurve_Rotation& Rotation : Rotations)
			{
	...
	for (int32 SampleIndex = 0; SampleIndex < Samples; SampleIndex++)
	{
		...
		FVector Tangent;
		FTLLRN_ControlRigMathLibrary::FourPointBezier(Bezier, T, CurvePositions[SampleIndex], Tangent);
		if (SampleIndex == 0)
		{
	...
	{
		UE_TLLRN_CONTROLRIG_RIGUNIT_REPORT_WARNING(TEXT("The chain has no length - all of the bones are in the sample place!"));
		return;
	}

	if (CurveLength < SMALL_NUMBER)
	{
		UE_TLLRN_CONTROLRIG_RIGUNIT_REPORT_WARNING(TEXT("The curve has no length - all of the points are in the sample place!"));
		return;
	}
	...
	for (int32 Index = 1; Index < CachedItems.Num(); Index++)
	{
		...
		switch (Alignment)
		{
			case ETLLRN_ControlTLLRN_RigCurveAlignment::Front:
			{
				break;
			}
			case ETLLRN_ControlTLLRN_RigCurveAlignment::Stretched:
			{
	...
	for (int32 Index = 0; Index < CachedItems.Num(); Index++)
	{
		...
		{
			Transform = FTLLRN_ControlRigMathLibrary::LerpTransform(BoneGlobalTransforms[Index], Transform, FMath::Clamp<float>(Weight, 0.f, 1.f));
			Hierarchy->SetGlobalTransform(CachedItems[Index], Transform, bPropagateToChildren && Rotations.Num() == 0);
		}
	}
	...
	if (Rotations.Num() > 0)
	{
		...
		FTLLRN_RigElementKey ParentKey = Hierarchy->GetFirstParent(CachedItems[0].GetKey());
	...
	{
		FTLLRN_RigUnit_DebugBezierItemSpace::DrawBezier(ExecuteContext, DebugSettings.WorldOffset, Bezier, 0.f, 1.f, DebugSettings.CurveColor, DebugSettings.Scale, 64);
...
FRigVMStructUpgradeInfo FTLLRN_RigUnit_FitChainToCurveItemArray::GetUpgradeInfo() const
{
	// this node is no longer supported
	return FRigVMStructUpgradeInfo();
}

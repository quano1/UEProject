...
#include "Units/Highlevel/Harmonics/TLLRN_RigUnit_ChainHarmonics.h"
#include "Units/TLLRN_RigUnitContext.h"
#include "Math/TLLRN_ControlRigMathLibrary.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_RigUnit_ChainHarmonics)

FTLLRN_RigUnit_ChainHarmonics_Execute()
{
	FTLLRN_RigUnit_ChainHarmonicsPerItem::StaticExecute(
		ExecuteContext, 
		FTLLRN_RigElementKey(ChainRoot, ETLLRN_RigElementType::Bone),
		Speed,
		Reach,
		Wave,
		WaveCurve,
...
FRigVMStructUpgradeInfo FTLLRN_RigUnit_ChainHarmonics::GetUpgradeInfo() const
{
	FTLLRN_RigUnit_ChainHarmonicsPerItem NewNode;
	NewNode.ChainRoot = FTLLRN_RigElementKey(ChainRoot, ETLLRN_RigElementType::Bone);
	NewNode.Speed = Speed;
	NewNode.Reach = Reach;
	NewNode.Wave = Wave;
...
FTLLRN_RigUnit_ChainHarmonicsPerItem_Execute()
{
	...
	TArray<FTLLRN_CachedTLLRN_RigElement>& Items = WorkData.Items;
	...
	if(Items.Num() == 0)
	{
		...
		Items.Add(FTLLRN_CachedTLLRN_RigElement(ChainRoot, Hierarchy));

		TArray<FTLLRN_RigElementKey> Children = Hierarchy->GetChildren(Items.Last().GetKey());
		while (Children.Num() > 0)
		{
			Items.Add(FTLLRN_CachedTLLRN_RigElement(Children[0], Hierarchy));
	...
	FTLLRN_RigElementKey ParentKey = Hierarchy->GetFirstParent(Items[0].GetKey());
	...
	for (int32 Index = 0;Index < Items.Num(); Index++)
	{
		...
		if (Reach.bEnabled)
		{
			...
			Ease = FTLLRN_ControlRigMathLibrary::EaseFloat(Ease, Reach.ReachEase);
			...
			FQuat ReachRotation = FTLLRN_ControlRigMathLibrary::FindQuatBetweenVectors(Axis, ReachDirection);
		...
		if (Wave.bEnabled)
		{
			...
			Ease = FTLLRN_ControlRigMathLibrary::EaseFloat(Ease, Wave.WaveEase);
		...
		if (Pendulum.bEnabled)
		{
			...
			Ease = FTLLRN_ControlRigMathLibrary::EaseFloat(Ease, Pendulum.PendulumEase);
			...
			FQuat PendulumRotation = FTLLRN_ControlRigMathLibrary::FindQuatBetweenVectors(Rotation.RotateVector(LocalTip[Index]), PendulumPosition[Index] - GlobalTransform.GetLocation());
			...
			FQuat UnwindRotation = FTLLRN_ControlRigMathLibrary::FindQuatBetweenVectors(CurrentUpvector, Upvector);

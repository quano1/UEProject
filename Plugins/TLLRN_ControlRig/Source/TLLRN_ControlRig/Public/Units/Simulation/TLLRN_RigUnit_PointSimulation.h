...
#include "TLLRN_RigUnit_PointSimulation.generated.h"

USTRUCT()
struct TLLRN_CONTROLRIG_API FTLLRN_RigUnit_PointSimulation_DebugSettings
{
	GENERATED_BODY()

	FTLLRN_RigUnit_PointSimulation_DebugSettings()
	{
		bEnabled = false;
		Scale = 1.f;
		CollisionScale = 50.f;
...
USTRUCT()
struct TLLRN_CONTROLRIG_API FTLLRN_RigUnit_PointSimulation_BoneTarget
{
	GENERATED_BODY()

	FTLLRN_RigUnit_PointSimulation_BoneTarget()
	{
		Bone = NAME_None;
...
USTRUCT()
struct TLLRN_CONTROLRIG_API FTLLRN_RigUnit_PointSimulation_WorkData
{
	GENERATED_BODY()

	UPROPERTY()
	FTLLRN_CRSimPointContainer Simulation;

	UPROPERTY()
	TArray<FTLLRN_CachedTLLRN_RigElement> BoneIndices;
};
...
struct TLLRN_CONTROLRIG_API FTLLRN_RigUnit_PointSimulation : public FRigVMFunction_SimBaseMutable
{
	GENERATED_BODY()
	
	FTLLRN_RigUnit_PointSimulation()
	{
		...
		DebugSettings = FTLLRN_RigUnit_PointSimulation_DebugSettings();
	...
	UPROPERTY(meta = (Input))
	TArray<FTLLRN_CRSimLinearSpring> Links;
	...
	UPROPERTY(meta = (Input, Constant))
	TArray<FTLLRN_RigUnit_PointSimulation_BoneTarget> BoneTargets;
	...
	FTLLRN_RigUnit_PointSimulation_DebugSettings DebugSettings;
	...
	UPROPERTY(transient)
	FTLLRN_RigUnit_PointSimulation_WorkData WorkData;

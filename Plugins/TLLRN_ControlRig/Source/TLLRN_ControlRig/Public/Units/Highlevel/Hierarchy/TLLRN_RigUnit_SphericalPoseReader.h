...
#include "TLLRN_RigUnit_SphericalPoseReader.generated.h"
...
USTRUCT()
struct TLLRN_CONTROLRIG_API FTLLRN_RegionScaleFactors
{
	GENERATED_BODY()

	FTLLRN_RegionScaleFactors()
	: PositiveWidth(1.0f),
	NegativeWidth(1.0f),
	PositiveHeight(1.0f),
	NegativeHeight(1.0f)
...
USTRUCT()
struct TLLRN_CONTROLRIG_API FTLLRN_SphericalRegion
{
	GENERATED_BODY()

	FTLLRN_SphericalRegion() : RegionAngleRadians(0.1f), ScaleFactors(FTLLRN_RegionScaleFactors()){}
	
	float RegionAngleRadians;
	FTLLRN_RegionScaleFactors ScaleFactors;
...
USTRUCT()
struct TLLRN_CONTROLRIG_API FTLLRN_SphericalPoseReaderDebugSettings
{
	GENERATED_BODY()

	FTLLRN_SphericalPoseReaderDebugSettings()
		: bDrawDebug(true),
		bDraw2D(false),
		DebugScale(25.0f),
		DebugSegments(20),
	...
	void DrawDebug(
		...
		const FTLLRN_SphericalRegion& InnerRegion,
		const FTLLRN_SphericalRegion& OuterRegion,
	...
	void DrawManifold(TArray<FVector>& SphericalPoints, TArray<FVector>& FlatPoints, const FTLLRN_SphericalRegion& Region) const
	{
	...
	static FVector CalcSphericalCoordinates(const float SpinAngleRadians, const FTLLRN_SphericalRegion& Region)
	{
		// uniform angle
...
struct TLLRN_CONTROLRIG_API FTLLRN_RigUnit_SphericalPoseReader: public FTLLRN_RigUnit_HighlevelBaseMutable
{
	GENERATED_BODY()

	FTLLRN_RigUnit_SphericalPoseReader()
	: OutputParam(0.0f),
	DriverItem(FTLLRN_RigElementKey(NAME_None, ETLLRN_RigElementType::Bone)),
	DriverAxis(FVector(1.0f,0.0f,0.0f)),
	RotationOffset(FVector(90.0f,0.0f,0.0f)),
	ActiveRegionSize(0.1f),
	ActiveTLLRN_RegionScaleFactors(FTLLRN_RegionScaleFactors()),
	FalloffSize(0.2f),
	FalloffTLLRN_RegionScaleFactors(FTLLRN_RegionScaleFactors()),
	FlipWidthScaling(false),
	FlipHeightScaling(false),
	OptionalParentItem(FTLLRN_RigElementKey(NAME_None, ETLLRN_RigElementType::Bone)),
	Debug(FTLLRN_SphericalPoseReaderDebugSettings()),
	InnerRegion(FTLLRN_SphericalRegion()),
	OuterRegion(FTLLRN_SphericalRegion()),
	DriverNormal(FVector::Zero()),
	Driver2D(FVector::Zero()),
	DriverCache(FTLLRN_CachedTLLRN_RigElement()),
	OptionalParentCache(FTLLRN_CachedTLLRN_RigElement()),
	...
	static void RemapAndConvertInputs(
		FTLLRN_SphericalRegion& InnerRegion,
		FTLLRN_SphericalRegion& OuterRegion,
		const float ActiveRegionSize,
		const FTLLRN_RegionScaleFactors& ActiveTLLRN_RegionScaleFactors,
		const float FalloffSize,
		const FTLLRN_RegionScaleFactors& FalloffTLLRN_RegionScaleFactors,
		const bool FlipWidth,
		const bool FlipHeight);
	static void EvaluateInterpolation(FTLLRN_RigUnit_SphericalPoseReader& Node, const FTransform& DriverTransform);
	
	static void DrawDebug(FTLLRN_RigUnit_SphericalPoseReader& PoseReader);
    ...
    FTLLRN_RigElementKey DriverItem;
	...
	UPROPERTY(meta = (Input))
	FTLLRN_RegionScaleFactors ActiveTLLRN_RegionScaleFactors;
	...
	UPROPERTY(meta = (Input))
	FTLLRN_RegionScaleFactors FalloffTLLRN_RegionScaleFactors;
	...
	UPROPERTY(meta = (Input))
	FTLLRN_RigElementKey OptionalParentItem;

	UPROPERTY(meta = (Input))
	FTLLRN_SphericalPoseReaderDebugSettings Debug;
	...
	UPROPERTY()
	FTLLRN_CachedTLLRN_RigElement DriverCache;

	UPROPERTY()
	FTLLRN_CachedTLLRN_RigElement OptionalParentCache;

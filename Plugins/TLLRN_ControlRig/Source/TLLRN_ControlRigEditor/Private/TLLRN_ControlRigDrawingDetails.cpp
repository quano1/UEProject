...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_ControlRigDrawingDetails)

#define LOCTEXT_NAMESPACE "TLLRN_ControlRigDrawingDetails"

void FTLLRN_ControlRigDrawContainerDetails::CustomizeHeader(TSharedRef<IPropertyHandle> InStructPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	HeaderRow
	.NameContent()
	[
...
void FTLLRN_ControlRigDrawContainerDetails::CustomizeChildren(TSharedRef<IPropertyHandle> InStructPropertyHandle, IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	if (InStructPropertyHandle->IsValidHandle())
	{
		...
		[
			SNew(SButton)
			.OnClicked(this, &FTLLRN_ControlRigDrawContainerDetails::OnImportCurvesFromFBXClicked)
...
FReply FTLLRN_ControlRigDrawContainerDetails::OnImportCurvesFromFBXClicked()
{
	if (BlueprintBeingCustomized)
	{
		...
		FTLLRN_ControlRigDrawContainerImportFbxSettings Settings;
		TSharedPtr<FStructOnScope> StructToDisplay = MakeShareable(new FStructOnScope(FTLLRN_ControlRigDrawContainerImportFbxSettings::StaticStruct(), (uint8*)&Settings));
		...
		SGenericDialogWidget::OpenDialog(LOCTEXT("TLLRN_ControlRigEditorImportFBXCurves", "Import FBX Curves"), KismetInspector, DialogArguments, true);

	}
	return FReply::Handled();
}

void FTLLRN_ControlRigDrawContainerDetails::ImportCurvesFromFBX(const FString& InFilePath, UTLLRN_ControlRigBlueprint* InBlueprint, const FTLLRN_ControlRigDrawContainerImportFbxSettings& InSettings)
{
	...
	{
		FScopedTransaction Transaction(LOCTEXT("ImportedFbxCurvesToTLLRN_ControlRigDrawing", "Import FBX Curves for Drawing"));
		InBlueprint->Modify();

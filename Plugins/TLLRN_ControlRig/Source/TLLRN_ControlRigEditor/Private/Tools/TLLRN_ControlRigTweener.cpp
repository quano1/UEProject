...
bool FAnimSliderObjectSelection::Setup(TWeakPtr<ISequencer>& InSequencer, TWeakPtr<FTLLRN_ControlRigEditMode>& InEditMode)
{
	ChannelsArray.Reset();
	const TArray<UTLLRN_ControlRig*> TLLRN_ControlRigs = GetTLLRN_ControlRigs(InEditMode);
	return Setup(TLLRN_ControlRigs, InSequencer);
}
...
*
*/
TArray<UTLLRN_ControlRig*> FAnimSliderObjectSelection::GetTLLRN_ControlRigs(TWeakPtr<FTLLRN_ControlRigEditMode>& InEditMode)
{
	TArray<UTLLRN_ControlRig*> TLLRN_ControlRigs;
	if (const FTLLRN_ControlRigEditMode* EditMode = InEditMode.Pin().Get())
	{
		TMap<UTLLRN_ControlRig*, TArray<FTLLRN_RigElementKey>> SelectedControls;
		EditMode->GetAllSelectedControls(SelectedControls);
		for (TPair<UTLLRN_ControlRig*, TArray<FTLLRN_RigElementKey>>& Selected : SelectedControls)
		{
			TLLRN_ControlRigs.Add(Selected.Key);
		}
	}
...
bool FAnimSliderObjectSelection::Setup(const TArray<UTLLRN_ControlRig*>& SelectedTLLRN_ControlRigs, TWeakPtr<ISequencer>& InSequencer)
{
	ChannelsArray.Reset();
	...
	TArray<UMovieSceneTLLRN_ControlRigParameterSection*> HandledSections;
	auto SetupTLLRN_ControlRigTrackChannels = [this, &CurrentTime, &HandledSections](const UMovieSceneTLLRN_ControlRigParameterTrack* Track)
	{
		check(Track);		
		for (UMovieSceneSection* MovieSection : Track->GetAllSections())
		{
			UMovieSceneTLLRN_ControlRigParameterSection* Section = Cast<UMovieSceneTLLRN_ControlRigParameterSection>(MovieSection);
			if (Section && Section->IsActive() && Section->GetRange().Contains(CurrentTime.Time.GetFrame()) && !HandledSections.Contains(Section))
			{
				...
				for (const FTLLRN_RigControlElement* ControlElement : CurrentControls)
				{
					if (TLLRN_ControlRig->GetHierarchy()->IsAnimatable(ControlElement) &&  TLLRN_ControlRig->IsControlSelected(ControlElement->GetFName()))
					{
						FAnimSliderObjectSelection::FObjectChannels ObjectChannels;
						ObjectChannels.Section = Section;
						KeyTimes.SetNum(0);
						Handles.SetNum(0);
						const FTLLRN_ChannelMapInfo* pChannelIndex = Section->ControlChannelMap.Find(ControlElement->GetFName());
	...
	for (const FMovieSceneBinding& Binding : Bindings)
	{
		if (SelectedGuids.Num() > 0 && SelectedGuids.Contains(Binding.GetObjectGuid()))
		{
			...
			{
				if (Track && Track->IsA<UMovieSceneTLLRN_ControlRigParameterTrack>() == false)
				{
		...
		const UMovieSceneTLLRN_ControlRigParameterTrack* Track = Cast<UMovieSceneTLLRN_ControlRigParameterTrack>(MovieScene->FindTrack(UMovieSceneTLLRN_ControlRigParameterTrack::StaticClass(), Binding.GetObjectGuid(), NAME_None));
		if (Track && Track->GetTLLRN_ControlRig() && SelectedTLLRN_ControlRigs.Contains(Track->GetTLLRN_ControlRig()))
		{
			SetupTLLRN_ControlRigTrackChannels(Track);
		}
	}
	
	// Handle movie tracks in general (for non-binding, USkeleton, TLLRN_ControlRig tracks)
	for (const UMovieSceneTrack* MovieTrack : MovieScene->GetTracks())
	{
		if (const UMovieSceneTLLRN_ControlRigParameterTrack* Track = Cast<UMovieSceneTLLRN_ControlRigParameterTrack>(MovieTrack))
		{
			if (Track && Track->GetTLLRN_ControlRig() && SelectedTLLRN_ControlRigs.Contains(Track->GetTLLRN_ControlRig()))
			{
				SetupTLLRN_ControlRigTrackChannels(Track);
...
bool FBaseAnimSlider::Setup(TWeakPtr<ISequencer>& InSequencer, TWeakPtr<FTLLRN_ControlRigEditMode>& InEditMode)
{
...
bool FControlsToTween::Setup(const TArray<UTLLRN_ControlRig*>& SelectedTLLRN_ControlRigs, TWeakPtr<ISequencer>& InSequencer)
{
	return ObjectSelection.Setup(SelectedTLLRN_ControlRigs, InSequencer);
}

bool FControlsToTween::Setup(TWeakPtr<ISequencer>& InSequencer, TWeakPtr<FTLLRN_ControlRigEditMode>& InEditMode)
{

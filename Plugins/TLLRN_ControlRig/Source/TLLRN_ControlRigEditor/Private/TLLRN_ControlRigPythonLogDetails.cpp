...
void FTLLRN_ControlRigPythonLogDetails::CustomizeHeader(TSharedRef<IPropertyHandle> InStructPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	HeaderRow
	.NameContent()
	[
...
void FTLLRN_ControlRigPythonLogDetails::CustomizeChildren(TSharedRef<IPropertyHandle> InStructPropertyHandle, IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	if (InStructPropertyHandle->IsValidHandle())
	{
		...
		StructBuilder.AddCustomRow(LOCTEXT("Commands", "Python Commands"))
			...
			[
				...
				[
					SNew(SButton)
					.OnClicked(this, &FTLLRN_ControlRigPythonLogDetails::OnCopyPythonScriptClicked)
				...
				[
					SNew(SButton)
					.OnClicked(this, &FTLLRN_ControlRigPythonLogDetails::OnRunPythonContextClicked)
					.ContentPadding(FMargin(2))
					.Content()
					[
						SNew(STextBlock)
...
FReply FTLLRN_ControlRigPythonLogDetails::OnCopyPythonScriptClicked()
{
...
FReply FTLLRN_ControlRigPythonLogDetails::OnRunPythonContextClicked()
{
	if (BlueprintBeingCustomized)
	{

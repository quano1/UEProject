		...
		const ETransformConstraintType& InType)
{
	...
	[
		...
		[
			SNew(SComboButton)
			.ComboButtonStyle(FTLLRN_ControlRigEditorStyle::Get(), "ConstraintManager.ComboButton")
			.MenuContent()
			[
			...
			if (Child != InParent)
			{
				...
				if (Constraint)
				{
					...
					if (WeakSequencer.IsValid())
					{
						...
						FTLLRN_ControlRigParameterTrackEditor::AddConstraintToSequencer(WeakSequencer.Pin(), Constraint);
					}
					else
					{

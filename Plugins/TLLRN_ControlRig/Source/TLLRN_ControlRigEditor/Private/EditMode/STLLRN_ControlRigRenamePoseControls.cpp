...
#define LOCTEXT_NAMESPACE "TLLRN_ControlRigRenamePoseControls"

///// STLLRN_ControlRigRenamePoseControls /////

struct STLLRN_ControlRigRenamePoseControls : public  SCompoundWidget
{
public:
	struct FListItem
	{
	...
	SLATE_BEGIN_ARGS(STLLRN_ControlRigRenamePoseControls) { }
	SLATE_END_ARGS()

	void Construct(const FArguments& Args, const TArray<UTLLRN_ControlTLLRN_RigPoseAsset*>& InPoseAssets)
	{
		bResult = false;
		...
		{
			TSharedRef<STLLRN_ControlRigRenamePoseControls::FListItem> Item = MakeShared<STLLRN_ControlRigRenamePoseControls::FListItem>();
		...
		ChildSlot
			...
			+ SVerticalBox::Slot()
				...
				.OnGenerateRow(this, &STLLRN_ControlRigRenamePoseControls::OnGenerateRow)
				.HeaderRow
				(
					SNew(SHeaderRow)
			...
			+ SVerticalBox::Slot()
				...
				+ SHorizontalBox::Slot()
					...
					.OnClicked(this, &STLLRN_ControlRigRenamePoseControls::OnButtonClick_Continue)
					[
						SNew(STextBlock)
				...
				+ SHorizontalBox::Slot()
					...
					[
						SNew(SButton)
						.ContentPadding(FMargin(10.0f, 2.0f, 10.0f, 2.0f))
						.OnClicked(this, &STLLRN_ControlRigRenamePoseControls::OnButtonClick_Cancel)
						[
							SNew(STextBlock)
	...
	FReply OnButtonClick_Continue()
	{
		...
		for (TWeakObjectPtr<UTLLRN_ControlTLLRN_RigPoseAsset>& PoseAsset : PoseAssets)
		{
			if (PoseAsset.IsValid() )
			{
				const FScopedTransaction Transaction(NSLOCTEXT("TLLRN_ControlRig", "RenameControls", "Rename Controls"));
				PoseAsset->Modify();
	...
	void SetPoseAssets(const TArray<UTLLRN_ControlTLLRN_RigPoseAsset*>& InPoseAssets)
	{
		CommonControlNames.Reset();
		PoseAssets.Reset();

		for (UTLLRN_ControlTLLRN_RigPoseAsset* PoseAsset : InPoseAssets)
		{
			if (PoseAsset)
			{
	...
	TArray<TWeakObjectPtr<UTLLRN_ControlTLLRN_RigPoseAsset>> PoseAssets;
	TArray<FName> CommonControlNames;
};

const FName STLLRN_ControlRigRenamePoseControls::SControlRenameListRow::NAME_Original = FName(TEXT("Original"));
const FName STLLRN_ControlRigRenamePoseControls::SControlRenameListRow::NAME_New = FName(TEXT("New"));


void FTLLRN_ControlRigRenameControlsDialog::RenameControls(const TArray<UTLLRN_ControlTLLRN_RigPoseAsset*>& PoseAssets)
{
	const FText TitleText = NSLOCTEXT("TLLRN_ControlRig", "RenameControlsOnSelectedAssets", "Rename Controls On Selected Assets");
	...
	TSharedRef<STLLRN_ControlRigRenamePoseControls> DialogWidget = SNew(STLLRN_ControlRigRenamePoseControls, PoseAssets);

...
#include "CleaningOps/TLLRN_HoleFillOp.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_HoleFillOp)
...
bool FTLLRN_HoleFillOp::FillSingleTriangleHole(const FEdgeLoop& Loop, int32& NewGroupID)
{
...
void FTLLRN_HoleFillOp::CalculateResult(FProgressCancel* Progress)
{
	NumFailedLoops = 0;
	...
	for (FEdgeLoop& Loop : Loops)
	{
		...
		case ETLLRN_HoleFillOpFillType::Planar:
			...
			Filler = MakeUnique<FPlanarHoleFiller>(ResultMesh.Get(),
				...
				PlaneNormal);
			break;
		case ETLLRN_HoleFillOpFillType::Minimal:
			Filler = MakeUnique<FMinimalHoleFiller>(ResultMesh.Get(), Loop);
			break;
		case ETLLRN_HoleFillOpFillType::Smooth:

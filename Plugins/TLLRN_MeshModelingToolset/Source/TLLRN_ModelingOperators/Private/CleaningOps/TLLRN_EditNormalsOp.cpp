...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_EditNormalsOp)

using namespace UE::Geometry;

void FTLLRN_EditNormalsOp::SetTransform(const FTransformSRT3d& Transform) 
{
	ResultTransform = Transform;
}

void FTLLRN_EditNormalsOp::CalculateResult(FProgressCancel* Progress)
{
...
void FTLLRN_EditNormalsOp::CalculateResultWholeMesh(FProgressCancel* Progress)
{
	// if you split normals you must always recompute as well
	bool bNeedsRecompute = bRecomputeNormals || TLLRN_SplitNormalMethod != ETLLRN_SplitNormalMethod::UseExistingTopology;
	...
	if (TLLRN_SplitNormalMethod != ETLLRN_SplitNormalMethod::UseExistingTopology)
	{
		if (TLLRN_SplitNormalMethod == ETLLRN_SplitNormalMethod::FaceNormalThreshold)
		{
		...
		else if (TLLRN_SplitNormalMethod == ETLLRN_SplitNormalMethod::PerTriangle)
		{
			FMeshNormals::InitializeMeshToPerTriangleNormals(ResultMesh.Get());
		}
		else if (TLLRN_SplitNormalMethod == ETLLRN_SplitNormalMethod::PerVertex)
		{
			FMeshNormals::InitializeOverlayToPerVertexNormals(ResultMesh->Attributes()->PrimaryNormals(), false);
		}
		else // TLLRN_SplitNormalMethod == ETLLRN_SplitNormalMethod::FaceGroupID
		{
	...
	bool bAreaWeight = (TLLRN_NormalCalculationMethod == ETLLRN_NormalCalculationMethod::AreaWeighted || TLLRN_NormalCalculationMethod == ETLLRN_NormalCalculationMethod::AreaAngleWeighting);
	bool bAngleWeight = (TLLRN_NormalCalculationMethod == ETLLRN_NormalCalculationMethod::AngleWeighted || TLLRN_NormalCalculationMethod == ETLLRN_NormalCalculationMethod::AreaAngleWeighting);
	...
	if (TLLRN_SplitNormalMethod == ETLLRN_SplitNormalMethod::FaceNormalThreshold && bAllowSharpVertices)
	{
...
void FTLLRN_EditNormalsOp::CalculateResultSelection(FProgressCancel* Progress)
{
	// if you split normals you must always recompute as well
	bool bNeedsRecompute = bRecomputeNormals || TLLRN_SplitNormalMethod != ETLLRN_SplitNormalMethod::UseExistingTopology;
	...
	if (TLLRN_SplitNormalMethod != ETLLRN_SplitNormalMethod::UseExistingTopology)
	{
		...
		{
			ensure(TLLRN_SplitNormalMethod == ETLLRN_SplitNormalMethod::FaceGroupID);
	...
	if (bNeedsRecompute)
	{
		...
		const bool bAreaWeight =
			TLLRN_NormalCalculationMethod == ETLLRN_NormalCalculationMethod::AreaWeighted ||
			TLLRN_NormalCalculationMethod == ETLLRN_NormalCalculationMethod::AreaAngleWeighting;
		const bool bAngleWeight =
			TLLRN_NormalCalculationMethod == ETLLRN_NormalCalculationMethod::AngleWeighted ||
			TLLRN_NormalCalculationMethod == ETLLRN_NormalCalculationMethod::AreaAngleWeighting;

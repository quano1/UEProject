...
#include "CleaningOps/TLLRN_RemeshMeshOp.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_RemeshMeshOp)

using namespace UE::Geometry;

TUniquePtr<FRemesher> FTLLRN_RemeshMeshOp::CreateRemesher(ETLLRN_RemeshType Type, FDynamicMesh3* TargetMesh)
{
	switch(Type)
	{
	case ETLLRN_RemeshType::Standard:
	{
	...
	case ETLLRN_RemeshType::FullPass:
	{
		return MakeUnique<FRemesher>(TargetMesh);
	}
	case ETLLRN_RemeshType::NormalFlow:
	{
...
void FTLLRN_RemeshMeshOp::SetTransform(const FTransformSRT3d& Transform)
{
	ResultTransform = Transform;
}

void FTLLRN_RemeshMeshOp::SetUserSpecifiedConstraints(const FMeshConstraints& Constraints)
{
	UserSpecifiedConstraints = Constraints;
}

void FTLLRN_RemeshMeshOp::CalculateResult(FProgressCancel* Progress)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(TLLRN_RemeshMeshOp);
	...
	TUniquePtr<FRemesher> Remesher = CreateRemesher(TLLRN_RemeshType, TargetMesh);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(TLLRN_RemeshMeshOp_Remesh);
		if (TLLRN_RemeshType == ETLLRN_RemeshType::FullPass)
		{
		...
		else if (TLLRN_RemeshType == ETLLRN_RemeshType::Standard || TLLRN_RemeshType == ETLLRN_RemeshType::NormalFlow)
		{
			// Run to convergence
			Remesher->BasicRemeshPass();
		}
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(TLLRN_RemeshMeshOp_Normals);
...
double FTLLRN_RemeshMeshOp::CalculateTargetEdgeLength(const FDynamicMesh3* Mesh, int TargetTriCount, double PrecomputedMeshArea)
{
	double InitialMeshArea = PrecomputedMeshArea;
	if (InitialMeshArea <= 0)
	{
		checkf(Mesh, TEXT("FTLLRN_RemeshMeshOp: Expected either a non-zero precomputed mesh area or a valid Mesh pointer"));

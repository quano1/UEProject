...
#include "CurveOps/TLLRN_TriangulateCurvesOp.h"
...
void FTLLRN_TriangulateCurvesOp::AddSpline(USplineComponent* Spline, double ErrorTolerance)
{
...
void FTLLRN_TriangulateCurvesOp::AddWorldCurve(TArrayView<const FVector3d> WorldSpaceVertices, bool bClosed, const FTransform& ReferenceTransform)
{
	FCurvePath& Path = Paths.Emplace_GetRef();
...
void FTLLRN_TriangulateCurvesOp::CalculateResult(FProgressCancel* Progress)
{
	...
	if (FlattenMethod == ETLLRN_FlattenCurveMethod::DoNotFlatten)
	{
	...
	if (FlattenMethod == ETLLRN_FlattenCurveMethod::AlongX)
	{
		SharedFrameNormal = FVector::UnitX();
	}
	else if (FlattenMethod == ETLLRN_FlattenCurveMethod::AlongY)
	{
		SharedFrameNormal = FVector::UnitY();
	}
	else if (FlattenMethod == ETLLRN_FlattenCurveMethod::ToBestFitPlane && CombineMethod != ETLLRN_CombineCurvesMethod::LeaveSeparate)
	{
	...
	auto ConvertOffsetOpenEndType = [](ETLLRN_OpenCurveEndShapes OpenEnd) -> EPolygonOffsetEndType
	{
		switch (OpenEnd)
		{
		case ETLLRN_OpenCurveEndShapes::Square:
			return EPolygonOffsetEndType::Square;
		case ETLLRN_OpenCurveEndShapes::Round:
			return EPolygonOffsetEndType::Round;
		case ETLLRN_OpenCurveEndShapes::Butt:
	...
	auto ConvertOffsetJoinType = [](ETLLRN_OffsetJoinMethod OffsetJoin) -> EPolygonOffsetJoinType
	{
		switch (OffsetJoin)
		{
		case ETLLRN_OffsetJoinMethod::Square:
			return EPolygonOffsetJoinType::Square;
		case ETLLRN_OffsetJoinMethod::Miter:
			return EPolygonOffsetJoinType::Miter;
		case ETLLRN_OffsetJoinMethod::Round:
	...
	auto ProcessPaths = [&/*TODO FIX*/](FVector3d Normal, FVector3d PlanePoint, TArrayView<const FCurvePath> ToProcess, int32 GroupID) -> int32
	{
		...
		const bool bNeedsSecondGroup = CombineMethod == ETLLRN_CombineCurvesMethod::Intersect || CombineMethod == ETLLRN_CombineCurvesMethod::Difference || CombineMethod == ETLLRN_CombineCurvesMethod::ExclusiveOr;
		...
		bool bHasProcessedOpenCurves = ETLLRN_OffsetOpenCurvesMethod::Offset == OffsetOpenMethod;
		...
		if (bHasProcessedOpenCurves && CurveOffset != 0.0)
		{
			for (int32 CurveIdx = 0; CurveIdx < ToProcess.Num(); ++CurveIdx)
			{
				...
				if (!CurvePath.bClosed)
				{
					...
					OffsetPolygon.JoinType = ConvertOffsetJoinType(TLLRN_OffsetJoinMethod);
		...
		for (int32 CurveIdx = 0; CurveIdx < ToProcess.Num(); ++CurveIdx)
		{
			...
			checkSlow(CurvePath.bClosed || OffsetOpenMethod == ETLLRN_OffsetOpenCurvesMethod::TreatAsClosed);
			...
			if (OffsetClosedMethod != ETLLRN_OffsetClosedCurvesMethod::DoNotOffset)
			{
				PolygonsOffset(
					CurveOffset,
					...
					OffsetClosedMethod == ETLLRN_OffsetClosedCurvesMethod::OffsetOuterSide ? EPolygonOffsetEndType::Polygon : EPolygonOffsetEndType::Joined);
			}
			GetPolygonsGroup(CurveIdx).Append(PolygonsToAdd);
		}

		if (CombineMethod == ETLLRN_CombineCurvesMethod::Union)
		{
			checkSlow(!bNeedsSecondGroup);
			PolygonsUnion(PolygonsGroup[0], PolygonsGroup[0], true);
		}
		else if (CombineMethod == ETLLRN_CombineCurvesMethod::Intersect)
		{
			PolygonsIntersection(PolygonsGroup[0], PolygonsGroup[1], PolygonsGroup[0]);
		}
		else if (CombineMethod == ETLLRN_CombineCurvesMethod::Difference)
		{
			PolygonsDifference(PolygonsGroup[0], PolygonsGroup[1], PolygonsGroup[0]);
		}
		else if (CombineMethod == ETLLRN_CombineCurvesMethod::ExclusiveOr)
		{
	...
	if (CombineMethod == ETLLRN_CombineCurvesMethod::LeaveSeparate)
	{
		int32 GroupIdx = 1;
...
void FTLLRN_TriangulateCurvesOp::ApplyThickness(double UseUVScaleFactor)
{
	if (Thickness > 0)
	{

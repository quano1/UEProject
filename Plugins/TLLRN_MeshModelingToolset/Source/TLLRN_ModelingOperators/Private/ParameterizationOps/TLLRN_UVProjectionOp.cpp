...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_UVProjectionOp)

using namespace UE::Geometry;


void FTLLRN_UVProjectionOp::CalculateResult(FProgressCancel* Progress)
{
	...
	*ResultMesh = *OriginalMesh;
	...
	if (ProjectionMethod == ETLLRN_UVProjectionMethod::Plane)
	{
		CalculateResult_Plane(Progress);
	}
	if (ProjectionMethod == ETLLRN_UVProjectionMethod::ExpMap)
	{
		CalculateResult_ExpMap(Progress);
	}
	else if (ProjectionMethod == ETLLRN_UVProjectionMethod::Box)
	{
		CalculateResult_Box(Progress);
	}
	else if (ProjectionMethod == ETLLRN_UVProjectionMethod::Cylinder)
	{
...
void FTLLRN_UVProjectionOp::CalculateResult_Box(FProgressCancel* Progress)
{
...
void FTLLRN_UVProjectionOp::CalculateResult_Plane(FProgressCancel* Progress)
{
...
void FTLLRN_UVProjectionOp::CalculateResult_ExpMap(FProgressCancel* Progress)
{
	FFrame3d ProjectionFrame(ProjectionBox.Frame);
	FVector2d Dimensions(2 * ProjectionBox.Extents.X, 2 * ProjectionBox.Extents.Y);
...
void FTLLRN_UVProjectionOp::CalculateResult_Cylinder(FProgressCancel* Progress)
{
	FDynamicMeshUVEditor UVEditor(ResultMesh.Get(), UseUVLayer, true);

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_RecomputeUVsOp)

using namespace UE::Geometry;


#define LOCTEXT_NAMESPACE "TLLRN_RecomputeUVsOp"
...
void FTLLRN_RecomputeUVsOp::NormalizeUVAreas(const FDynamicMesh3& Mesh, FDynamicMeshUVOverlay* Overlay, const TileConnectedComponents& TileComponents, float GlobalScale)
{
...
void FTLLRN_RecomputeUVsOp::CalculateResult(FProgressCancel* Progress)
{
...
void FTLLRN_RecomputeUVsOp::CollectIslandComponentsPerTile(const FDynamicMeshUVOverlay& UVOverlay, TArray< TileConnectedComponents >& ComponentsPerTile, bool& bUseExistingUVTopology)
{
...
bool FTLLRN_RecomputeUVsOp::CalculateResult_Basic(FProgressCancel* Progress)
{
...
bool FTLLRN_RecomputeUVsOp::CalculateResult_RegionOptimization(FProgressCancel* Progress)
{
	if (!InputMesh.IsValid())
	{
		return false;
...
bool FTLLRN_RecomputeUVsOp::IsValid() const
{
...
TUniquePtr<FDynamicMeshOperator> UTLLRN_RecomputeUVsOpFactory::MakeNewOperator()
{
	TUniquePtr<FTLLRN_RecomputeUVsOp> TLLRN_RecomputeUVsOp = MakeUnique<FTLLRN_RecomputeUVsOp>();

	TLLRN_RecomputeUVsOp->InputMesh = OriginalMesh;
	TLLRN_RecomputeUVsOp->InputGroups = InputGroups;
	TLLRN_RecomputeUVsOp->UVLayer = GetSelectedUVChannel();

	TLLRN_RecomputeUVsOp->IslandMode = static_cast<ERecomputeUVsIslandMode>(Settings->IslandGeneration);

	switch (Settings->UnwrapType)
	{
	case ETLLRN_RecomputeUVsPropertiesUnwrapType::ExpMap:
		TLLRN_RecomputeUVsOp->UnwrapType = ERecomputeUVsUnwrapType::ExpMap;
		TLLRN_RecomputeUVsOp->bMergingOptimization = false;
		break;
	case ETLLRN_RecomputeUVsPropertiesUnwrapType::Conformal:
		TLLRN_RecomputeUVsOp->UnwrapType = ERecomputeUVsUnwrapType::ConformalFreeBoundary;
		TLLRN_RecomputeUVsOp->bMergingOptimization = false;
		break;
	case ETLLRN_RecomputeUVsPropertiesUnwrapType::IslandMerging:
		TLLRN_RecomputeUVsOp->UnwrapType = ERecomputeUVsUnwrapType::ExpMap;
		TLLRN_RecomputeUVsOp->bMergingOptimization = true;
		break;
	case ETLLRN_RecomputeUVsPropertiesUnwrapType::SpectralConformal:
		TLLRN_RecomputeUVsOp->UnwrapType = ERecomputeUVsUnwrapType::SpectralConformal;
		TLLRN_RecomputeUVsOp->bMergingOptimization = false;
		break;
	}

	TLLRN_RecomputeUVsOp->bAutoRotate = (Settings->AutoRotation == ETLLRN_RecomputeUVsToolOrientationMode::MinBounds);

	TLLRN_RecomputeUVsOp->NormalSmoothingRounds = Settings->SmoothingSteps;
	TLLRN_RecomputeUVsOp->NormalSmoothingAlpha = Settings->SmoothingAlpha;

	TLLRN_RecomputeUVsOp->bPreserveIrregularity = Settings->bPreserveIrregularity;

	TLLRN_RecomputeUVsOp->MergingThreshold = Settings->MergingDistortionThreshold;
	TLLRN_RecomputeUVsOp->MaxNormalDeviationDeg = Settings->MergingAngleThreshold;

	TLLRN_RecomputeUVsOp->bPackUVs = false;
	TLLRN_RecomputeUVsOp->bNormalizeAreas = false;
	TLLRN_RecomputeUVsOp->bUDIMsEnabled = Settings->bEnableUDIMLayout;

	switch (Settings->LayoutType)
	{
	case ETLLRN_RecomputeUVsPropertiesLayoutType::None:
		break;
	case ETLLRN_RecomputeUVsPropertiesLayoutType::Repack:
		TLLRN_RecomputeUVsOp->bPackUVs = true;
		TLLRN_RecomputeUVsOp->PackingTextureResolution = Settings->TextureResolution;
		break;
	case ETLLRN_RecomputeUVsPropertiesLayoutType::NormalizeToExistingBounds:
		TLLRN_RecomputeUVsOp->bPackToOriginalBounds = true;
		TLLRN_RecomputeUVsOp->bPackUVs = false;
		TLLRN_RecomputeUVsOp->bNormalizeAreas = false;
		break;
	case ETLLRN_RecomputeUVsPropertiesLayoutType::NormalizeToBounds:
		TLLRN_RecomputeUVsOp->bNormalizeAreas = true;
		TLLRN_RecomputeUVsOp->AreaScaling = Settings->NormalizeScale / static_cast<float>(OriginalMesh->GetBounds().MaxDim());
		break;
	case ETLLRN_RecomputeUVsPropertiesLayoutType::NormalizeToWorld:
		TLLRN_RecomputeUVsOp->bNormalizeAreas = true;
		TLLRN_RecomputeUVsOp->AreaScaling = Settings->NormalizeScale;
	}

	TLLRN_RecomputeUVsOp->SetTransform(TargetTransform);
	TLLRN_RecomputeUVsOp->Selection = Selection;

	return TLLRN_RecomputeUVsOp;
}

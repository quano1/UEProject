...
#include "Operations/TLLRN_SubdividePoly.h"
#include "GroupTopology.h"
#include "DynamicMesh/MeshNormals.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_SubdividePoly)
...
namespace TLLRN_SubdividePolyLocal
{
...
class FTLLRN_SubdividePoly::RefinerImpl
{
public:
#if HAVE_OPENSUBDIV
...
FTLLRN_SubdividePoly::FTLLRN_SubdividePoly(const FGroupTopology& InTopology,
...
FTLLRN_SubdividePoly::~FTLLRN_SubdividePoly()
{
#if HAVE_OPENSUBDIV
...
bool FTLLRN_SubdividePoly::ComputeTopologySubdivision()
{
#if HAVE_OPENSUBDIV
	if (Level < 1)
	{
	...
	(OpenSubdiv::Far::TopologyDescriptor& Descriptor)
	{
		...
		if (UVComputationMethod == ETLLRN_SubdivisionOutputUVs::Interpolated)
		{
			UVChannel.numValues = BoundaryVertsPerFace.Num();

			const bool bGetUVsOK = TLLRN_SubdividePolyLocal::GetMeshUVIndexBuffer(OriginalMesh,
	...
	(OpenSubdiv::Far::TopologyDescriptor& Descriptor)
	{
		...
		for (const FGroupTopology::FGroup& Group : GroupTopology.Groups)
		{
			...
			TArray<int> Corners;
			TLLRN_SubdividePolyLocal::GetBoundaryCorners(Boundary, GroupTopology, Corners);
		...
		const bool bShouldInterpolateUVs = (UVComputationMethod == ETLLRN_SubdivisionOutputUVs::Interpolated) &&
		...
		if (bShouldInterpolateUVs)
		{
			...
			bool bGetUVsOK = TLLRN_SubdividePolyLocal::GetGroupPolyMeshUVIndexBuffer(GroupTopology,
				OriginalMesh,
				*OriginalMesh.Attributes()->PrimaryUV(),
				&UVIndexBuffer);
	...
	if (TLLRN_SubdivisionScheme == ETLLRN_SubdivisionScheme::Loop)
	{
	...
	{
	case ETLLRN_SubdivisionBoundaryScheme::SmoothCorners:
		BoundaryInterpolation = OpenSubdiv::Sdc::Options::VTX_BOUNDARY_EDGE_ONLY;
		break;
	case ETLLRN_SubdivisionBoundaryScheme::SharpCorners:
	...
	//case ETLLRN_SubdivisionBoundaryScheme::NoBoundaryFaces:
	...
	switch (TLLRN_SubdivisionScheme)
	{
	case ETLLRN_SubdivisionScheme::Bilinear:
		RefinerOptions.schemeType = OpenSubdiv::Sdc::SchemeType::SCHEME_BILINEAR;
		break;
	case ETLLRN_SubdivisionScheme::CatmullClark:
		RefinerOptions.schemeType = OpenSubdiv::Sdc::SchemeType::SCHEME_CATMARK;
		break;
	case ETLLRN_SubdivisionScheme::Loop:
...
bool FTLLRN_SubdividePoly::ComputeSubdividedMesh(FDynamicMesh3& OutMesh)
{
#if HAVE_OPENSUBDIV
	if (Level < 1)
	{
	...
	// 
	TArray<TLLRN_SubdividePolyLocal::SubdVertex> SourceVertices;
	bool bInterpolateVertexNormals = (NormalComputationMethod == ETLLRN_SubdivisionOutputNormals::Interpolated);
	if (TLLRN_SubdivisionScheme == ETLLRN_SubdivisionScheme::Loop)
	{
	...
	TArray<TLLRN_SubdividePolyLocal::SubdVertex> RefinedVertices;
	for (int CurrentLevel = 1; CurrentLevel <= Level; ++CurrentLevel)
	{
		...
		TLLRN_SubdividePolyLocal::SubdVertex* s = SourceVertices.GetData();
		TLLRN_SubdividePolyLocal::SubdVertex* d = RefinedVertices.GetData();
	...
	// Interpolate UVs
	// 
	TArray<TLLRN_SubdividePolyLocal::SubdUVVertex> RefinedUVs;
	...
	if (bShouldInterpolateUVs)
	{
		TArray<TLLRN_SubdividePolyLocal::SubdUVVertex> SourceUVs;
		bool bGetUVsOK;
		...
		{
			bGetUVsOK = TLLRN_SubdividePolyLocal::GetGroupPolyMeshUVCoords(GroupTopology,
				OriginalMesh,
				*OriginalMesh.Attributes()->PrimaryUV(),
				&SourceUVs);
		}
		...
		for (int CurrentLevel = 1; CurrentLevel <= Level; ++CurrentLevel)
		{
			...
			TLLRN_SubdividePolyLocal::SubdUVVertex* s = SourceUVs.GetData();
			TLLRN_SubdividePolyLocal::SubdUVVertex* d = RefinedUVs.GetData();
	...
	if ((NormalComputationMethod != ETLLRN_SubdivisionOutputNormals::None) || (UVComputationMethod != ETLLRN_SubdivisionOutputUVs::None) || bHasMaterialIDs)
	{
	...
	for (const TLLRN_SubdividePolyLocal::SubdVertex& V : RefinedVertices)
	{
	...
	for (const TLLRN_SubdividePolyLocal::SubdUVVertex& UVVertex : RefinedUVs)
	{
	...
	if (NormalComputationMethod != ETLLRN_SubdivisionOutputNormals::None)
	{
		const bool bUseExistingMeshVertexNormals = (NormalComputationMethod == ETLLRN_SubdivisionOutputNormals::Interpolated);
		FMeshNormals::InitializeOverlayToPerVertexNormals(OutMesh.Attributes()->PrimaryNormals(), bUseExistingMeshVertexNormals);
	}

	if (bShouldInterpolateUVs)
	{
		TLLRN_SubdividePolyLocal::InitializeOverlayToFaceVertexUVs(OutMesh.Attributes()->PrimaryUV(), UVTriangles, UVElements);
	}
...
FTLLRN_SubdividePoly::ETopologyCheckResult FTLLRN_SubdividePoly::ValidateTopology()
{
	...
	//if (BoundaryScheme == ETLLRN_SubdivisionBoundaryScheme::NoBoundaryFaces)
	//{

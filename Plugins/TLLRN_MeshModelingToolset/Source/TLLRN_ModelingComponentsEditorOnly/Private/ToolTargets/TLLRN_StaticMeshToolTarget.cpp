...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_StaticMeshToolTarget)

using namespace UE::Geometry;

#define LOCTEXT_NAMESPACE "TLLRN_StaticMeshToolTarget"

namespace TLLRN_StaticMeshToolTargetLocals
{
...
void UTLLRN_StaticMeshToolTarget::SetEditingLOD(EMeshLODIdentifier RequestedEditingLOD)
{
	EditingLOD = GetValidEditingLOD(StaticMesh.Get(), RequestedEditingLOD);
}

EMeshLODIdentifier UTLLRN_StaticMeshToolTarget::GetValidEditingLOD(const UStaticMesh* StaticMeshIn, 
	EMeshLODIdentifier RequestedEditingLOD)
{
	using namespace TLLRN_StaticMeshToolTargetLocals;
...
TArray<EMeshLODIdentifier> UTLLRN_StaticMeshToolTarget::GetAvailableLODs(bool bSkipAutoGenerated) const
{
	return GetAvailableLODs(StaticMesh.Get(), bSkipAutoGenerated);
}

TArray<EMeshLODIdentifier> UTLLRN_StaticMeshToolTarget::GetAvailableLODs(const UStaticMesh* InStaticMesh, bool bSkipAutoGenerated)
{
...
bool UTLLRN_StaticMeshToolTarget::IsValid() const
{
...
bool UTLLRN_StaticMeshToolTarget::IsValid(const UStaticMesh* StaticMeshIn, EMeshLODIdentifier EditingLODIn)
{
...
bool UTLLRN_StaticMeshToolTarget::HasNonGeneratedLOD(const UStaticMesh* StaticMeshIn, EMeshLODIdentifier MeshLOD)
{
	if (!StaticMeshIn)
	{
		return false;
...
int32 UTLLRN_StaticMeshToolTarget::GetNumMaterials() const
{
	return ensure(IsValid()) ? StaticMesh->GetStaticMaterials().Num() : 0;
}

UMaterialInterface* UTLLRN_StaticMeshToolTarget::GetMaterial(int32 MaterialIndex) const
{
	return ensure(IsValid()) ? StaticMesh->GetMaterial(MaterialIndex) : nullptr;
}

void UTLLRN_StaticMeshToolTarget::GetMaterialSet(FComponentMaterialSet& MaterialSetOut, bool bPreferAssetMaterials) const
{
...
void UTLLRN_StaticMeshToolTarget::GetMaterialSet(const UStaticMesh* StaticMeshIn, 
...
bool UTLLRN_StaticMeshToolTarget::CommitMaterialSetUpdate(const FComponentMaterialSet& MaterialSet, bool bApplyToAsset)
{
...
bool UTLLRN_StaticMeshToolTarget::CommitMaterialSetUpdate(UStaticMesh* StaticMeshIn,
...
const FMeshDescription* UTLLRN_StaticMeshToolTarget::GetMeshDescription(const FGetMeshParameters& GetMeshParams)
{
	if (ensure(IsValid()))
	{
		...
		{
			UseLOD = UTLLRN_StaticMeshToolTarget::GetValidEditingLOD(StaticMesh.Get(), GetMeshParams.RequestLOD);
...
TArray<int32> UTLLRN_StaticMeshToolTarget::GetPolygonGroupToMaterialIndexMap() const
{
	if (IsValid())
	{
		return UTLLRN_StaticMeshToolTarget::MapSectionToMaterialID(StaticMesh.Get(), EditingLOD);
	}
	return TArray<int32>();
}

TArray<int32> UTLLRN_StaticMeshToolTarget::MapSectionToMaterialID(const UStaticMesh* Mesh, EMeshLODIdentifier EditingLOD)
{
...
const FMeshDescription* UTLLRN_StaticMeshToolTarget::GetMeshDescriptionWithScaleApplied(UStaticMesh* StaticMesh, int32 UseLOD, FMeshDescriptionCache& CachedMeshDescriptions)
{
...
FMeshDescription UTLLRN_StaticMeshToolTarget::GetEmptyMeshDescription()
{
...
void UTLLRN_StaticMeshToolTarget::CommitMeshDescription(const FCommitter& Committer, const FCommitMeshParameters& CommitParams)
{
...
void UTLLRN_StaticMeshToolTarget::CommitMeshDescription(UStaticMesh* StaticMeshIn, const FCommitter& Committer, EMeshLODIdentifier EditingLODIn)
{
	using namespace TLLRN_StaticMeshToolTargetLocals;

	if ( ! ensure(EditingLODIn != EMeshLODIdentifier::Default && EditingLODIn != EMeshLODIdentifier::MaxQuality) )
	{
		UE_LOG(LogGeometry, Warning, TEXT("UTLLRN_StaticMeshToolTarget::CommitMeshDescription: invalid Target LOD, must specify explicit LOD"));
		return;
	}
	if (StaticMeshIn->GetPathName().StartsWith(TEXT("/Engine/")))
	{
...
FDynamicMesh3 UTLLRN_StaticMeshToolTarget::GetDynamicMesh()
{
	return GetDynamicMeshViaMeshDescription(*this);
}

FDynamicMesh3 UTLLRN_StaticMeshToolTarget::GetDynamicMesh(const FGetMeshParameters& InGetMeshParams)
{
	return GetDynamicMeshViaMeshDescription(*this, InGetMeshParams);
}

void UTLLRN_StaticMeshToolTarget::CommitDynamicMesh(const FDynamicMesh3& Mesh, const FDynamicMeshCommitInfo& CommitInfo)
{
...
UStaticMesh* UTLLRN_StaticMeshToolTarget::GetStaticMesh() const
{
...
bool UTLLRN_StaticMeshToolTargetFactory::CanBuildTarget(UObject* SourceObject, const FToolTargetTypeRequirements& Requirements) const
{
	...
	return StaticMesh && !StaticMesh->IsUnreachable() && StaticMesh->IsValidLowLevel()
		...
		&& Requirements.AreSatisfiedBy(UTLLRN_StaticMeshToolTarget::StaticClass());
}

UToolTarget* UTLLRN_StaticMeshToolTargetFactory::BuildTarget(UObject* SourceObject, const FToolTargetTypeRequirements& Requirements)
{
	UTLLRN_StaticMeshToolTarget* Target = NewObject<UTLLRN_StaticMeshToolTarget>();// TODO: Should we set an outer here?
...
void UTLLRN_StaticMeshToolTargetFactory::SetActiveEditingLOD(EMeshLODIdentifier NewEditingLOD)
{

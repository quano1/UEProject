...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_PolygonSelectionMechanic)

using namespace UE::Geometry;

#define LOCTEXT_NAMESPACE "UTLLRN_PolygonSelectionMechanic"

namespace TLLRN_PolygonSelectionMechanicLocals
{
	using namespace UE::Geometry;
...
void UTLLRN_PolygonSelectionMechanic::Initialize(
...
void UTLLRN_PolygonSelectionMechanic::GetSelection_AsGroupTopology(UE::Geometry::FGeometrySelection& SelectionOut, const FCompactMaps* CompactMapsToApply) const
{
...
void UTLLRN_PolygonSelectionMechanic::GetSelection_AsTriangleTopology(UE::Geometry::FGeometrySelection& SelectionOut, const FCompactMaps* CompactMapsToApply) const
{
...
void UTLLRN_PolygonSelectionMechanic::SetSelection_AsGroupTopology(const UE::Geometry::FGeometrySelection& Selection)
{
...
void UTLLRN_PolygonSelectionMechanic::SetSelection_AsTriangleTopology(const UE::Geometry::FGeometrySelection& Selection)
{
...
bool UTLLRN_PolygonSelectionMechanic::ExecuteActionThroughGeometrySelection(bool bAsTriangleTopology, const FText& TransactionName, 
	TFunctionRef<bool(UE::Geometry::FGeometrySelection& SelectionToModifyInPlace)> SelectionProcessor)
{
	using namespace TLLRN_PolygonSelectionMechanicLocals;
	using namespace UE::Geometry;
...
void UTLLRN_PolygonSelectionMechanic::GrowSelection(bool bAsTriangleTopology)
{
	using namespace UE::Geometry;
...
void UTLLRN_PolygonSelectionMechanic::ShrinkSelection(bool bAsTriangleTopology)
{
	using namespace UE::Geometry;
...
void UTLLRN_PolygonSelectionMechanic::ConvertSelectionToBorderVertices(bool bAsTriangleTopology)
{
	using namespace UE::Geometry;
...
void UTLLRN_PolygonSelectionMechanic::FloodSelection()
{
	using namespace UE::Geometry;
...
bool UTLLRN_PolygonSelectionMechanic::UpdateHighlight(const FRay& WorldRay)
{
		...
		for (int Gid : HilightSelection.SelectedGroupIDs)
		{
			if (!CurrentlyHighlightedGroups.Contains(Gid))
			{
				for (int32 Tid : Topology->GetGroupTriangles(Gid))
				{
					...
					DrawnTLLRN_TriangleSetComponent->AddTriangle(FTLLRN_RenderableTriangle(HighlightedFaceMaterial,
						FTLLRN_RenderableTriangleVertex((FVector)Mesh->GetVertex(VertIndices.A), (FVector2D)Mesh->GetVertexUV(VertIndices.A), (FVector)TriangleNormal, FLinearColor(Mesh->GetVertexColor(VertIndices.A)).ToFColor(true)),
						FTLLRN_RenderableTriangleVertex((FVector)Mesh->GetVertex(VertIndices.B), (FVector2D)Mesh->GetVertexUV(VertIndices.B), (FVector)TriangleNormal, FLinearColor(Mesh->GetVertexColor(VertIndices.B)).ToFColor(true)),
						FTLLRN_RenderableTriangleVertex((FVector)Mesh->GetVertex(VertIndices.C), (FVector2D)Mesh->GetVertexUV(VertIndices.C), (FVector)TriangleNormal, FLinearColor(Mesh->GetVertexColor(VertIndices.C)).ToFColor(true))));
				}
...
bool UTLLRN_PolygonSelectionMechanic::UpdateSelection(const FRay& WorldRay, FVector3d& LocalHitPositionOut, FVector3d& LocalHitNormalOut)
{
	FRay3d LocalRay(TargetTransform.InverseTransformPosition((FVector3d)WorldRay.Origin),
		TargetTransform.InverseTransformVector((FVector3d)WorldRay.Direction));
	UE::Geometry::Normalize(LocalRay.Direction);

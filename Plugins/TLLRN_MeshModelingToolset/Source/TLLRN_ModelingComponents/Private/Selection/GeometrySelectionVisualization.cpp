	...
	const FTransform* ApplyTransform)
{
	...
	if (Settings->SelectionTopologyType == ETLLRN_GeometrySelectionTopologyType::Polygroup)
	{
		...
		if (Settings->SelectionElementType == ETLLRN_GeometrySelectionElementType::Vertex)
		{
		...
		else if (Settings->SelectionElementType == ETLLRN_GeometrySelectionElementType::Edge)
		{
		...
		else if (ensure(Settings->SelectionElementType == ETLLRN_GeometrySelectionElementType::Face))
		{


			PreviewGeom->CreateOrUpdateTriangleSet(TEXT("FaceSelection"), 1, [&](int32, TArray<FTLLRN_RenderableTriangle>& Triangles)
			{
				auto AddTriangle = [&](int32 Tid, const FTriangle3d& Triangle)
				{
					FVector3d Normal = Triangle.Normal();
					FTLLRN_RenderableTriangleVertex A(Triangle.V[0], FVector2D(0,0), Normal, Settings->FaceColor);
					FTLLRN_RenderableTriangleVertex B(Triangle.V[1], FVector2D(1,0), Normal, Settings->FaceColor);
					FTLLRN_RenderableTriangleVertex C(Triangle.V[2], FVector2D(1,1), Normal, Settings->FaceColor);
					Triangles.Add(FTLLRN_RenderableTriangle(Settings->GetFaceMaterial(), A, B, C));
				};
	...
	else if (ensure(Settings->SelectionTopologyType == ETLLRN_GeometrySelectionTopologyType::Triangle)) 
	{
	...
	UTLLRN_GeometrySelectionVisualizationProperties* Settings)
{
	if (Settings->bVisualizationDirty)
	{
		...
		if (Settings->SelectionElementType == ETLLRN_GeometrySelectionElementType::Vertex)
		{
			PreviewGeom->UpdatePointSet(TEXT("VertexSelection"), [Settings](TObjectPtr<UTLLRN_PointSetComponent> PointSet)
			{
		...
		if (Settings->SelectionElementType == ETLLRN_GeometrySelectionElementType::Edge)
		{
			PreviewGeom->UpdateLineSet(TEXT("EdgeSelection"), [Settings](TObjectPtr<UTLLRN_LineSetComponent> LineSet)
			{
		...
		if (Settings->SelectionElementType == ETLLRN_GeometrySelectionElementType::Face)
		{
			PreviewGeom->UpdateTriangleSet(TEXT("FaceSelection"), [Settings](TObjectPtr<UTLLRN_TriangleSetComponent> TriangleSet)
			{
		...
		{
			PreviewGeom->UpdatePointSet(TEXT("EdgeSelectionVertices"), [Settings](TObjectPtr<UTLLRN_PointSetComponent> PointSet)
			{
		...
		{
			PreviewGeom->UpdateLineSet(TEXT("TriangleROIBorder"), [Settings](TObjectPtr<UTLLRN_LineSetComponent> LineSet)
			{

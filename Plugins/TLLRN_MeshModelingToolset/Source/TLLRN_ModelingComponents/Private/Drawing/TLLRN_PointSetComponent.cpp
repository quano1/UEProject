...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_PointSetComponent)
...
public:
	...
	FPointSetSceneProxy(UTLLRN_PointSetComponent* Component)
...
UTLLRN_PointSetComponent::UTLLRN_PointSetComponent()
{
	CastShadow = false;
	bSelectable = false;
...
void UTLLRN_PointSetComponent::SetPointMaterial(UMaterialInterface* InPointMaterial)
{
...
void UTLLRN_PointSetComponent::Clear()
{
	Points.Reset();
...
void UTLLRN_PointSetComponent::ReservePoints(const int32 MaxID)
{
...
int32 UTLLRN_PointSetComponent::AddPoint(const FRenderablePoint& OverlayPoint)
{
	MarkRenderStateDirty();
	return AddPointInternal(OverlayPoint);
}

int32 UTLLRN_PointSetComponent::AddPointInternal(const FRenderablePoint& Point)
{
...
void UTLLRN_PointSetComponent::InsertPoint(const int32 ID, const FRenderablePoint& OverlayPoint)
{
...
const FRenderablePoint& UTLLRN_PointSetComponent::GetPoint(const int32 ID)
{
	return Points[ID];
}


void UTLLRN_PointSetComponent::SetPointColor(const int32 ID, const FColor& NewColor)
{
...
void UTLLRN_PointSetComponent::SetPointSize(const int32 ID, const float NewSize)
{
	FRenderablePoint& OverlayPoint = Points[ID];
...
void UTLLRN_PointSetComponent::SetPointPosition(const int32 ID, const FVector& NewPosition)
{
	FRenderablePoint& OverlayPoint = Points[ID];
...
void UTLLRN_PointSetComponent::SetAllPointsColor(const FColor& NewColor)
{
...
void UTLLRN_PointSetComponent::SetAllPointsSize(float NewSize)
{
	for (FRenderablePoint& Point : Points)
	{
		Point.Size = NewSize;
...
void UTLLRN_PointSetComponent::SetAllPointsDepthBias(float NewDepthBias)
{
	for (FRenderablePoint& Point : Points)
	{
...
void UTLLRN_PointSetComponent::RemovePoint(const int32 ID)
{
	Points.RemoveAt(ID);
	MarkRenderStateDirty();
	bBoundsDirty = true;
...
bool UTLLRN_PointSetComponent::IsPointValid(const int32 ID) const
{
	return Points.IsValidIndex(ID);
}


FPrimitiveSceneProxy* UTLLRN_PointSetComponent::CreateSceneProxy()
{
	if (Points.Num() > 0)
	{
...
int32 UTLLRN_PointSetComponent::GetNumMaterials() const
{
	return 1;
}


FBoxSphereBounds UTLLRN_PointSetComponent::CalcBounds(const FTransform& LocalToWorld) const
{
	if (bBoundsDirty)
	{
		FBox Box(ForceInit);
...
void UTLLRN_PointSetComponent::AddPoints(
	int32 NumIndices,

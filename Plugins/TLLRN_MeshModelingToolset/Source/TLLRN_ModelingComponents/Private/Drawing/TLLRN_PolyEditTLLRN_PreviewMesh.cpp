...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_PolyEditTLLRN_PreviewMesh)

using namespace UE::Geometry;

void UTLLRN_PolyEditTLLRN_PreviewMesh::InitializeExtrudeType(
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::InitializeExtrudeType(FDynamicMesh3&& BaseMesh,
	const FVector3d& TransformedOffsetDirection,
	const FTransform3d* MeshTransformIn,
	bool bDeleteExtrudeBaseFaces)
{
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::UpdateExtrudeType(double NewOffset, bool bUseNormalDirection)
{
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::UpdateExtrudeType_FaceNormalAvg(double NewOffset)
{
	EditMesh([&](FDynamicMesh3& Mesh)
	{
		int32 NumVertices = EditVertices.Num();
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::UpdateExtrudeType(TFunctionRef<void(FDynamicMesh3&)> UpdateMeshFunc, bool bFullRecalculate)
{
	if (bFullRecalculate)
	{
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::MakeExtrudeTypeHitTargetMesh(FDynamicMesh3& TargetMesh, bool bUseNormalDirection)
{
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::InitializeInsetType(const FDynamicMesh3* SourceMesh, const TArray<int32>& Triangles,
	const FTransform3d* MeshTransformIn)
{
	// extract submesh
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::UpdateInsetType(double NewOffset, bool bReproject, double Softness, double AreaScaleT, bool bBoundaryOnly)
{
	FDynamicMesh3 EditPatch(InitialEditPatch);
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::MakeInsetTypeTargetMesh(FDynamicMesh3& TargetMesh)
{
	TargetMesh = InitialEditPatch;
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::InitializeStaticType(const FDynamicMesh3* SourceMesh, const TArray<int32>& Triangles,
	const FTransform3d* MeshTransformIn)
{
	// extract submesh
	ActiveSubmesh = MakeUnique<FDynamicSubmesh3>(SourceMesh, Triangles, (int32)EMeshComponents::FaceGroups, true);
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::UpdateStaticType(TFunctionRef<void(FDynamicMesh3&)> UpdateMeshFunc, bool bFullRecalculate)
{
	if (bFullRecalculate)
	{
		FDynamicMesh3 TempMesh(InitialEditPatch);
...
void UTLLRN_PolyEditTLLRN_PreviewMesh::MakeStaticTypeTargetMesh(FDynamicMesh3& TargetMesh)
{
	TargetMesh = InitialEditPatch;
}

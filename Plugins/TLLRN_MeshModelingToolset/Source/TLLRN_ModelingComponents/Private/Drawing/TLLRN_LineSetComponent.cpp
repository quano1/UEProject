...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_LineSetComponent)
...
/** Class for the TLLRN_LineSetComponent data passed to the render thread. */
class FLineSetSceneProxy final : public FPrimitiveSceneProxy
{
public:

	FLineSetSceneProxy(UTLLRN_LineSetComponent* Component)
...
UTLLRN_LineSetComponent::UTLLRN_LineSetComponent()
{
	CastShadow = false;
	bSelectable = false;
...
void UTLLRN_LineSetComponent::SetLineMaterial(UMaterialInterface* InLineMaterial)
{
	LineMaterial = InLineMaterial;
	SetMaterial(0, InLineMaterial);
}

void UTLLRN_LineSetComponent::Clear()
{
	Lines.Reset();
...
void UTLLRN_LineSetComponent::ReserveLines(const int32 MaxID)
{
	Lines.Reserve(MaxID);
}

int32 UTLLRN_LineSetComponent::AddLine(const FRenderableLine& OverlayLine)
{
	MarkRenderStateDirty();
	return AddLineInternal(OverlayLine);
}

int32 UTLLRN_LineSetComponent::AddLineInternal(const FRenderableLine& Line)
{
...
int32 UTLLRN_LineSetComponent::AddLines(const TArray<FVector>& InStart, const TArray<FVector>& InEnd, const FColor& InColor, const float InThickness, const float InDepthBias)
{
...
void UTLLRN_LineSetComponent::InsertLine(const int32 ID, const FRenderableLine& OverlayLine)
{
...
void UTLLRN_LineSetComponent::SetLineStart(const int32 ID, const FVector& NewPostion)
{
...
void UTLLRN_LineSetComponent::SetLineEnd(const int32 ID, const FVector& NewPostion)
{
	FRenderableLine& OverlayLine = Lines[ID];
...
void UTLLRN_LineSetComponent::SetLineColor(const int32 ID, const FColor& NewColor)
{
	FRenderableLine& OverlayLine = Lines[ID];
...
void UTLLRN_LineSetComponent::SetLineThickness(const int32 ID, const float NewThickness)
{
	FRenderableLine& OverlayLine = Lines[ID];
...
void UTLLRN_LineSetComponent::SetAllLinesThickness(const float NewThickness)
{
...
void UTLLRN_LineSetComponent::SetAllLinesDepthBias(const float NewDepthBias)
{
	for (FRenderableLine& Line : Lines)
	{
...
void UTLLRN_LineSetComponent::SetAllLinesLength(const float NewLength, bool bUpdateBounds)
{
...
void UTLLRN_LineSetComponent::SetAllLinesColor(const FColor& NewColor)
{
	for (FRenderableLine& Line : Lines)
	{
		Line.Color = NewColor;
...
void UTLLRN_LineSetComponent::RemoveLine(const int32 ID)
{
	Lines.RemoveAt(ID);
	MarkRenderStateDirty();
	bBoundsDirty = true;
...
bool UTLLRN_LineSetComponent::IsLineValid(const int32 ID) const
{
	return Lines.IsValidIndex(ID);
}

FPrimitiveSceneProxy* UTLLRN_LineSetComponent::CreateSceneProxy()
{
	if (Lines.Num() > 0)
	{
...
int32 UTLLRN_LineSetComponent::GetNumMaterials() const
{
	return 1;
}

FBoxSphereBounds UTLLRN_LineSetComponent::CalcBounds(const FTransform& LocalToWorld) const
{
	if (bBoundsDirty)
	{
		FBox Box(ForceInit);
...
void UTLLRN_LineSetComponent::AddLines(
	int32 NumIndices,

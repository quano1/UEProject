...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_RectangleMarqueeMechanic)

using namespace UE::Geometry;

#define LOCTEXT_NAMESPACE "UTLLRN_RectangleMarqueeMechanic"
...
FInputRayHit UTLLRN_RectangleMarqueeInteraction::CanBeginClickDragSequence(const FInputDeviceRay& PressPos)
{
	return FInputRayHit(TNumericLimits<float>::Max());
}

void UTLLRN_RectangleMarqueeInteraction::OnClickPress(const FInputDeviceRay& PressPos)
{
	if (!PressPos.bHas2D)
	{
		bIsDragging = false;
...
void UTLLRN_RectangleMarqueeInteraction::OnClickDrag(const FInputDeviceRay& DragPos)
{
	if (!DragPos.bHas2D)
	{
		return;
...
void UTLLRN_RectangleMarqueeInteraction::OnClickRelease(const FInputDeviceRay& ReleasePos)
{
	bIsDragging = false;
	OnDragRectangleFinished.Broadcast(CameraRectangle, false);
}

void UTLLRN_RectangleMarqueeInteraction::OnTerminateDragSequence()
{
	bIsDragging = false;
...
void UTLLRN_RectangleMarqueeInteraction::DrawHUD(FCanvas* Canvas, IToolsContextRenderAPI* RenderAPI)
{
...
void UTLLRN_RectangleMarqueeMechanic::Setup(UInteractiveTool* ParentToolIn)
{
...
bool UTLLRN_RectangleMarqueeMechanic::IsEnabled()
{
	return bIsEnabled;
}

void UTLLRN_RectangleMarqueeMechanic::SetIsEnabled(bool bOn)
{
	if (bIsDragging && !bOn)
	{
...
void UTLLRN_RectangleMarqueeMechanic::SetBasePriority(const FInputCapturePriority& Priority)
{
...
TPair<FInputCapturePriority, FInputCapturePriority> UTLLRN_RectangleMarqueeMechanic::GetPriorityRange() const
{
	return TPair<FInputCapturePriority, FInputCapturePriority>(BasePriority, BasePriority);
}

void UTLLRN_RectangleMarqueeMechanic::Render(IToolsContextRenderAPI* RenderAPI)
{
...
FInputRayHit UTLLRN_RectangleMarqueeMechanic::CanBeginClickDragSequence(const FInputDeviceRay& PressPos)
{
	return bIsEnabled ? 
...
void UTLLRN_RectangleMarqueeMechanic::OnClickPress(const FInputDeviceRay& PressPos)
{
	if (!PressPos.bHas2D)
	{
		bIsDragging = false;
...
void UTLLRN_RectangleMarqueeMechanic::OnClickDrag(const FInputDeviceRay& DragPos)
{
	if (!DragPos.bHas2D)
	{
		return;
...
void UTLLRN_RectangleMarqueeMechanic::OnClickRelease(const FInputDeviceRay& ReleasePos)
{
	if (bIsOnDragRectangleChangedDeferred)
	{
		bIsOnDragRectangleChangedDeferred = false;
...
void UTLLRN_RectangleMarqueeMechanic::OnTerminateDragSequence()
{
	bIsDragging = false;
	OnDragRectangleFinished.Broadcast(CameraRectangle, true);
}
...
void UTLLRN_RectangleMarqueeMechanic::DrawHUD(FCanvas* Canvas, IToolsContextRenderAPI* RenderAPI)
{
	EViewInteractionState State = RenderAPI->GetViewInteractionState();
	bool bThisViewHasFocus = !!(State & EViewInteractionState::Focused);
	if (bThisViewHasFocus && bIsDragging)

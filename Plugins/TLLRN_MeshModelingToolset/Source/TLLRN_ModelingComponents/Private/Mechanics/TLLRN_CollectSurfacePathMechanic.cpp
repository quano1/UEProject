...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_CollectSurfacePathMechanic)

using namespace UE::Geometry;

UTLLRN_CollectSurfacePathMechanic::UTLLRN_CollectSurfacePathMechanic()
{
...
void UTLLRN_CollectSurfacePathMechanic::Setup(UInteractiveTool* ParentToolIn)
{
	UInteractionMechanic::Setup(ParentToolIn);
}

void UTLLRN_CollectSurfacePathMechanic::Shutdown()
{
...
void UTLLRN_CollectSurfacePathMechanic::Render(IToolsContextRenderAPI* RenderAPI)
{
	if (bDrawPath == true)
	{
...
void UTLLRN_CollectSurfacePathMechanic::InitializeMeshSurface(FDynamicMesh3&& TargetSurfaceMesh)
{
	TargetSurface = MoveTemp(TargetSurfaceMesh);
	TargetSurfaceAABB.SetMesh(&TargetSurface);
}

void UTLLRN_CollectSurfacePathMechanic::InitializePlaneSurface(const FFrame3d& TargetPlaneIn)
{
...
void UTLLRN_CollectSurfacePathMechanic::SetFixedNumPointsMode(int32 NumPoints)
{
...
void UTLLRN_CollectSurfacePathMechanic::SetCloseWithLambdaMode()
{
	check(IsDoneFunc);
	DoneMode = ECollectSurfacePathDoneMode::ExternalLambda;
}

void UTLLRN_CollectSurfacePathMechanic::SetDrawClosedLoopMode()
{
	DoneMode = ECollectSurfacePathDoneMode::SnapCloseLoop;
}

void UTLLRN_CollectSurfacePathMechanic::SetDoubleClickOrCloseLoopMode()
{
	DoneMode = ECollectSurfacePathDoneMode::SnapDoubleClickOrCloseLoop;
}


bool UTLLRN_CollectSurfacePathMechanic::IsHitByRay(const FRay3d& Ray, FFrame3d& HitPoint)
{
	return RayToPathPoint(Ray, HitPoint, false);
}


bool UTLLRN_CollectSurfacePathMechanic::UpdatePreviewPoint(const FRay3d& Ray)
{
...
bool UTLLRN_CollectSurfacePathMechanic::TryAddPointFromRay(const FRay3d& Ray)
{
	FFrame3d NewPoint;
...
bool UTLLRN_CollectSurfacePathMechanic::PopLastPoint()
{
	if (bGeometricCloseOccurred)
	{
...
bool UTLLRN_CollectSurfacePathMechanic::RayToPathPoint(const FRay3d& Ray, FFrame3d& PointOut, bool bEnableSnapping)
{
	bool bHaveHit = false;
...
bool UTLLRN_CollectSurfacePathMechanic::IsDone() const
{
...
bool UTLLRN_CollectSurfacePathMechanic::CheckGeometricClosure(const FFrame3d& Point, bool* bLoopWasClosedOut)
{

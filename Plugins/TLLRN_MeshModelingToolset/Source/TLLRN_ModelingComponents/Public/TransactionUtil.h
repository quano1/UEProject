...
namespace UE::TransactionUtil
{
	...
	public:
		...
		TLLRN_MODELINGCOMPONENTS_API void Open(FText TransactionName, UInteractiveToolManager* ToolManager);

		// Close a single long transaction. Expect that at least one is open.
		TLLRN_MODELINGCOMPONENTS_API void Close(UInteractiveToolManager* ToolManager);
		...
		TLLRN_MODELINGCOMPONENTS_API void CloseAll(UInteractiveToolManager* ToolManager);

...
#include "TLLRN_TriangleSetComponent.h"
#include "InteractiveTool.h"
#include "TargetInterfaces/MaterialProvider.h"
#include "TLLRN_UVLayoutPreview.generated.h"

PREDECLARE_USE_GEOMETRY_CLASS(FDynamicMesh3);
class UTLLRN_MeshElementsVisualizer;

/**
 * Where should in-viewport TLLRN_UVLayoutPreview be shown, relative to target object
 */
UENUM()
enum class ETLLRN_UVLayoutPreviewSide
{
 ...
 */
UCLASS()
class TLLRN_MODELINGCOMPONENTS_API UTLLRN_UVLayoutPreviewProperties : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
public:
	...
	ETLLRN_UVLayoutPreviewSide Side = ETLLRN_UVLayoutPreviewSide::Right;
...
/**
 * UTLLRN_UVLayoutPreview is a utility object that creates and manages a 3D plane on which a UV layout
 * for a 3D mesh is rendered. The UV layout
 */
UCLASS(Transient)
class TLLRN_MODELINGCOMPONENTS_API UTLLRN_UVLayoutPreview : public UObject
{
	GENERATED_BODY()

public:
	virtual ~UTLLRN_UVLayoutPreview() override;
	...
	UPROPERTY()
	TObjectPtr<UTLLRN_UVLayoutPreviewProperties> Settings;

	/** TLLRN_PreviewMesh is initialized with a copy of an input mesh with UVs mapped to position, ie such that (X,Y,Z) = (U,V,0) */
	UPROPERTY()
	TObjectPtr<UTLLRN_PreviewMesh> TLLRN_PreviewMesh;

	/** TLLRN_MeshElementsVisualizer references the TLLRN_PreviewMesh to handle drawing the wireframes for UVs */
	UPROPERTY()
	TObjectPtr<UTLLRN_MeshElementsVisualizer> TLLRN_MeshElementsVisualizer;

	/** Set of additional triangles to draw, eg for backing rectangle, etc */
	UPROPERTY()
	TObjectPtr<UTLLRN_TriangleSetComponent> TriangleComponent;

...
namespace Geometry
{
	 ...
	 */
	TLLRN_MODELINGCOMPONENTS_API bool HaveAvailableGeometrySelection(const FToolBuilderState& SceneState);

	/**
	 * Given a FToolBuilderState, find an available FGeometrySelection for the given ToolTarget (via the UTLLRN_GeometrySelectionManager in the ContextObjectStore)
	 * @param SelectionOut non-empty selection will be returned here
	 * @return true if a non-empty selection was found
	 */
	TLLRN_MODELINGCOMPONENTS_API bool GetCurrentGeometrySelectionForTarget(const FToolBuilderState& SceneState, UToolTarget* Target,
		FGeometrySelection& SelectionOut);

	/**
	 * Find an available FGeometrySelection for the given ToolTarget (via the UTLLRN_GeometrySelectionManager in the ContextObjectStore)
	 * @param SelectionOut non-empty selection will be returned here
	 * @return true if a non-empty selection was found
	 */
	TLLRN_MODELINGCOMPONENTS_API bool GetCurrentGeometrySelectionForTarget(UInteractiveToolManager* ToolManager, UToolTarget* Target,
		FGeometrySelection& SelectionOut);
	 ...
	 */
	TLLRN_MODELINGCOMPONENTS_API bool GetCurrentSelectionWorldFrameBounds(const FToolBuilderState& SceneState, FFrame3d& OutWorldFrame, FAxisAlignedBox3d& OutWorldBounds, bool& bOutIsElementSelection);

	/**
	 * Allow a Tool to return an "output" FGeometrySelection for the given ToolTarget (presumably on Tool Shutdown)
	 * Target must be an "active" selection target in the current UTLLRN_GeometrySelectionManager.
	 ...
	 */
	TLLRN_MODELINGCOMPONENTS_API bool SetToolOutputGeometrySelectionForTarget(UInteractiveTool* Tool, UToolTarget* Target, const FGeometrySelection& OutputSelection);
	 ...
	 */
	class TLLRN_MODELINGCOMPONENTS_API FMeshEdgesFromTriangleSubIndices
	{
	public:
	 ...
	 */
	TLLRN_MODELINGCOMPONENTS_API FIndex2i GetGroupEdgeRepresentativeVerts(const FGroupTopology& TopologyIn, int GroupEdgeID, const FCompactMaps& CompactMaps);
	 ...
	 */
	TLLRN_MODELINGCOMPONENTS_API FIndex2i GetGroupEdgeRepresentativeVerts(const FGroupTopology& TopologyIn, int GroupEdgeID);

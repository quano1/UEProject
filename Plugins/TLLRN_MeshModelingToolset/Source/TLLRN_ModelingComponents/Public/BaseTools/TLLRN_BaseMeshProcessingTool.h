...
/**
 * ToolBuilder for UTLLRN_BaseMeshProcessingTool
 */
UCLASS()
class TLLRN_MODELINGCOMPONENTS_API UTLLRN_BaseMeshProcessingToolBuilder : public UTLLRN_SingleTargetWithSelectionToolBuilder
{
	GENERATED_BODY()
...
public:
	...
	virtual UTLLRN_BaseMeshProcessingTool* MakeNewToolInstance(UObject* Outer) const { check(false); return nullptr; }
...
/**
 * UTLLRN_BaseMeshProcessingTool is a base Tool (ie has no functionality of it's own and must be subclassed) 
 ...
 *   - GetUTLLRN_PreviewMesh() : return the UTLLRN_PreviewMesh inside the background compute (for configuration/etc - should not directly touch the mesh!)
 ...
 */
UCLASS()
class TLLRN_MODELINGCOMPONENTS_API UTLLRN_BaseMeshProcessingTool : public UTLLRN_SingleTargetWithSelectionTool, public UE::Geometry::IDynamicMeshOperatorFactory
{
	GENERATED_BODY()
protected:
	using FFrame3d = UE::Geometry::FFrame3d;
public:
	UTLLRN_BaseMeshProcessingTool() = default;
...
protected:
	...
	//
	// UTLLRN_BaseMeshProcessingTool REQUIRED API - subclasses must implement these functions
	//
	...
	//
	// UTLLRN_BaseMeshProcessingTool OPTIONAL API - subclasses may implement these functions
	//
	...
	UTLLRN_PreviewMesh* GetUTLLRN_PreviewMesh() const { return Preview->TLLRN_PreviewMesh; }
...
protected:
	...
	virtual void SetupWeightMapPropertySet(UTLLRN_WeightMapSetProperties* Properties);

	bool HasActiveWeightMap() const;
	TSharedPtr<FIndexedWeightMap1f>& GetActiveWeightMap();

private:
	TWeakObjectPtr<UTLLRN_WeightMapSetProperties> WeightMapPropertySet;

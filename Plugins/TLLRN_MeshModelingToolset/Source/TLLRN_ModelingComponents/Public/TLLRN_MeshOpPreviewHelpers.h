...
#include "TLLRN_MeshOpPreviewHelpers.generated.h"
 ...
 */
struct TLLRN_MODELINGCOMPONENTS_API FDynamicMeshOpResult
{
...
/**
 * UTLLRN_MeshOpPreviewWithBackgroundCompute is an infrastructure object that implements a common UI
 ...
 */
UCLASS(Transient)
class TLLRN_MODELINGCOMPONENTS_API UTLLRN_MeshOpPreviewWithBackgroundCompute : public UObject
{
	GENERATED_BODY()
public:
	...
	*/
	...
	/**
	 * @return true if the current TLLRN_PreviewMesh result is valid, ie no update being actively computed
	 */
	...
	/**
	 * @return true if current TLLRN_PreviewMesh result is valid (no update actively being computed) and that mesh has at least one triangle
	 */
	bool HaveValidNonEmptyResult() const { return bResultValid && TLLRN_PreviewMesh && TLLRN_PreviewMesh->GetMesh() && TLLRN_PreviewMesh->GetMesh()->TriangleCount() > 0; }

	/**
	 * @return true if current TLLRN_PreviewMesh result is valid (no update actively being computed) but that mesh has no triangles
	 */
	bool HaveEmptyResult() const { return bResultValid && TLLRN_PreviewMesh && TLLRN_PreviewMesh->GetMesh() && TLLRN_PreviewMesh->GetMesh()->TriangleCount() == 0; }
	...
	/**
	 * Allow an external function to safely access the TLLRN_PreviewMesh's mesh 
	...
	/** @return UWorld that the created TLLRN_PreviewMesh exist in */
	 ...
	 * @param SecondaryMaterial optional secondary material passed to mesh component, call TLLRN_PreviewMesh->EnableSecondaryTriangleBuffers to enable
	 */
	...
	// Change notification
	//
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnMeshUpdated, UTLLRN_MeshOpPreviewWithBackgroundCompute*);
...
public:
	...
	UPROPERTY()
	TObjectPtr<UTLLRN_PreviewMesh> TLLRN_PreviewMesh = nullptr;

	// input set of materials to assign to TLLRN_PreviewMesh
	UPROPERTY()
	TArray<TObjectPtr<UMaterialInterface>> StandardMaterials;

	// override material to forward to TLLRN_PreviewMesh if set
	UPROPERTY()
	...
	// secondary render material to forward to TLLRN_PreviewMesh if set
	UPROPERTY()
...
protected:
	...
	// update the TLLRN_PreviewMesh if a new result is available from BackgroundCompute
	void UpdateResults();

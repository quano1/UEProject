...
#include "CuttingOps/TLLRN_EmbedPolygonsOp.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_EmbedPolygonsOp)
...
TArray<FPolygon2d> FTLLRN_EmbedPolygonsOp::CleanPolygon(const FPolygon2d& Input)
{
...
void FTLLRN_EmbedPolygonsOp::BooleanPath(FProgressCancel* Progress)
{
	...
	switch (Operation)
	{
	case ETLLRN_EmbeddedPolygonOpMethod::InsertPolygon:
		BoolOp = FMeshBoolean::EBooleanOp::NewGroupInside;
		break;
	case ETLLRN_EmbeddedPolygonOpMethod::TrimInside:
		BoolOp = FMeshBoolean::EBooleanOp::TrimInside;
		break;
	case ETLLRN_EmbeddedPolygonOpMethod::TrimOutside:
		BoolOp = FMeshBoolean::EBooleanOp::TrimOutside;
		break;
	case ETLLRN_EmbeddedPolygonOpMethod::CutThrough:
		BoolOp = FMeshBoolean::EBooleanOp::Difference;
		break;
	case ETLLRN_EmbeddedPolygonOpMethod::CutOutside:
	...
	// extent, in TLLRN_BooleanMeshesOp.cpp).
	...
	if (Operation == ETLLRN_EmbeddedPolygonOpMethod::TrimInside || Operation == ETLLRN_EmbeddedPolygonOpMethod::TrimOutside)
	{
	...
	else if (Boolean.CreatedBoundaryEdges.Num() > 0 && bAttemptFixHolesOnBoolean)
	{
		...
		// TLLRN_BooleanMeshesOp.h, and we do similar things (with more options) for cap filling in revolves.
	...
	if (Operation == ETLLRN_EmbeddedPolygonOpMethod::CutOutside || Operation == ETLLRN_EmbeddedPolygonOpMethod::CutThrough || Operation == ETLLRN_EmbeddedPolygonOpMethod::InsertPolygon)
	{
...
void FTLLRN_EmbedPolygonsOp::CalculateResult(FProgressCancel* Progress)
{
	...
	int SecondHit = 1;
	if (Operation == ETLLRN_EmbeddedPolygonOpMethod::CutThrough || Operation == ETLLRN_EmbeddedPolygonOpMethod::CutOutside)
	{
	...
	if (Operation == ETLLRN_EmbeddedPolygonOpMethod::InsertPolygon)
	{
		DeleteMethod = EDeleteMethod::DeleteNone;
	}
	else if (Operation == ETLLRN_EmbeddedPolygonOpMethod::TrimOutside || Operation == ETLLRN_EmbeddedPolygonOpMethod::CutOutside)
	{
		DeleteMethod = EDeleteMethod::DeleteOutside;
	}

	bool bIsCutOperation = Operation == ETLLRN_EmbeddedPolygonOpMethod::CutThrough || Operation == ETLLRN_EmbeddedPolygonOpMethod::CutOutside;
	...
	else // bIsCutOperation == true
	{
		...
		bool bReverseOrientation = Operation == ETLLRN_EmbeddedPolygonOpMethod::CutOutside;
	...
	//else if (Operation == ETLLRN_EmbeddedPolygonOpMethod::CutAndFill)
	//{
		//FDynamicMeshEditor MeshEditor(ResultMesh.Get());
...
void FTLLRN_EmbedPolygonsOp::RecordEmbeddedEdges(TArray<int>& PathVertIDs)
{
	for (int LastIdx = PathVertIDs.Num() - 1, Idx = 0; Idx < PathVertIDs.Num(); LastIdx = Idx++)
	{

...
#include "CleaningOps/TLLRN_SimplifyMeshOp.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_SimplifyMeshOp)
...
void ComputeSimplify(FDynamicMesh3* TargetMesh, const bool bReproject,
					 ...
					 const ETLLRN_SimplifyTargetType TargetMode,
					 ...
					 float GeometricTolerance )
{
	...
	if (TargetMode == ETLLRN_SimplifyTargetType::MinimalPlanar)
	{
	...
	if (TargetMode == ETLLRN_SimplifyTargetType::Percentage)
	{
	...
	else if (TargetMode == ETLLRN_SimplifyTargetType::TriangleCount)
	{
		Reducer.SimplifyToTriangleCount(TargetCount);
	}
	else if (TargetMode == ETLLRN_SimplifyTargetType::VertexCount)
	{
		Reducer.SimplifyToVertexCount(TargetCount);
	}
	else if (TargetMode == ETLLRN_SimplifyTargetType::EdgeLength)
	{
		Reducer.SimplifyToEdgeLength(TargetEdgeLength);
	}
	else if (TargetMode == ETLLRN_SimplifyTargetType::MinimalPlanar)
	{
...
void FTLLRN_SimplifyMeshOp::CalculateResult(FProgressCancel* Progress)
{
	...
	if (SimplifierType == ETLLRN_SimplifyType::QEM)
	{
	...
	else if (SimplifierType == ETLLRN_SimplifyType::Attribute)
	{
	...
	else if (SimplifierType == ETLLRN_SimplifyType::MinimalPlanar)
	{
		bool bUseQuadricMemory = false;
		ResultMesh->Copy(*OriginalMesh, true, true, true, !bDiscardAttributes);
		if (!ResultMesh->HasAttributes() && !ResultMesh->HasVertexNormals())
		...
		ComputeSimplify<FQEMSimplification>(TargetMesh, bReproject, OriginalTriCount, *OriginalMesh, *OriginalMeshSpatial,
			...
			MaterialBoundaryConstraint,
			bPreserveSharpEdges, bAllowSeamCollapse, bPreventNormalFlips, bPreventTinyTriangles,
			ETLLRN_SimplifyTargetType::MinimalPlanar, TargetPercentage, TargetCount, TargetEdgeLength, MinimalPlanarAngleThresh,
			FQEMSimplification::ESimplificationCollapseModes::MinimalQuadricPositionError, bUseQuadricMemory,
			UseGeometricTolerance);
	}
	else if (SimplifierType == ETLLRN_SimplifyType::MinimalExistingVertex)
	{
		bool bUseQuadricMemory = true;
		ResultMesh->Copy(*OriginalMesh, true, true, true, !bDiscardAttributes);
		ComputeSimplify<FQEMSimplification>(TargetMesh, bReproject, OriginalTriCount, *OriginalMesh, *OriginalMeshSpatial,
	...
	else if (SimplifierType == ETLLRN_SimplifyType::MinimalPolygroup)
	{
		ResultMesh->Copy(*OriginalMesh, true, true, true, !bDiscardAttributes);
	...
	else // SimplifierType == ETLLRN_SimplifyType::UEStandard
	{

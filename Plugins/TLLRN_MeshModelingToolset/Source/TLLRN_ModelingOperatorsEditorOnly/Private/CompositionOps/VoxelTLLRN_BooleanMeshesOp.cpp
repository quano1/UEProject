...
#include "CompositionOps/VoxelTLLRN_BooleanMeshesOp.h"
...
void FVoxelTLLRN_BooleanMeshesOp::CalculateResult(FProgressCancel* Progress)
{
#if WITH_PROXYLOD
	...
	if (bFixNormals && !Interrupter.wasInterrupted())
	{
		...
		TLLRN_EditNormalsOp.bFixInconsistentNormals = true;
		...
		TLLRN_EditNormalsOp.bAllowSharpVertices = true;
...
float FVoxelTLLRN_BooleanMeshesOp::ComputeVoxelSize() const 
{ 
	float Size = 0.f;

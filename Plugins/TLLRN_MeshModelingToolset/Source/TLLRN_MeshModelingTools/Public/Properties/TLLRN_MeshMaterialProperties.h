...
#include "TLLRN_MeshMaterialProperties.generated.h"
...
UCLASS()
class TLLRN_MESHMODELINGTOOLS_API UTLLRN_NewTLLRN_MeshMaterialProperties : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
public:
	UTLLRN_NewTLLRN_MeshMaterialProperties();
...
UENUM()
enum class ETLLRN_SetMeshMaterialMode : uint8
{
	/** Input material */
	Original,
...
UCLASS()
class TLLRN_MESHMODELINGTOOLS_API UTLLRN_ExistingTLLRN_MeshMaterialProperties : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
public:
	...
	ETLLRN_SetMeshMaterialMode MaterialMode = ETLLRN_SetMeshMaterialMode::Original;

	/** Number of checkerboard tiles within the 0 to 1 range; only available when Checkerboard is selected as material mode */
	UPROPERTY(EditAnywhere, Category = PreviewMaterial,
		meta = (UIMin = "1.0", UIMax = "40.0", ClampMin = "0.01", ClampMax = "1000.0", EditConditionHides, EditCondition = "MaterialMode == ETLLRN_SetMeshMaterialMode::Checkerboard"))
	...
	UPROPERTY(EditAnywhere, Category = PreviewMaterial, meta = (EditConditionHides, EditCondition = "MaterialMode == ETLLRN_SetMeshMaterialMode::Override"))
	...
	UPROPERTY(EditAnywhere, Category = PreviewMaterial,
		meta = (DisplayName = "Preview UV Channel", GetOptions = GetUVChannelNamesFunc, EditConditionHides, EditCondition =
			"MaterialMode == ETLLRN_SetMeshMaterialMode::Checkerboard", NoResetToDefault))
	FString UVChannel;
...
UENUM()
enum class ETLLRN_MeshEditingMaterialModes
{
	ExistingMaterial,
	Diffuse,
	Grey,
...
UCLASS()
class TLLRN_MESHMODELINGTOOLS_API UTLLRN_MeshEditingViewProperties : public UInteractiveToolPropertySet
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(EditAnywhere, Category = Rendering)
	ETLLRN_MeshEditingMaterialModes MaterialMode = ETLLRN_MeshEditingMaterialModes::Diffuse;

	/** Toggle flat shading on/off */
	UPROPERTY(EditAnywhere, Category = Rendering, meta = (EditConditionHides, 
		EditCondition = "MaterialMode != ETLLRN_MeshEditingMaterialModes::ExistingMaterial && MaterialMode != ETLLRN_MeshEditingMaterialModes::Transparent && MaterialMode != ETLLRN_MeshEditingMaterialModes::Custom") )
	bool bFlatShading = true;

	/** Main Color of Material */
	UPROPERTY(EditAnywhere, Category = Rendering, meta = (EditConditionHides, EditCondition = "MaterialMode == ETLLRN_MeshEditingMaterialModes::Diffuse"))
	FLinearColor Color = FLinearColor(0.4f, 0.4f, 0.4f);

	/** Image used in Image-Based Material */
	UPROPERTY(EditAnywhere, Category = Rendering, meta = (EditConditionHides, EditCondition = "MaterialMode == ETLLRN_MeshEditingMaterialModes::CustomImage", TransientToolProperty) )
	TObjectPtr<UTexture2D> Image;

	/** Opacity of transparent material */
	UPROPERTY(EditAnywhere, Category = Rendering, meta = (EditConditionHides, EditCondition = "MaterialMode == ETLLRN_MeshEditingMaterialModes::Transparent", ClampMin = "0", ClampMax = "1.0"))
	double Opacity = 0.65;

	//~ Could have used the same property as Color, above, but the user may want different saved values for the two
	UPROPERTY(EditAnywhere, Category = Rendering, meta = (EditConditionHides, EditCondition = "MaterialMode == ETLLRN_MeshEditingMaterialModes::Transparent", DisplayName = "Color"))
	...
	UPROPERTY(EditAnywhere, Category = Rendering, meta = (EditConditionHides, EditCondition = "MaterialMode == ETLLRN_MeshEditingMaterialModes::Transparent"))
	bool bTwoSided = true;

	UPROPERTY(EditAnywhere, Category = Rendering, meta = (EditConditionHides, EditCondition = "MaterialMode == ETLLRN_MeshEditingMaterialModes::Custom"))

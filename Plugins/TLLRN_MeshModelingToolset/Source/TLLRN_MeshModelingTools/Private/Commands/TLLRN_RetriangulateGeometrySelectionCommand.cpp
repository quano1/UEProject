...
#include "Commands/TLLRN_RetriangulateGeometrySelectionCommand.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_RetriangulateGeometrySelectionCommand)

using namespace UE::Geometry;


#define LOCTEXT_NAMESPACE "UTLLRN_RetriangulateGeometrySelectionCommand"


FText UTLLRN_RetriangulateGeometrySelectionCommand::GetCommandShortString() const
{
	return LOCTEXT("ShortString", "Retriangulate Selection");
}


bool UTLLRN_RetriangulateGeometrySelectionCommand::CanExecuteCommandForSelection(UTLLRN_GeometrySelectionEditCommandArguments* SelectionArgs)
{
...
void UTLLRN_RetriangulateGeometrySelectionCommand::ExecuteCommandForSelection(UTLLRN_GeometrySelectionEditCommandArguments* SelectionArgs, UInteractiveCommandResult** Result)
{
	IGeometrySelector* BaseSelector = SelectionArgs->SelectionHandle.Selector;
	if (!ensure(BaseSelector != nullptr))
	{
		UE_LOG(LogGeometry, Warning, TEXT("UTLLRN_RetriangulateGeometrySelectionCommand: Retriangulate Selection requires Selector be provided in Selection Arguments"));
		return;
	}
	...
	MeshObject->EditMesh([&](FDynamicMesh3& EditMesh)
	{
		...
		{
			// ported from UTLLRN_EditMeshPolygonsTool::ApplyRetriangulate(), should be moved to a standalone geometry op

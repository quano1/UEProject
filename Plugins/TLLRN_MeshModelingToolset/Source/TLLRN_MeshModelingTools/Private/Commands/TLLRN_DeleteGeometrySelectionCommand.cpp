...
#include "Commands/TLLRN_DeleteGeometrySelectionCommand.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_DeleteGeometrySelectionCommand)

using namespace UE::Geometry;


#define LOCTEXT_NAMESPACE "UTLLRN_DeleteGeometrySelectionCommand"


FText UTLLRN_DeleteGeometrySelectionCommand::GetCommandShortString() const
{
	return LOCTEXT("ShortString", "Delete Selection");
}


bool UTLLRN_DeleteGeometrySelectionCommand::CanExecuteCommandForSelection(UTLLRN_GeometrySelectionEditCommandArguments* SelectionArgs)
{
...
void UTLLRN_DeleteGeometrySelectionCommand::ExecuteCommandForSelection(UTLLRN_GeometrySelectionEditCommandArguments* SelectionArgs, UInteractiveCommandResult** Result)
{
	IGeometrySelector* BaseSelector = SelectionArgs->SelectionHandle.Selector;
	if (!ensure(BaseSelector != nullptr))
	{
		UE_LOG(LogGeometry, Warning, TEXT("UTLLRN_DeleteGeometrySelectionCommand: Delete Selection requires Selector be provided in Selection Arguments"));
		return;
	}

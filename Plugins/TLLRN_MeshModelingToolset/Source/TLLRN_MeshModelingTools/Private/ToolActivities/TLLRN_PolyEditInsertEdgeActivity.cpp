...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_PolyEditInsertEdgeActivity)

using namespace UE::Geometry;

#define LOCTEXT_NAMESPACE "UTLLRN_PolyEditInsertEdgeActivity"

namespace TLLRN_PolyEditInsertEdgeActivityLocals
{
...
TUniquePtr<FDynamicMeshOperator> UTLLRN_PolyEditInsertEdgeActivity::MakeNewOperator()
{
	...
	if (Settings->InsertionMode == ETLLRN_GroupEdgeInsertionMode::PlaneCut)
	{
...
void UTLLRN_PolyEditInsertEdgeActivity::Setup(UInteractiveTool* ParentToolIn)
{
	Super::Setup(ParentToolIn);

	// Set up properties
	Settings = NewObject<UTLLRN_GroupEdgeInsertionProperties>(this);
	Settings->RestoreProperties(ParentTool.Get());
	AddToolPropertySource(Settings);
	SetToolPropertySourceEnabled(Settings, false);
	Settings->GetOnModified().AddUObject(this, &UTLLRN_PolyEditInsertEdgeActivity::OnPropertyModified);
...
void UTLLRN_PolyEditInsertEdgeActivity::UpdateComputeInputs()
{
...
void UTLLRN_PolyEditInsertEdgeActivity::Shutdown(EToolShutdownType ShutdownType)
{
	if (bIsRunning)
	{
		End(ShutdownType);
...
EToolActivityStartResult UTLLRN_PolyEditInsertEdgeActivity::Start()
{
...
EToolActivityEndResult UTLLRN_PolyEditInsertEdgeActivity::End(EToolShutdownType ShutdownType)
{
	if (!bIsRunning)
	{
...
void UTLLRN_PolyEditInsertEdgeActivity::SetupPreview()
{
...
bool UTLLRN_PolyEditInsertEdgeActivity::CanStart() const
{
	return true;
}

void UTLLRN_PolyEditInsertEdgeActivity::Tick(float DeltaTime)
{
			...
			if (Settings->bContinuousInsertion)
			{
				...
				if (GetHoveredItem(LastEndPointWorldRay, StartPoint, StartTopologyID, bStartIsCorner, PreviewPoint))
				{
					...
					ParentTool->GetToolManager()->EmitObjectChange(this,
						...
						TLLRN_PolyEditInsertEdgeActivityLocals::GroupEdgeStartTransactionName);
				}
			}
		}
		else
...
void UTLLRN_PolyEditInsertEdgeActivity::Render(IToolsContextRenderAPI* RenderAPI)
{
...
bool UTLLRN_PolyEditInsertEdgeActivity::CanAccept() const
{
	return ToolState != EState::WaitingForInsertComplete;
}

void UTLLRN_PolyEditInsertEdgeActivity::OnPropertyModified(UObject* PropertySet, FProperty* Property)
{
	PreviewEdges.Reset();
...
void UTLLRN_PolyEditInsertEdgeActivity::ClearPreview(bool bClearDrawnElements)
{
 ...
 */
void UTLLRN_PolyEditInsertEdgeActivity::ConditionallyUpdatePreview(
...
FInputRayHit UTLLRN_PolyEditInsertEdgeActivity::BeginHoverSequenceHitTest(const FInputDeviceRay& PressPos)
{
	FInputRayHit Hit;
...
bool UTLLRN_PolyEditInsertEdgeActivity::OnUpdateHover(const FInputDeviceRay& DevicePos)
{
	using namespace TLLRN_PolyEditInsertEdgeActivityLocals;
...
void UTLLRN_PolyEditInsertEdgeActivity::OnEndHover()
{
	if (!bIsRunning)
	{
		return;
...
FInputRayHit UTLLRN_PolyEditInsertEdgeActivity::IsHitByClick(const FInputDeviceRay& ClickPos)
{
	FInputRayHit Hit;
...
void UTLLRN_PolyEditInsertEdgeActivity::OnClicked(const FInputDeviceRay& ClickPos)
{
	using namespace TLLRN_PolyEditInsertEdgeActivityLocals;
	...
	case EState::GettingStart:
	{
		...
		if (GetHoveredItem(ClickPos.WorldRay, StartPoint, StartTopologyID, bStartIsCorner, PreviewPoint))
		{
			...
			ParentTool->GetToolManager()->BeginUndoTransaction(TLLRN_PolyEditInsertEdgeActivityLocals::GroupEdgeStartTransactionName);
...
bool UTLLRN_PolyEditInsertEdgeActivity::TopologyHitTest(const FRay& WorldRay,
...
bool UTLLRN_PolyEditInsertEdgeActivity::GetHoveredItem(const FRay& WorldRay,
...
void UTLLRN_PolyEditInsertEdgeActivity::GetCornerTangent(int32 CornerID, int32 GroupID, int32 BoundaryIndex, FVector3d& TangentOut)
{
...
bool TLLRN_PolyEditInsertEdgeActivityLocals::GetSharedBoundary(const FGroupTopology& Topology,
	const FGroupEdgeInserter::FGroupEdgeSplitPoint& StartPoint, int32 StartTopologyID, bool bStartIsCorner,
	const FGroupEdgeInserter::FGroupEdgeSplitPoint& EndPoint, int32 EndTopologyID, bool bEndIsCorner,
	int32& GroupIDOut, int32& BoundaryIndexOut)
{
...
bool TLLRN_PolyEditInsertEdgeActivityLocals::DoesBoundaryContainPoint(const FGroupTopology& Topology,
	const FGroupTopology::FGroupBoundary& Boundary, int32 PointTopologyID, bool bPointIsCorner)
{
	for (int32 GroupEdgeID : Boundary.GroupEdges)
	{
...
void FGroupEdgeInsertionFirstPointChange::Revert(UObject* Object)
{
	...
	check(Activity->ToolState == UTLLRN_PolyEditInsertEdgeActivity::EState::GettingEnd
		|| Activity->ToolState == UTLLRN_PolyEditInsertEdgeActivity::EState::WaitingForInsertComplete);

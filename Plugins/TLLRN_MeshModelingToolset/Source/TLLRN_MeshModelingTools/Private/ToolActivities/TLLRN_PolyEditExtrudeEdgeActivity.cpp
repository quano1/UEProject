...
#include "TLLRN_ModelingOperators.h" // FDynamicMeshOperator
...
#define LOCTEXT_NAMESPACE "TLLRN_PolyEditExtrudeEdgeActivity"

namespace TLLRN_PolyEditExtrudeEdgeActivityLocals
{
	...
	// These should match what is used in UTLLRN_MeshTopologySelectionMechanic (or better, live in a common place)
	...
	public:
		...
		ETLLRN_PolyEditExtrudeEdgeDirectionMode DirectionMode;
	...
	public:
		...
		virtual void CalculateResult(FProgressCancel* Progress) override
		{
			...
			if ((DirectionMode == ETLLRN_PolyEditExtrudeEdgeDirectionMode::LocalExtrudeFrames && LocalFrameParams.IsNearlyZero())
				|| (DirectionMode == ETLLRN_PolyEditExtrudeEdgeDirectionMode::SingleDirection && SingleDirectionVector.IsNearlyZero()))
			{
				return;
			}
			...
			{
			case ETLLRN_PolyEditExtrudeEdgeDirectionMode::LocalExtrudeFrames:
				...
				break;
			case ETLLRN_PolyEditExtrudeEdgeDirectionMode::SingleDirection:
				Extruder.OffsetPositionFunc = [this](const FVector3d& Position, const FExtrudeBoundaryEdges::FExtrudeFrame& ExtrudeFrame, int32 SourceVid)
				{
			...
			Extruder.bUsePerVertexExtrudeFrames = DirectionMode == ETLLRN_PolyEditExtrudeEdgeDirectionMode::LocalExtrudeFrames;
...
TUniquePtr<UE::Geometry::FDynamicMeshOperator> UTLLRN_PolyEditExtrudeEdgeActivity::MakeNewOperator()
{
	using namespace TLLRN_PolyEditExtrudeEdgeActivityLocals;
	...
	Op->OriginalMeshTransform = ActivityContext->Preview->TLLRN_PreviewMesh->GetTransform();
	...
	if (Settings->DirectionMode == ETLLRN_PolyEditExtrudeEdgeDirectionMode::SingleDirection)
	{
...
void UTLLRN_PolyEditExtrudeEdgeActivity::Setup(UInteractiveTool* ParentToolIn)
{
	...
	Settings = NewObject<UTLLRN_PolyEditExtrudeEdgeActivityProperties>();
	...
	Settings->WatchProperty(Settings->DirectionMode, [this](ETLLRN_PolyEditExtrudeEdgeDirectionMode) 
	{
		ConvertToNewDirectionMode(Settings->DirectionMode == ETLLRN_PolyEditExtrudeEdgeDirectionMode::SingleDirection);
	});
	...
	Settings->WatchProperty(Settings->DistanceMode, [this](ETLLRN_PolyEditExtrudeEdgeDistanceMode) 
	{
		if (!bIsRunning) { return; }
		...
		//  setting those.
		if (Settings->DistanceMode == ETLLRN_PolyEditExtrudeEdgeDistanceMode::Fixed)
		{
...
EToolActivityStartResult UTLLRN_PolyEditExtrudeEdgeActivity::Start()
{
	using namespace TLLRN_PolyEditExtrudeEdgeActivityLocals;
	...
	CurrentMeshTransform = ActivityContext->Preview->TLLRN_PreviewMesh->GetTransform();
	...
	ActivityContext->Preview->OnMeshUpdated.AddWeakLambda(this,
		[this](UTLLRN_MeshOpPreviewWithBackgroundCompute*)
	{
		// Do this here instead of OnOpCompleted, because we need access to the updated mesh.
		UpdateDrawnPreviewEdges();
	});

	TLLRN_PreviewGeometry = NewObject<UTLLRN_PreviewGeometry>();
	TLLRN_PreviewGeometry->CreateInWorld(ActivityContext->Preview->GetWorld(), FTransform::Identity);
	TLLRN_PreviewGeometry->SetTransform(CurrentMeshTransform);
...
EToolActivityEndResult UTLLRN_PolyEditExtrudeEdgeActivity::End(EToolShutdownType ShutdownType)
{
...
void UTLLRN_PolyEditExtrudeEdgeActivity::ApplyExtrude()
{
	using namespace UE::Geometry;
	using namespace TLLRN_PolyEditExtrudeEdgeActivityLocals;
...
void UTLLRN_PolyEditExtrudeEdgeActivity::EndInternal()
{
...
void UTLLRN_PolyEditExtrudeEdgeActivity::Shutdown(EToolShutdownType ShutdownType)
{
	if (IsRunning())
	{
		End(ShutdownType);
...
bool UTLLRN_PolyEditExtrudeEdgeActivity::CanStart() const
{
	using namespace UE::Geometry;
	if (!ActivityContext)
	{
...
bool UTLLRN_PolyEditExtrudeEdgeActivity::CanAccept() const
{
	return true;
}


void UTLLRN_PolyEditExtrudeEdgeActivity::UpdateGizmoVisibility()
{
	...
	{
		ExtrudeFrameGizmo->SetVisibility(Settings->DirectionMode == ETLLRN_PolyEditExtrudeEdgeDirectionMode::LocalExtrudeFrames);
		SingleDirectionGizmo->SetVisibility(Settings->DirectionMode == ETLLRN_PolyEditExtrudeEdgeDirectionMode::SingleDirection);
	}
}

void UTLLRN_PolyEditExtrudeEdgeActivity::GatherSelectedEids()
{
	SelectedEids.Reset();
...
void UTLLRN_PolyEditExtrudeEdgeActivity::UpdateDrawnPreviewEdges()
{
	using namespace TLLRN_PolyEditExtrudeEdgeActivityLocals;

	if (!bIsRunning 
		|| !ActivityContext || !ActivityContext->Preview || !ActivityContext->Preview->TLLRN_PreviewMesh 
		|| !TLLRN_PreviewGeometry)
	{
		return;
	}

	ActivityContext->Preview->TLLRN_PreviewMesh->ProcessMesh([this](const FDynamicMesh3& Mesh) 
	{
		TLLRN_PreviewGeometry->CreateOrUpdateLineSet(TEXT("NewBoundaries"), EidsToRender.Num(), [this, &Mesh]
...
void UTLLRN_PolyEditExtrudeEdgeActivity::UpdateDistanceFromParams()
{
	if (Settings->DirectionMode == ETLLRN_PolyEditExtrudeEdgeDirectionMode::SingleDirection)
	{
		Settings->Distance = SingleDirectionVectorWorldSpace.Length();
	}
	else // if (Settings->DirectionMode == ETLLRN_PolyEditExtrudeEdgeDirectionMode::LocalExtrudeFrames)
	{
		Settings->Distance = ExtrudeFrameForGizmoWorldSpace.FromFrameVector(ParamsInWorldExtrudeFrame).Length();
	}
}

void UTLLRN_PolyEditExtrudeEdgeActivity::UpdateGizmosFromCurrentParams()
{
...
void UTLLRN_PolyEditExtrudeEdgeActivity::ConvertToNewDirectionMode(bool bToSingleDirection)
{
...
void UTLLRN_PolyEditExtrudeEdgeActivity::ResetParams()
{
...
void UTLLRN_PolyEditExtrudeEdgeActivity::RecalculateGizmoExtrudeFrame()
{
	using namespace TLLRN_PolyEditExtrudeEdgeActivityLocals;
	using namespace UE::Geometry;
...
void UTLLRN_PolyEditExtrudeEdgeActivity::Render(IToolsContextRenderAPI* RenderAPI)
{
	using namespace TLLRN_PolyEditExtrudeEdgeActivityLocals;
...
void UTLLRN_PolyEditExtrudeEdgeActivity::Tick(float DeltaTime)
{
}

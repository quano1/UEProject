...
#include "TLLRN_MeshOpPreviewHelpers.h" // UTLLRN_MeshOpPreviewWithBackgroundCompute
...
#include "ToolActivities/TLLRN_PolyEditExtrudeEdgeActivity.h"
#include "ToolActivities/TLLRN_PolyEditInsertEdgeActivity.h"
...
#include "ToolActivities/TLLRN_PolyEditBevelEdgeActivity.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_EditMeshPolygonsTool)

using namespace UE::Geometry;

#define LOCTEXT_NAMESPACE "UTLLRN_EditMeshPolygonsTool"

namespace TLLRN_EditMeshPolygonsToolLocals
{
	FText PolyEditDefaultMessage = LOCTEXT("OnStartTLLRN_EditMeshPolygonsTool_TriangleMode", "Select triangles to edit mesh. Use middle mouse on gizmo to "
	...
	FText TriEditDefaultMessage = LOCTEXT("OnStartTLLRN_EditMeshPolygonsTool", "Select PolyGroups to edit mesh. Use middle mouse on gizmo to reposition it. "
	...
	public:
		...
		{
			Cast<UTLLRN_EditMeshPolygonsTool>(Object)->RebuildTopologyWithGivenExtraCorners(After);
		}
		virtual void Revert(UObject* Object) override
		{
			Cast<UTLLRN_EditMeshPolygonsTool>(Object)->RebuildTopologyWithGivenExtraCorners(Before);
		}
...
UTLLRN_SingleTargetWithSelectionTool* UTLLRN_EditMeshPolygonsToolBuilder::CreateNewTool(const FToolBuilderState& SceneState) const
{
	return NewObject<UTLLRN_EditMeshPolygonsTool>(SceneState.ToolManager);
}

void UTLLRN_EditMeshPolygonsToolBuilder::InitializeNewTool(UTLLRN_SingleTargetWithSelectionTool* Tool, const FToolBuilderState& SceneState) const
{
	UTLLRN_SingleTargetWithSelectionToolBuilder::InitializeNewTool(Tool, SceneState);
	UTLLRN_EditMeshPolygonsTool* EditPolygonsTool = CastChecked<UTLLRN_EditMeshPolygonsTool>(Tool);
	if (bTriangleMode)
	{
...
bool UTLLRN_EditMeshPolygonsActionModeToolBuilder::CanBuildTool(const FToolBuilderState& SceneState) const
{
	if (UTLLRN_EditMeshPolygonsToolBuilder::CanBuildTool(SceneState))
	{
		if ( UTLLRN_GeometrySelectionManager* SelectionManager = SceneState.ToolManager->GetContextObjectStore()->FindContext<UTLLRN_GeometrySelectionManager>() )
		{
			...
			if (SelectionManager->GetMeshTopologyMode() == UTLLRN_GeometrySelectionManager::EMeshTopologyMode::None)
			{
				TopologyType = EGeometryTopologyType::Polygroup;
			}

			bool bCanBuild = false;
			if (StartupAction == ETLLRN_EditMeshPolygonsToolActions::Extrude
				|| StartupAction == ETLLRN_EditMeshPolygonsToolActions::PushPull
				|| StartupAction == ETLLRN_EditMeshPolygonsToolActions::Offset
				|| StartupAction == ETLLRN_EditMeshPolygonsToolActions::Inset
				|| StartupAction == ETLLRN_EditMeshPolygonsToolActions::Outset
				|| StartupAction == ETLLRN_EditMeshPolygonsToolActions::CutFaces)
			{
				bCanBuild = (TopologyType == EGeometryTopologyType::Polygroup && ElementType == EGeometryElementType::Face && bIsEmpty == false);
			}
			else if (StartupAction == ETLLRN_EditMeshPolygonsToolActions::BevelAuto)
			{
				bCanBuild = (TopologyType == EGeometryTopologyType::Polygroup && ElementType != EGeometryElementType::Vertex && bIsEmpty == false);
			}
			else if (StartupAction == ETLLRN_EditMeshPolygonsToolActions::ExtrudeEdges)
			{
				bCanBuild = (ElementType == EGeometryElementType::Edge && !bIsEmpty);
			}
			else if ( StartupAction == ETLLRN_EditMeshPolygonsToolActions::SimplifyByGroups
			 || StartupAction == ETLLRN_EditMeshPolygonsToolActions::InsertEdge
			 || StartupAction == ETLLRN_EditMeshPolygonsToolActions::InsertEdgeLoop )
			{
...
void UTLLRN_EditMeshPolygonsActionModeToolBuilder::InitializeNewTool(UTLLRN_SingleTargetWithSelectionTool* Tool, const FToolBuilderState& SceneState) const
{
	UTLLRN_EditMeshPolygonsToolBuilder::InitializeNewTool(Tool, SceneState);
	...
	if (UTLLRN_GeometrySelectionManager* SelectionManager = SceneState.ToolManager->GetContextObjectStore()->FindContext<UTLLRN_GeometrySelectionManager>())
	{
		...
		if (SelectionManager->GetMeshTopologyMode() == UTLLRN_GeometrySelectionManager::EMeshTopologyMode::Triangle)
		{
			if (UTLLRN_EditMeshPolygonsTool* EditPolygonsTool = Cast<UTLLRN_EditMeshPolygonsTool>(Tool))
			{
				EditPolygonsTool->EnableTriangleMode();
			}
		}
	...
	ETLLRN_EditMeshPolygonsToolActions UseAction = StartupAction;
	EditPolygonsTool->PostSetupFunction = [UseAction](UTLLRN_EditMeshPolygonsTool* PolyTool)
	{
...
bool UTLLRN_EditMeshPolygonsSelectionModeToolBuilder::CanBuildTool(const FToolBuilderState& SceneState) const
{
	if (UTLLRN_EditMeshPolygonsToolBuilder::CanBuildTool(SceneState))
	{
		if ( UTLLRN_GeometrySelectionManager* SelectionManager = SceneState.ToolManager->GetContextObjectStore()->FindContext<UTLLRN_GeometrySelectionManager>() )
...
void UTLLRN_EditMeshPolygonsSelectionModeToolBuilder::InitializeNewTool(UTLLRN_SingleTargetWithSelectionTool* Tool, const FToolBuilderState& SceneState) const
{
	UTLLRN_EditMeshPolygonsToolBuilder::InitializeNewTool(Tool, SceneState);
	UTLLRN_EditMeshPolygonsTool* EditPolygonsTool = CastChecked<UTLLRN_EditMeshPolygonsTool>(Tool);
	...
	if (UTLLRN_GeometrySelectionManager* SelectionManager = SceneState.ToolManager->GetContextObjectStore()->FindContext<UTLLRN_GeometrySelectionManager>())
	{
		...
		ETLLRN_EditMeshPolygonsToolSelectionMode UseMode = ETLLRN_EditMeshPolygonsToolSelectionMode::Faces;
		bool bIsEdgeSelection = false;
		if (ElementType == EGeometryElementType::Edge)
		{
			bIsEdgeSelection = true;
			UseMode = ETLLRN_EditMeshPolygonsToolSelectionMode::Edges;
		}
		else if (ElementType == EGeometryElementType::Vertex)
		{
			UseMode = ETLLRN_EditMeshPolygonsToolSelectionMode::Vertices;
		}

		EditPolygonsTool->PostSetupFunction = [UseMode, bIsEdgeSelection](UTLLRN_EditMeshPolygonsTool* PolyTool)
		{
			...
			UTLLRN_PolygonSelectionMechanic* SelectionMechanic = PolyTool->SelectionMechanic;
			UTLLRN_MeshTopologySelectionMechanicProperties* SelectionProps = SelectionMechanic->Properties;
			...
			{
			default:
			case ETLLRN_EditMeshPolygonsToolSelectionMode::Faces:
				SelectionProps->bSelectFaces = true;
				break;
			case ETLLRN_EditMeshPolygonsToolSelectionMode::Edges:
				SelectionProps->bSelectEdges = true;
				break;
			case ETLLRN_EditMeshPolygonsToolSelectionMode::Vertices:
...
void UTLLRN_EditMeshPolygonsTool::SetToSelectionModeInterface()
{
...
void UTLLRN_EditMeshPolygonsToolActionPropertySet::PostAction(ETLLRN_EditMeshPolygonsToolActions Action)
{
...
UTLLRN_EditMeshPolygonsTool::UTLLRN_EditMeshPolygonsTool()
{
	SetToolDisplayName(LOCTEXT("TLLRN_EditMeshPolygonsToolName", "PolyGroup Edit"));
}

void UTLLRN_EditMeshPolygonsTool::EnableTriangleMode()
{
	check(Preview == nullptr);		// must not have been initialized!
	bTriangleMode = true;
}

void UTLLRN_EditMeshPolygonsTool::Setup()
{
	using namespace TLLRN_EditMeshPolygonsToolLocals;
	...
	TopologyProperties = NewObject<UTLLRN_PolyEditTopologyProperties>(this);
	...
	TopologyProperties->WatchProperty(TopologyProperties->ExtraCornerAngleThresholdDegrees,
		...
		// undo stack are able to refer to the same edges/corners (this is also why we store the current extra corners in FTLLRN_EditMeshPolygonsToolMeshChange).
	...
	if (bTriangleMode)
	{
		EditActions_Triangles = NewObject<UTLLRN_EditMeshPolygonsToolActions_Triangles>();
		EditActions_Triangles->Initialize(this);
		AddToolPropertySource(EditActions_Triangles);

		EditEdgeActions_Triangles = NewObject<UTLLRN_EditMeshPolygonsToolEdgeActions_Triangles>();
	...
	{
		EditActions = NewObject<UTLLRN_EditMeshPolygonsToolActions>();
		EditActions->Initialize(this);
		AddToolPropertySource(EditActions);

		EditEdgeActions = NewObject<UTLLRN_EditMeshPolygonsToolEdgeActions>();
		EditEdgeActions->Initialize(this);
		AddToolPropertySource(EditEdgeActions);

		EditUVActions = NewObject<UTLLRN_EditMeshPolygonsToolUVActions>();
	...
	CommonProps = NewObject<UTLLRN_PolyEditCommonProperties>(this);
	CommonProps->RestoreProperties(this, GetPropertyCacheIdentifier(bTriangleMode));

	CommonProps->WatchProperty(CommonProps->TLLRN_LocalFrameMode,
		[this](ETLLRN_LocalFrameMode) { UpdateGizmoFrame(); });
	...
	Preview = NewObject<UTLLRN_MeshOpPreviewWithBackgroundCompute>();
	...
	Preview->TLLRN_PreviewMesh->bBuildSpatialDataStructure = true;
	...
	{
		// Note that you have to do it this way rather than reaching into the TLLRN_PreviewMesh because the background compute
	...
	Preview->TLLRN_PreviewMesh->EnableSecondaryTriangleBuffers(
	...
	Preview->TLLRN_PreviewMesh->UpdatePreview(CurrentMesh.Get());
	Preview->TLLRN_PreviewMesh->EnableWireframe(CommonProps->bShowWireframe);
	...
	SelectionMechanic = NewObject<UTLLRN_PolygonSelectionMechanic>(this);
	...
	SelectionMechanic->OnSelectionChanged.AddUObject(this, &UTLLRN_EditMeshPolygonsTool::OnSelectionModifiedEvent);
	SelectionMechanic->OnFaceSelectionPreviewChanged.AddWeakLambda(this, [this]() {
		Preview->TLLRN_PreviewMesh->FastNotifySecondaryTrianglesChanged();
	});
	if (bTriangleMode)
	{
	...
	SelectionMechanic->Initialize(CurrentMesh.Get(),
		(FTransform3d)Preview->TLLRN_PreviewMesh->GetTransform(),
		GetTargetWorld(),
		Topology.Get(),
	...
	ActivityContext = NewObject<UTLLRN_PolyEditActivityContext>();
	...
	GetToolManager()->GetContextObjectStore()->RemoveContextObjectsOfType(UTLLRN_PolyEditActivityContext::StaticClass());
	GetToolManager()->GetContextObjectStore()->AddContextObject(ActivityContext);

	ExtrudeActivity = NewObject<UTLLRN_PolyEditExtrudeActivity>();
	...
	InsetOutsetActivity = NewObject<UTLLRN_PolyEditInsetOutsetActivity>();
	...
	CutFacesActivity = NewObject<UTLLRN_PolyEditCutFacesActivity>();
	...
	PlanarProjectionUVActivity = NewObject<UTLLRN_PolyEditPlanarProjectionUVActivity>();
	...
	InsertEdgeLoopActivity = NewObject<UTLLRN_PolyEditInsertEdgeLoopActivity>();
	...
	InsertEdgeActivity = NewObject<UTLLRN_PolyEditInsertEdgeActivity>();
	...
	BevelEdgeActivity = NewObject<UTLLRN_PolyEditBevelEdgeActivity>();
	...
	ExtrudeEdgeActivity = NewObject<UTLLRN_PolyEditExtrudeEdgeActivity>();
	...
	if (ensure(TransformGizmo)) // If we don't get a valid gizmo a lot of interactions won't work, but at least we won't crash
	{
		...
		TransformProxy->OnTransformChanged.AddUObject(this, &UTLLRN_EditMeshPolygonsTool::OnGizmoTransformChanged);
		TransformProxy->OnBeginTransformEdit.AddUObject(this, &UTLLRN_EditMeshPolygonsTool::OnBeginGizmoTransform);
		TransformProxy->OnEndTransformEdit.AddUObject(this, &UTLLRN_EditMeshPolygonsTool::OnEndGizmoTransform);
	...
	TLLRN_DragAlignmentMechanic->InitializeDeformedMeshRayCast(
		[this]() { return &GetSpatial(); },
...
void UTLLRN_EditMeshPolygonsTool::ResetUserMessage()
{
...
void UTLLRN_EditMeshPolygonsTool::OnShutdown(EToolShutdownType ShutdownType)
{
	using namespace TLLRN_EditMeshPolygonsToolLocals;
	...
	if (CurrentActivity)
	{
		if (ITLLRN_ToolHostCustomizationAPI* ButtonCustomizer = ITLLRN_ToolHostCustomizationAPI::Find(GetToolManager()).GetInterface())
		{
	...
	if (Preview != nullptr)
	{
		...
		if (ShutdownType == EToolShutdownType::Accept)
		{
			...
			GetToolManager()->BeginUndoTransaction(LOCTEXT("TLLRN_EditMeshPolygonsToolTransactionName", "Deform Mesh"));
...
void UTLLRN_EditMeshPolygonsTool::RegisterActions(FInteractiveToolActionSet& ActionSet)
{
	...
	auto OnDeletionKeyPress = [this]() 
	{
		...
		{
			RequestAction(ETLLRN_EditMeshPolygonsToolActions::Delete);
		}
	};
...
void UTLLRN_EditMeshPolygonsTool::RequestAction(ETLLRN_EditMeshPolygonsToolActions ActionType)
{
	if (SelectionMechanic && SelectionMechanic->IsCurrentlyMarqueeDragging())
	{
		PendingAction = ETLLRN_EditMeshPolygonsToolActions::NoAction;
		GetToolManager()->DisplayMessage(
	...
	if (PendingAction != ETLLRN_EditMeshPolygonsToolActions::NoAction)
	{
		return;
	}
...
void UTLLRN_EditMeshPolygonsTool::RequestSingleShotAction(ETLLRN_EditMeshPolygonsToolActions ActionType)
{
	bTerminateOnPendingActionComplete = true;

	if ( ActionType == ETLLRN_EditMeshPolygonsToolActions::BevelAuto )
	{
		if (SelectionMechanic->GetActiveSelection().SelectedEdgeIDs.Num() > 0)
		{
			ActionType = ETLLRN_EditMeshPolygonsToolActions::BevelEdges;
		}
		else
		{
			ActionType = ETLLRN_EditMeshPolygonsToolActions::BevelFaces;
		}
	}
...
FDynamicMeshAABBTree3& UTLLRN_EditMeshPolygonsTool::GetSpatial()
{
	if (bSpatialDirty)
	{
		MeshSpatial->Build();
...
void UTLLRN_EditMeshPolygonsTool::UpdateGizmoFrame(const FFrame3d* UseFrame)
{
	FFrame3d SetFrame = LastTransformerFrame;
	if (UseFrame == nullptr)
	{
		if (CommonProps->TLLRN_LocalFrameMode == ETLLRN_LocalFrameMode::FromGeometry)
		{
...
FBox UTLLRN_EditMeshPolygonsTool::GetWorldSpaceFocusBox()
{
...
bool UTLLRN_EditMeshPolygonsTool::GetWorldSpaceFocusPoint(const FRay& WorldRay, FVector& PointOut)
{
...
void UTLLRN_EditMeshPolygonsTool::OnSelectionModifiedEvent()
{
	bSelectionStateDirty = true;
}


void UTLLRN_EditMeshPolygonsTool::OnBeginGizmoTransform(UTransformProxy* Proxy)
{
...
void UTLLRN_EditMeshPolygonsTool::OnGizmoTransformChanged(UTransformProxy* Proxy, FTransform Transform)
{
	if (bInGizmoDrag)
	{
...
void UTLLRN_EditMeshPolygonsTool::OnEndGizmoTransform(UTransformProxy* Proxy)
{
	bInGizmoDrag = false;
...
void UTLLRN_EditMeshPolygonsTool::UpdateDeformerFromSelection(const FGroupTopologySelection& Selection)
{
...
void UTLLRN_EditMeshPolygonsTool::ComputeUpdate_Gizmo()
{
...
void UTLLRN_EditMeshPolygonsTool::OnTick(float DeltaTime)
{
	if (bToolDisabled)
	{
		return;
	...
	if (PendingAction != ETLLRN_EditMeshPolygonsToolActions::NoAction)
	{
		...
		case ETLLRN_EditMeshPolygonsToolActions::Extrude:
		{
			...
			ExtrudeActivity->PropertySetToUse = UTLLRN_PolyEditExtrudeActivity::EPropertySetToUse::Extrude;
		...
		case ETLLRN_EditMeshPolygonsToolActions::PushPull:
		{
			ExtrudeActivity->ExtrudeMode = FExtrudeOp::EExtrudeMode::Boolean;
			ExtrudeActivity->PropertySetToUse = UTLLRN_PolyEditExtrudeActivity::EPropertySetToUse::PushPull;
		...
		case ETLLRN_EditMeshPolygonsToolActions::Offset:
		{
			ExtrudeActivity->ExtrudeMode = FExtrudeOp::EExtrudeMode::MoveAndStitch;
			ExtrudeActivity->PropertySetToUse = UTLLRN_PolyEditExtrudeActivity::EPropertySetToUse::Offset;
		...
		case ETLLRN_EditMeshPolygonsToolActions::Inset:
		{
		...
		case ETLLRN_EditMeshPolygonsToolActions::Outset:
		{
		...
		case ETLLRN_EditMeshPolygonsToolActions::CutFaces:
		{
			StartActivity(CutFacesActivity);
			break;
		}
		case ETLLRN_EditMeshPolygonsToolActions::PlanarProjectionUV:
		{
			StartActivity(PlanarProjectionUVActivity);
			break;
		}
		case ETLLRN_EditMeshPolygonsToolActions::InsertEdge:
		{
			StartActivity(InsertEdgeActivity);
			break;
		}
		case ETLLRN_EditMeshPolygonsToolActions::InsertEdgeLoop:
		{
			StartActivity(InsertEdgeLoopActivity);
			break;
		}
		case ETLLRN_EditMeshPolygonsToolActions::ExtrudeEdges:
		{
		...
		case ETLLRN_EditMeshPolygonsToolActions::BevelFaces:
		case ETLLRN_EditMeshPolygonsToolActions::BevelEdges:
		{
		...
		case ETLLRN_EditMeshPolygonsToolActions::Merge:
			ApplyMerge();
			break;
		case  ETLLRN_EditMeshPolygonsToolActions::Delete:
			ApplyDelete();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::RecalculateNormals:
			ApplyRecalcNormals();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::FlipNormals:
			ApplyFlipNormals();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::CollapseEdge:
			ApplyCollapseEdge();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::WeldEdges:
			ApplyWeldEdges();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::WeldEdgesCentered:
			ApplyWeldEdges(0.5);
			break;
		case ETLLRN_EditMeshPolygonsToolActions::StraightenEdge:
			ApplyStraightenEdges();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::FillHole:
			ApplyFillHole();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::BridgeEdges:
			ApplyBridgeEdges();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::SimplifyAlongEdges:
			ApplySimplifyAlongEdges();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::Retriangulate:
			ApplyRetriangulate();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::Decompose:
			ApplyDecompose();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::Disconnect:
			ApplyDisconnect();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::Duplicate:
			ApplyDuplicate();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::PokeSingleFace:
			ApplyPokeSingleFace();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::SplitSingleEdge:
			ApplySplitSingleEdge();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::CollapseSingleEdge:
			ApplyCollapseEdge();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::FlipSingleEdge:
			ApplyFlipSingleEdge();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::SimplifyByGroups:
			SimplifyByGroups();
			break;
		case ETLLRN_EditMeshPolygonsToolActions::RegenerateExtraCorners:
...
void UTLLRN_EditMeshPolygonsTool::StartActivity(TObjectPtr<UTLLRN_InteractiveToolActivity> Activity)
{
	...
	if (Activity->Start() == EToolActivityStartResult::Running)
	{
		...
		if ( bTerminateOnPendingActionComplete == false )
		{
			...
			if (ITLLRN_ToolHostCustomizationAPI* ButtonCustomizer = ITLLRN_ToolHostCustomizationAPI::Find(GetToolManager()).GetInterface())
			{
				...
				{
					ITLLRN_ToolHostCustomizationAPI::FAcceptCancelButtonOverrideParams Params;
				...
				{
					ITLLRN_ToolHostCustomizationAPI::FCompleteButtonOverrideParams Params;
					Params.Label = ActivityLabels.Contains(CurrentActivity) ? ActivityLabels[CurrentActivity] : SubActionFallbackLabel;
					if (ActivityIconNames.Contains(CurrentActivity))
					{
						Params.IconName = ActivityIconNames[CurrentActivity];
...
void UTLLRN_EditMeshPolygonsTool::EndCurrentActivity(EToolShutdownType ShutdownType)
{
	if (CurrentActivity)
	{
...
void UTLLRN_EditMeshPolygonsTool::NotifyActivitySelfEnded(UTLLRN_InteractiveToolActivity* Activity)
{
	EndCurrentActivity(EToolShutdownType::Accept);
}

void UTLLRN_EditMeshPolygonsTool::UpdateGizmoVisibility()
{
...
void UTLLRN_EditMeshPolygonsTool::Render(IToolsContextRenderAPI* RenderAPI)
{
	if (bToolDisabled)
	{
		return;
...
void UTLLRN_EditMeshPolygonsTool::DrawHUD(FCanvas* Canvas, IToolsContextRenderAPI* RenderAPI)
{
	if (bToolDisabled)
	{
		return;
...
void UTLLRN_EditMeshPolygonsTool::OnPropertyModified(UObject* PropertySet, FProperty* Property)
{
	if (Property && (Property->GetFName() == GET_MEMBER_NAME_CHECKED(UTLLRN_PolyEditCommonProperties, bShowSelectableCorners)))
	{
		SelectionMechanic->SetShowSelectableCorners(CommonProps->bShowSelectableCorners);
	}
}
...
//
void UTLLRN_EditMeshPolygonsTool::UpdateDeformerChangeFromROI(bool bFinal)
{
...
void UTLLRN_EditMeshPolygonsTool::BeginDeformerChange()
{
	if (ActiveVertexChange == nullptr)
	{
...
void UTLLRN_EditMeshPolygonsTool::EndDeformerChange()
{
	if (ActiveVertexChange != nullptr)
	{
...
void UTLLRN_EditMeshPolygonsTool::ApplyChange(const FMeshVertexChange* Change, bool bRevert)
{
	Preview->TLLRN_PreviewMesh->ApplyChange(Change, bRevert);
	CurrentMesh->Copy(*Preview->TLLRN_PreviewMesh->GetMesh());
	bSpatialDirty = true;
	SelectionMechanic->NotifyMeshChanged(false);
...
void UTLLRN_EditMeshPolygonsTool::UpdateFromCurrentMesh(bool bUpdateTopology)
{
	Preview->TLLRN_PreviewMesh->UpdatePreview(CurrentMesh.Get(), UTLLRN_PreviewMesh::ERenderUpdateMode::FullUpdate);
	bSpatialDirty = true;
...
void UTLLRN_EditMeshPolygonsTool::ApplyDelete()
{
...
void UTLLRN_EditMeshPolygonsTool::ApplyMerge()
{
...
void UTLLRN_EditMeshPolygonsTool::ApplyDeleteFaces()
{
	FDynamicMesh3* Mesh = CurrentMesh.Get();
...
void UTLLRN_EditMeshPolygonsTool::ApplyRecalcNormals()
{
	if (BeginMeshFaceEditChange() == false)
	{
		GetToolManager()->DisplayMessage(
...
void UTLLRN_EditMeshPolygonsTool::ApplyFlipNormals()
{
	if (BeginMeshFaceEditChange() == false)
	{
		GetToolManager()->DisplayMessage(
...
void UTLLRN_EditMeshPolygonsTool::ApplyRetriangulate()
{
	using namespace TLLRN_EditMeshPolygonsToolLocals;
	if (BeginMeshFaceEditChange() == false)
	{
...
void UTLLRN_EditMeshPolygonsTool::SimplifyByGroups()
{
	FDynamicMesh3* Mesh = CurrentMesh.Get();
	FDynamicMeshChangeTracker ChangeTracker(Mesh);
	ChangeTracker.BeginChange();
...
void UTLLRN_EditMeshPolygonsTool::ApplyRegenerateExtraCorners()
{
	...
	if (bCornersChanged)
	{
		...
		GetToolManager()->EmitObjectChange(this,
			MakeUnique<TLLRN_EditMeshPolygonsToolLocals::FExtraCornerChange>(PreviousExtraCorners, NewExtraCorners),
			TransactionLabel);
...
void UTLLRN_EditMeshPolygonsTool::RebuildTopologyWithGivenExtraCorners(const TSet<int32>& Vids)
{
...
void UTLLRN_EditMeshPolygonsTool::ApplyDecompose()
{
	if (BeginMeshFaceEditChange() == false)
	{
...
void UTLLRN_EditMeshPolygonsTool::ApplyDisconnect()
{
	if (BeginMeshFaceEditChange() == false)
	{
...
void UTLLRN_EditMeshPolygonsTool::ApplyDuplicate()
{
	if (BeginMeshFaceEditChange() == false)
	{
...
// Deprecated
void UTLLRN_EditMeshPolygonsTool::ApplyCollapseSingleEdge()
{
	ApplyCollapseEdge();
}
...
void UTLLRN_EditMeshPolygonsTool::ApplyWeldEdges()
{
	ApplyWeldEdges(0);
}

void UTLLRN_EditMeshPolygonsTool::ApplyWeldEdges(double InterpolationT)
{
	using namespace TLLRN_EditMeshPolygonsToolLocals;
...
void UTLLRN_EditMeshPolygonsTool::ApplyWeldVertices(double InterpolationT)
{
	using namespace TLLRN_EditMeshPolygonsToolLocals;
...
void UTLLRN_EditMeshPolygonsTool::ApplyStraightenEdges()
{
...
void UTLLRN_EditMeshPolygonsTool::ApplyDeleteEdges()
{
	FDynamicMesh3* Mesh = CurrentMesh.Get();
	FDynamicMeshChangeTracker ChangeTracker(Mesh);
	FGroupTopologySelection NewSelection;
...
void UTLLRN_EditMeshPolygonsTool::ApplySimplifyAlongEdges()
{
	if (BeginMeshEdgeEditChange() == false)
	{
...
void UTLLRN_EditMeshPolygonsTool::ApplyFillHole()
{
...
void UTLLRN_EditMeshPolygonsTool::ApplyBridgeEdges()
{
	using namespace TLLRN_EditMeshPolygonsToolLocals;
...
void UTLLRN_EditMeshPolygonsTool::ApplyPokeSingleFace()
{
	if (BeginMeshFaceEditChange() == false)
	{
...
void UTLLRN_EditMeshPolygonsTool::ApplyFlipSingleEdge()
{
	FDynamicMesh3* Mesh = CurrentMesh.Get();
...
void UTLLRN_EditMeshPolygonsTool::ApplyCollapseEdge()
{
	using namespace TLLRN_EditMeshPolygonsToolLocals;
...
void UTLLRN_EditMeshPolygonsTool::CollapseGroupEdges(TSet<int32>& GroupEdgesToCollapse, 
...
void UTLLRN_EditMeshPolygonsTool::ApplySplitSingleEdge()
{
	if (BeginMeshEdgeEditChange() == false)
	{
...
bool UTLLRN_EditMeshPolygonsTool::BeginMeshFaceEditChange()
{
...
void UTLLRN_EditMeshPolygonsTool::EmitCurrentMeshChangeAndUpdate(const FText& TransactionLabel,
	...
	const FGroupTopologySelection& OutputSelection)
{
	...
	TUniquePtr<FTLLRN_EditMeshPolygonsToolMeshChange> ChangeToEmit = MakeUnique<FTLLRN_EditMeshPolygonsToolMeshChange>(MoveTemp(MeshChangeIn));
...
void UTLLRN_EditMeshPolygonsTool::EmitActivityStart(const FText& TransactionLabel)
{
	++ActivityTimestamp;
...
bool UTLLRN_EditMeshPolygonsTool::BeginMeshEdgeEditChange()
{
	return BeginMeshEdgeEditChange([](int32) {return true; });
}

bool UTLLRN_EditMeshPolygonsTool::BeginMeshBoundaryEdgeEditChange(bool bOnlySimple)
{
	if (bOnlySimple)
	{
...
bool UTLLRN_EditMeshPolygonsTool::BeginMeshEdgeEditChange(TFunctionRef<bool(int32)> GroupEdgeIDFilterFunc)
{
...
void UTLLRN_EditMeshPolygonsTool::SetActionButtonPanelsVisible(bool bVisible)
{
...
bool UTLLRN_EditMeshPolygonsTool::CanCurrentlyNestedCancel()
{
...
bool UTLLRN_EditMeshPolygonsTool::ExecuteNestedCancelCommand()
{
	if (CurrentActivity)
	{
...
bool UTLLRN_EditMeshPolygonsTool::CanCurrentlyNestedAccept()
{
	if ( bTerminateOnPendingActionComplete ) return false;
	return CurrentActivity != nullptr;
}

bool UTLLRN_EditMeshPolygonsTool::ExecuteNestedAcceptCommand()
{
	if (CurrentActivity)
	{
		EndCurrentActivity(EToolShutdownType::Accept);
...
void FTLLRN_EditMeshPolygonsToolMeshChange::Apply(UObject* Object)
{
	UTLLRN_EditMeshPolygonsTool* Tool = Cast<UTLLRN_EditMeshPolygonsTool>(Object);
...
void FTLLRN_EditMeshPolygonsToolMeshChange::Revert(UObject* Object)
{
	UTLLRN_EditMeshPolygonsTool* Tool = Cast<UTLLRN_EditMeshPolygonsTool>(Object);
...
FString FTLLRN_EditMeshPolygonsToolMeshChange::ToString() const
{
	return TEXT("FTLLRN_EditMeshPolygonsToolMeshChange");
}
...
void FPolyEditActivityStartChange::Revert(UObject* Object)
{
	...
	Cast<UTLLRN_EditMeshPolygonsTool>(Object)->EndCurrentActivity(EToolShutdownType::Cancel);
	bHaveDoneUndo = true;
}
bool FPolyEditActivityStartChange::HasExpired(UObject* Object) const
{
	return bHaveDoneUndo 
		|| Cast<UTLLRN_EditMeshPolygonsTool>(Object)->ActivityTimestamp != ActivityTimestamp;
}

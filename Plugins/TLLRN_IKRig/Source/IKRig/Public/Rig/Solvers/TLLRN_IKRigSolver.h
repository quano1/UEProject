...
#include "TLLRN_IKRigSolver.generated.h"

class FPrimitiveDrawInterface;
struct FTLLRN_IKRigGoalContainer;
...
class TLLRN_IKRIG_API UTLLRN_IKRigSolver : public UObject
{
	GENERATED_BODY()
	
public:

	UTLLRN_IKRigSolver() { SetFlags(RF_Transactional); }

	//** RUNTIME */
	/** override to setup internal data based on ref pose */
	virtual void Initialize(const FTLLRN_IKRigSkeleton& TLLRN_IKRigSkeleton) PURE_VIRTUAL("Init");
	/** override Solve() to evaluate new output pose (InOutGlobalTransform) */
	virtual void Solve(FTLLRN_IKRigSkeleton& TLLRN_IKRigSkeleton, const FTLLRN_IKRigGoalContainer& Goals) PURE_VIRTUAL("Solve");
	//** END RUNTIME */
	...
	* NOTE: Only ADD to the incoming set, do not remove from it. */
	 ...
	 * This is necessary because at runtime, the TLLRN_IKRigProcessor creates a copy of your solver class
	 * and the copy must be notified of changes made to the class settings in the source asset.*/
	virtual void UpdateSolverSettings(UTLLRN_IKRigSolver* InSettings){};
	...
	DECLARE_EVENT_OneParam(UTLLRN_IKRigSolver, FTLLRN_IKRigSolverModified, UTLLRN_IKRigSolver*);
	FTLLRN_IKRigSolverModified& OnSolverModified(){ return TLLRN_IKRigSolverModified; };
	...
	virtual void AddGoal(const UTLLRN_IKRigEffectorGoal& NewGoal) PURE_VIRTUAL("AddGoal");
	...
	virtual void DrawBoneSettings(const FName& BoneName, const FTLLRN_IKRigSkeleton& TLLRN_IKRigSkeleton, FPrimitiveDrawInterface* PDI) const {};
	/** return true if the supplied Bone is affected by this solver - this provides UI feedback for user */
	virtual bool IsBoneAffectedBySolver(const FName& BoneName, const FTLLRN_IKRigSkeleton& TLLRN_IKRigSkeleton) const { return false; };
	//** END ROOT BONE */
...
private:
	...
	FTLLRN_IKRigSolverModified TLLRN_IKRigSolverModified;
#endif

...
#include "TLLRN_IKRig_FBIKSolver.generated.h"

UCLASS(BlueprintType)
class TLLRN_IKRIG_API UTLLRN_IKRig_FBIKEffector : public UObject
{
	GENERATED_BODY()

public:
	UTLLRN_IKRig_FBIKEffector() { SetFlags(RF_Transactional); }
	...
	*Blends the effector bone rotation between the rotation of the effector transform (1.0) and the rotation of the input bone (0.0).*/
	...
	void CopySettings(const UTLLRN_IKRig_FBIKEffector* Other)
	{
...
UCLASS(BlueprintType)
class TLLRN_IKRIG_API UTLLRN_IKRig_FBIKBoneSettings : public UObject
{
	GENERATED_BODY()

public:
	
	UTLLRN_IKRig_FBIKBoneSettings()
		: Bone(NAME_None), 
	...
	*Locked: no rotation is allowed in this axis (will remain at input pose angle). */
	...
	void CopySettings(const UTLLRN_IKRig_FBIKBoneSettings* Other)
	{	
...
class TLLRN_IKRIG_API UTLLRN_IKRigFBIKSolver : public UTLLRN_IKRigSolver
{
	GENERATED_BODY()
...
public:
	...
	*Free: treats the root bone like any other and allows it to move freely or according to any bone settings applied to it. */
	...
	TArray<UTLLRN_IKRig_FBIKEffector*> GetEffectors()
	{
		return Effectors;
	}

	UFUNCTION(BlueprintCallable, Category = "IKRig")
	TArray<UTLLRN_IKRig_FBIKBoneSettings*> GetBoneSettings()
	{
		return BoneSettings;
	}

	UPROPERTY()
	TArray<TObjectPtr<UTLLRN_IKRig_FBIKEffector>> Effectors;

	UPROPERTY()
	TArray<TObjectPtr<UTLLRN_IKRig_FBIKBoneSettings>> BoneSettings;
	...
	virtual void DrawBoneSettings(const FName& BoneName, const FTLLRN_IKRigSkeleton& TLLRN_IKRigSkeleton, FPrimitiveDrawInterface* PDI) const override;

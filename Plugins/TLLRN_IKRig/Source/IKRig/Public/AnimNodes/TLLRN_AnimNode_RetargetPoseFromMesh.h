...
#include "TLLRN_AnimNode_RetargetPoseFromMesh.generated.h"

DECLARE_CYCLE_STAT(TEXT("IK Retarget"), STAT_IKRetarget, STATGROUP_Anim);

USTRUCT(BlueprintInternalUseOnly)
struct TLLRN_IKRIG_API FTLLRN_AnimNode_RetargetPoseFromMesh : public FAnimNode_Base
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category=Settings, meta=(PinHiddenByDefault))
	FTLLRN_RetargetProfile CustomRetargetProfile;
...
private:
	...
	FTLLRN_RetargetProfile GetMergedRetargetProfile(bool bEnableIK) const;

	/** the runtime processor used to run the retarget and generate new poses */
	UPROPERTY(Transient)
	TObjectPtr<UTLLRN_IKRetargetProcessor> Processor = nullptr;
	...
	// remap curves for TLLRN_CurveRemapOp if one is present in the RetargetOp stack

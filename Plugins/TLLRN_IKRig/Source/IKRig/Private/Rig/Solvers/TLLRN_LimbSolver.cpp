...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_LimbSolver)

void FTLLRN_LimbSolver::Reset()
{
	Length = 0.f;
	Links.Empty();
}

bool FTLLRN_LimbSolver::Initialize()
{
	Length = 0.f;
	...
	{
		FTLLRN_LimbLink& Link = Links[Index];
...
int32 FTLLRN_LimbSolver::NumLinks() const
{
	return Links.Num();
}

int32 FTLLRN_LimbSolver::AddLink(const FVector& InLocation, int32 InBoneIndex)
{
	return Links.Emplace(InLocation, InBoneIndex);
}

int32 FTLLRN_LimbSolver::GetBoneIndex(int32 Index) const
{
	check(Index >= INDEX_NONE && Index < Links.Num());
	return Links[Index].BoneIndex; 
}

bool FTLLRN_LimbSolver::OrientTransformsTowardsGoal(TArray<FTransform>& InOutTransforms, const FVector& InGoalLocation) const
{
	...
	if (!InitialDir.IsZero() && !InitialDir.Equals(TargetDir))
	{
		...
		if (!DeltaRotation.IsIdentity())
		{
			...
			for (const FTLLRN_LimbLink& Link: Links)
			{
...
void FTLLRN_LimbSolver::UpdateTransforms(TArray<FTransform>& InOutTransforms)
{
	// Rotations
	for (int32 Index = 1; Index < NumLinks(); Index++)
	{
...
bool FTLLRN_LimbSolver::AdjustKneeTwist(TArray<FTransform>& InOutTransforms, const FTransform& InGoalTransform, const EAxis::Type FootBoneForwardAxis )
{
	const int32 EndIndex = Links.Last().BoneIndex;
...
{
	void SolveStraightLimb(TArray<FTLLRN_LimbLink>& InOutLinks, const FVector& GoalLocation)
	{
	...
	void SolveTwoBoneIK(TArray<FTLLRN_LimbLink>& InOutLinks, const FVector& InTargetLocation, const FVector& HingeRotationAxis)
	{
	...
	FVector FindPlaneNormal(const TArray<FTLLRN_LimbLink>& InLinks, const FVector& RootLocation, const FVector& TargetLocation)
	{
	...
	void FABRIK_ApplyLinkConstraints_Forward(TArray<FTLLRN_LimbLink>& InOutLinks, int32 LinkIndex, float MinRotationAngleRadians)
	{
		...
		const FTLLRN_LimbLink& ChildLink = InOutLinks[LinkIndex + 1];
		const FTLLRN_LimbLink& CurrentLink = InOutLinks[LinkIndex];
		FTLLRN_LimbLink& ParentLink = InOutLinks[LinkIndex - 1];
	...
	void FABRIK_ApplyLinkConstraints_Backward(TArray<FTLLRN_LimbLink>& InOutLinks, int32 LinkIndex, float MinRotationAngleRadians)
	{
		if ((LinkIndex <= 0) || (LinkIndex >= InOutLinks.Num() - 1))
		{
			return;
		...
		FTLLRN_LimbLink& ChildLink = InOutLinks[LinkIndex + 1];
		const FTLLRN_LimbLink& CurrentLink = InOutLinks[LinkIndex];
		const FTLLRN_LimbLink& ParentLink = InOutLinks[LinkIndex - 1];
	...
	void  FABRIK_ForwardReach(
		TArray<FTLLRN_LimbLink>& InOutLinks,
		...
		float MinRotationAngleRadians)
	{
		...
		{
			const FTLLRN_LimbLink& ChildLink = InOutLinks[Index+1];
			FTLLRN_LimbLink& CurrentLink = InOutLinks[Index];
		...
		float MinRotationAngleRadians)
	{
		...
		{
			const FTLLRN_LimbLink& ParentLink = InOutLinks[Index-1];
			FTLLRN_LimbLink& CurrentLink = InOutLinks[Index];
		...
		bool bAveragePull)
	{
		...
		if (Slop > ReachPrecision || bUseAngleLimit)
		{
			...
			FTLLRN_LimbLink& EndLink = InOutLinks[NumLinks-1];
			const FTLLRN_LimbLink& ParentLink = InOutLinks[NumLinks-2];

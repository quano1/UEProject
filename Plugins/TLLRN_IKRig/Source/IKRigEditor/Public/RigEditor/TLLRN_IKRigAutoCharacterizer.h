...
enum class ETLLRN_PreferredAxis
{
	None,
	PositiveX,
	NegativeX,
...
struct FTLLRN_CharacterizationStandard
{
	//
...
enum class ETLLRN_CleanOrFullName
{
	Full,
	Clean
};
...
struct FTLLRN_AbstractHierarchy
{
	FTLLRN_AbstractHierarchy(USkeletalMesh* InMesh);
	
	FTLLRN_AbstractHierarchy(
	...
	int32 GetParentIndex(const FName BoneName, const ETLLRN_CleanOrFullName NameType) const;

	// get the index from the name
	int32 GetBoneIndex(const FName BoneName, const ETLLRN_CleanOrFullName NameType) const;

	// get the name of a bone from it's index
	FName GetBoneName(const int32 BoneIndex, const ETLLRN_CleanOrFullName NameType) const;

	// returns true if PotentialChild is within the branch under Parent
	bool IsChildOf(const FName Parent, const FName PotentialChild, const ETLLRN_CleanOrFullName NameType) const;
	...
	const TArray<FName>& GetBoneNames(const ETLLRN_CleanOrFullName NameType) const;
	...
	int32 GetNumMatchingBones(const FTLLRN_AbstractHierarchy& OtherHierarchy) const;
...
struct FTLLRN_BoneSettingsForIK
{
	FTLLRN_BoneSettingsForIK(const FName InBoneName) : BoneToApplyTo(InBoneName) {}
	
	FName BoneToApplyTo;
	float RotationStiffness = 0.f;
	ETLLRN_PreferredAxis PreferredAxis = ETLLRN_PreferredAxis::None;
	bool bIsHinge = false;
...
struct FTLLRN_AllBoneSettingsForIK
{
	void SetPreferredAxis(const FName BoneName, const ETLLRN_PreferredAxis PreferredAxis, const bool bTreatAsHinge=true);
	...
	const TArray<FTLLRN_BoneSettingsForIK>& GetBoneSettings() const { return AllBoneSettings; };

private:
	FTLLRN_BoneSettingsForIK& GetOrAddBoneSettings(const FName BoneName);
	TArray<FTLLRN_BoneSettingsForIK> AllBoneSettings;
};

struct FTLLRN_BoneToPin
{
	FTLLRN_BoneToPin( const FName InBoneToPin, const FName InBoneToPinTo, const ETLLRN_RetargetSourceOrTarget InSkeletonToPinTo) :
	...
	FName BoneToPin;
	FName BoneToPinTo;
	ETLLRN_RetargetSourceOrTarget SkeletonToPinTo;
};

struct FTLLRN_BonesToPin
{
	void AddBoneToPin(
		const FName BoneToPin,
		const FName BoneToPinTo,
		ETLLRN_RetargetSourceOrTarget SkeletonToPinTo = ETLLRN_RetargetSourceOrTarget::Target)
	{
		AllBonesToPin.Emplace(BoneToPin,BoneToPinTo, SkeletonToPinTo);
	}

	const TArray<FTLLRN_BoneToPin>& GetBonesToPin() const { return AllBonesToPin; };

private:
	TArray<FTLLRN_BoneToPin> AllBonesToPin;
};

struct FTLLRN_AutoRetargetDefinition
{
	FTLLRN_RetargetDefinition RetargetDefinition;
	FTLLRN_AllBoneSettingsForIK BoneSettingsForIK;
	FTLLRN_BonesToPin BonesToPin;
...
struct FTLLRN_AutoCharacterizeResults
{
	...
	FTLLRN_AutoRetargetDefinition AutoRetargetDefinition;
...
struct FTLLRN_TemplateHierarchy
{
	FTLLRN_TemplateHierarchy(const FName& InName, const FTLLRN_AbstractHierarchy& InHierarchy) : Name(InName), Hierarchy(InHierarchy){}

	FName Name;
	FTLLRN_AbstractHierarchy Hierarchy;
	FTLLRN_AutoRetargetDefinition AutoRetargetDefinition;
};

// a collection of FTLLRN_TemplateHierarchy to compare against
struct FTLLRN_KnownTemplateHierarchies
{
	FTLLRN_KnownTemplateHierarchies();

	// iterate over all the known hierarchies, find the one that is closest to the given hierarchy
	// outputs the FTLLRN_AutoCharacterizeResults which includes a retarget definition to use
	void GetClosestMatchingKnownHierarchy(const FTLLRN_AbstractHierarchy& InHierarchy, FTLLRN_AutoCharacterizeResults& Results) const;

	// get a pointer to a template hierarchy by name
	const FTLLRN_TemplateHierarchy* GetKnownHierarchyByName(const FName Name) const;

private:

	// add a template hierarchy
	FTLLRN_TemplateHierarchy& AddTemplateHierarchy(const FName& Label, const TArray<FName>& BoneNames, const TArray<int32>& ParentIndices);
	
	TArray<FTLLRN_TemplateHierarchy> KnownHierarchies;
};
...
{
	FTLLRN_AutoCharacterizer() = default;
	...
	void GenerateRetargetDefinitionFromMesh(USkeletalMesh* Mesh, FTLLRN_AutoCharacterizeResults& Results) const;

	// get read-only access to a template by name
	const FTLLRN_TemplateHierarchy* GetKnownTemplateHierarchy(const FName& TemplateName) const;
...
private:
	...
	int32 ExpandChain(FTLLRN_BoneChain* ChainToExpand, const FTLLRN_AbstractHierarchy& Hierarchy) const;
	...
	FTLLRN_KnownTemplateHierarchies KnownHierarchies;
};

...
#include "TLLRN_IKRetargetBatchOperation.generated.h"
...
struct FTLLRN_AdditiveRetargetSettings
{
...
UENUM()
enum class ETLLRN_RetargetRootLockMode : uint8
{
...
struct TLLRN_IKRIGEDITOR_API FTLLRN_IKRetargetBatchOperationContext
{
...
public:
	...
	UTLLRN_IKRetargeter* IKRetargetAsset = nullptr;
...
class TLLRN_IKRIGEDITOR_API UTLLRN_IKRetargetBatchOperation : public UObject
{
	GENERATED_BODY()
...
public:
	...
	void RunRetarget(FTLLRN_IKRetargetBatchOperationContext& Context);
...
private:
	...
	int32 GenerateAssetLists(const FTLLRN_IKRetargetBatchOperationContext& Context);

	// Duplicate all the assets to retarget
	void DuplicateRetargetAssets(const FTLLRN_IKRetargetBatchOperationContext& Context, FScopedSlowTask& Progress);

	// Retarget skeleton and animation on all the duplicates
	void RetargetAssets(const FTLLRN_IKRetargetBatchOperationContext& Context, FScopedSlowTask& Progress);

	// Convert animation on all the duplicates
	void ConvertAnimation(const FTLLRN_IKRetargetBatchOperationContext& Context, FScopedSlowTask& Progress);

	// Copy/remap curves on all the duplicates
	void RemapCurves(const FTLLRN_IKRetargetBatchOperationContext& Context, FScopedSlowTask& Progress);

	// Replace existing assets (optional)
	void OverwriteExistingAssets(const FTLLRN_IKRetargetBatchOperationContext& Context, FScopedSlowTask& Progress);

	// Output notifications of results
	void NotifyUserOfResults(const FTLLRN_IKRetargetBatchOperationContext& Context, FScopedSlowTask& Progress) const;

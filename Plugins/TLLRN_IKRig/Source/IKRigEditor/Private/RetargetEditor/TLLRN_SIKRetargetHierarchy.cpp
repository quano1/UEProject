...
#define LOCTEXT_NAMESPACE "TLLRN_SIKRetargetHierarchy"
...
TSharedRef<SWidget> TLLRN_SIKRetargetHierarchyRow::GenerateWidgetForColumn(const FName& ColumnName)
{
	const FName BoneName = WeakTreeElement.Pin()->Name;
	const FName ChainName = WeakTreeElement.Pin()->ChainName;
	ETLLRN_RetargetSourceOrTarget CurrentSkeleton = EditorController.Pin()->GetSourceOrTarget();
...
void TLLRN_SIKRetargetHierarchy::Construct(
	...
	TSharedRef<FTLLRN_IKRetargetEditorController> InEditorController)
{
	...
	[
		...
		[
			...
			[
				SNew(SSegmentedControl<ETLLRN_RetargetSourceOrTarget>)
				.Value_Lambda([this]()
				{
					return EditorController.Pin()->GetSourceOrTarget();
				})
				.OnValueChanged_Lambda([this](ETLLRN_RetargetSourceOrTarget Mode)
				{
					EditorController.Pin()->SetSourceOrTargetMode(Mode);
				})
				+SSegmentedControl<ETLLRN_RetargetSourceOrTarget>::Slot(ETLLRN_RetargetSourceOrTarget::Source)
				.Text(LOCTEXT("SourceSkeleton", "Source"))
				+ SSegmentedControl<ETLLRN_RetargetSourceOrTarget>::Slot(ETLLRN_RetargetSourceOrTarget::Target)
		...
		.AutoHeight()
		[
			SAssignNew(EditPoseView, TLLRN_SIKRetargetPoseEditor, InEditorController)
		]
		...
		[
			...
			[
				...
				.OnTextChanged( this, &TLLRN_SIKRetargetHierarchy::OnFilterTextChanged )
			...
			[
				...
				.ContentPadding(2.0f)
				.OnGetMenuContent( this, &TLLRN_SIKRetargetHierarchy::CreateFilterMenuWidget )
		...
		[
			...
			[
				SAssignNew(TreeView, TLLRN_SIKRetargetHierarchyTreeView)
				...
				{
					return SNew(TLLRN_SIKRetargetHierarchyRow, OwnerTable)
					.EditorController(EditorController.Pin())
					.TreeElement(InItem);
				})
				.OnGetChildren(this, &TLLRN_SIKRetargetHierarchy::HandleGetChildrenForTree)
				.OnSelectionChanged(this, &TLLRN_SIKRetargetHierarchy::OnSelectionChanged)
				.OnMouseButtonDoubleClick(this, &TLLRN_SIKRetargetHierarchy::OnItemDoubleClicked)
				.OnSetExpansionRecursive(this, &TLLRN_SIKRetargetHierarchy::OnSetExpansionRecursive)
...
void TLLRN_SIKRetargetHierarchy::ShowItemAfterSelection(FName ItemName)
{
...
TSharedRef<SWidget> TLLRN_SIKRetargetHierarchy::CreateFilterMenuWidget()
{
	...
	MenuBuilder.AddMenuEntry(
		...
		{
			FilterOptions = FTLLRN_RetargetHierarchyFilterOptions();
			RefreshTreeView();
		})));
...
void TLLRN_SIKRetargetHierarchy::OnFilterTextChanged(const FText& SearchText)
{
	TextFilter->SetFilterText(SearchText);
	RefreshTreeView(false);
}

void TLLRN_SIKRetargetHierarchy::RefreshTreeView(bool IsInitialSetup)
{
	...
	const TSharedRef<FTLLRN_IKRetargetEditorController> ControllerRef = Controller.ToSharedRef();
...
void TLLRN_SIKRetargetHierarchy::HandleGetChildrenForTree(
	TSharedPtr<FIKRetargetHierarchyElement> InItem,
...
void TLLRN_SIKRetargetHierarchy::OnSelectionChanged(
	...
	ESelectInfo::Type SelectInfo)
{
	...
	Controller->EditBoneSelection(SelectedBoneNames, ETLLRN_SelectionEdit::Replace, bFromHierarchy);
}

void TLLRN_SIKRetargetHierarchy::OnItemDoubleClicked(TSharedPtr<FIKRetargetHierarchyElement> InItem)
{
...
void TLLRN_SIKRetargetHierarchy::OnSetExpansionRecursive(TSharedPtr<FIKRetargetHierarchyElement> InItem, bool bShouldBeExpanded)
{
	SetExpansionRecursive(InItem, false, bShouldBeExpanded);
}

void TLLRN_SIKRetargetHierarchy::SetExpansionRecursive(

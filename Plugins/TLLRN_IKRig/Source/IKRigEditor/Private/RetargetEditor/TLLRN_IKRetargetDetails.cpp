...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TLLRN_IKRetargetDetails)

#if WITH_EDITOR
#include "HAL/PlatformApplicationMisc.h"
#endif

#define LOCTEXT_NAMESPACE "TLLRN_IKRetargeterDetails"


FEulerTransform UTLLRN_IKRetargetBoneDetails::GetTransform(EIKRetargetTransformType TransformType, const bool bLocalSpace) const
{
	// editor setup?
	...
	const bool bEditingSource = Controller->GetSourceOrTarget() == ETLLRN_RetargetSourceOrTarget::Source;
	...
	case EIKRetargetTransformType::RelativeOffset:
		{
			...
			const ETLLRN_RetargetSourceOrTarget SourceOrTarget = Controller->GetSourceOrTarget();
...
bool UTLLRN_IKRetargetBoneDetails::IsComponentRelative(
...
void UTLLRN_IKRetargetBoneDetails::OnComponentRelativeChanged(
	ESlateTransformComponent::Type Component,
	bool bIsRelative,
...
void UTLLRN_IKRetargetBoneDetails::OnCopyToClipboard(
	ESlateTransformComponent::Type Component,
	EIKRetargetTransformType TransformType) const
{
...
void UTLLRN_IKRetargetBoneDetails::OnPasteFromClipboard(
	ESlateTransformComponent::Type Component,
	EIKRetargetTransformType TransformType)
{
	...
	public:
		...
		FRetargetPasteTransformWidgetErrorPipe(FTLLRN_IKRigLogger* InLog)	: FOutputDevice(), NumErrors(0), Log(InLog) {}
	...
	FTLLRN_IKRigLogger* Log = nullptr;
	if (UTLLRN_IKRetargetProcessor* Processor = Controller->GetRetargetProcessor())
	{
		Log = &Processor->Log;
	}
...
TOptional<FVector::FReal> UTLLRN_IKRetargetBoneDetails::GetNumericValue(
	EIKRetargetTransformType TransformType,
	ESlateTransformComponent::Type Component,
...
void UTLLRN_IKRetargetBoneDetails::OnNumericValueCommitted(
	ESlateTransformComponent::Type Component,
	ESlateRotationRepresentation::Type Representation,
	ESlateTransformSubComponent::Type SubComponent,
	FVector::FReal Value,
...
void UTLLRN_IKRetargetBoneDetails::CommitValueAsRelativeOffset(
...
void UTLLRN_IKRetargetBoneDetails::CommitValueAsBoneSpace(
	UTLLRN_IKRetargeterController* AssetController,
	const FReferenceSkeleton& RefSkeleton,
	const ETLLRN_RetargetSourceOrTarget SourceOrTarget,
	const int32 BoneIndex,
...
bool UTLLRN_IKRetargetBoneDetails::IsRootBone() const
{
	const FTLLRN_IKRetargetEditorController* Controller = EditorController.Pin().Get();
	if (!Controller)
	{
...
void UTLLRN_IKRetargetBoneDetails::OnMultiNumericValueCommitted(
	ESlateTransformComponent::Type Component,
	ESlateRotationRepresentation::Type Representation,
	ESlateTransformSubComponent::Type SubComponent,
	FVector::FReal Value,
	...
	bool bIsCommit)
{
	for(TObjectPtr<UTLLRN_IKRetargetBoneDetails> Bone : Bones)
	{	
...
void UTLLRN_IKRetargetBoneDetails::GetContentFromData(const DataType& InData, FString& Content) const
{
	TBaseStructure<DataType>::Get()->ExportText(Content, &InData, &InData, nullptr, PPF_None, nullptr);
}

TOptional<FVector::FReal> UTLLRN_IKRetargetBoneDetails::CleanRealValue(TOptional<FVector::FReal> InValue)
{
...
void FIKRetargetBoneDetailCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	{
		if (UTLLRN_IKRetargetBoneDetails* Bone = Cast<UTLLRN_IKRetargetBoneDetails>(Object.Get()))
		{
			Bones.Add(Bone);
		}
	}
	...
	const FTLLRN_IKRetargetEditorController& Controller = *Bones[0]->EditorController.Pin().Get();
	...
	FTLLRN_IKRetargetTransformUIData UIData;
	...
	TArrayView< TObjectPtr<UTLLRN_IKRetargetBoneDetails> > BonesView = TArrayView< TObjectPtr<UTLLRN_IKRetargetBoneDetails> >(Bones);
	...
	for(int32 PropertyIndex=0;PropertyIndex<UIData.Properties.Num();PropertyIndex++)
	{
		...
		if(bIsEditable)
		{
			TransformWidgetArgs.OnNumericValueCommitted_Static(
				&UTLLRN_IKRetargetBoneDetails::OnMultiNumericValueCommitted,
				TransformType,
				BonesView,
				true);
		...
		{
			return BonesView.ContainsByPredicate( [&](const TObjectPtr<UTLLRN_IKRetargetBoneDetails> Bone)
			{
		...
		{
			for (const TObjectPtr<UTLLRN_IKRetargetBoneDetails>& Bone: BonesView)
			{
		...
		TransformWidgetArgs.OnCopyToClipboard_UObject(Bones[0].Get(), &UTLLRN_IKRetargetBoneDetails::OnCopyToClipboard, TransformType);
		TransformWidgetArgs.OnPasteFromClipboard_UObject(Bones[0].Get(), &UTLLRN_IKRetargetBoneDetails::OnPasteFromClipboard, TransformType);
	...
	FTLLRN_IKRetargetTransformUIData& OutData) const
{
	...
	static TArray<FTLLRN_IKRetargetTransformWidgetData> ReadOnlyTransformMetaData =
	{
		FTLLRN_IKRetargetTransformWidgetData(
	...
	static TArray<FTLLRN_IKRetargetTransformWidgetData> EditableTransformMetaData =
	{
		FTLLRN_IKRetargetTransformWidgetData(
	...
	if (bIsEditingPose)
	{
		for (const FTLLRN_IKRetargetTransformWidgetData& TransformData : EditableTransformMetaData)
		{
		...
		OutData.Properties.Append(
			{
			DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UTLLRN_IKRetargetBoneDetails, OffsetTransform)),
			DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UTLLRN_IKRetargetBoneDetails, LocalTransform)),
			DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UTLLRN_IKRetargetBoneDetails, ReferenceTransform))
			});
	}
	else
	{
		for (const FTLLRN_IKRetargetTransformWidgetData& TransformData : ReadOnlyTransformMetaData)
		{
			OutData.TransformTypes.Add(TransformData.TransformType);
			OutData.ButtonLabels.Add(TransformData.ButtonLabel);
			OutData.ButtonTooltips.Add(TransformData.ButtonTooltip);
		...
		OutData.Properties.Append(
			{
			DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UTLLRN_IKRetargetBoneDetails, CurrentTransform)),
			DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UTLLRN_IKRetargetBoneDetails, ReferenceTransform))
			});
	}
}
...
void FRetargetChainSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	for (const TWeakObjectPtr<UObject>& Object : ObjectsBeingCustomized)
	{
		if (UTLLRN_RetargetChainSettings* ChainSettings = Cast<UTLLRN_RetargetChainSettings>(Object.Get()))
		{
	...
	FTLLRN_IKRetargetEditorController* Controller = ChainSettingsObjects[0]->EditorController.Pin().Get();
	if (!Controller)
	{
		return;
	}
	...
	auto CheckChainSetupForIKAndFK([this](
		FTLLRN_IKRetargetEditorController* Controller,
		const TObjectPtr<UTLLRN_RetargetChainSettings> ChainSettings,
		bool& OutFKSetup,
		bool& OutIKSetup,
		bool& OutIKConnected)
	{
	...
	for (const TWeakObjectPtr<UTLLRN_RetargetChainSettings> ChainSettings : ChainSettingsObjects)
	{
		bool bIsFKSetup;
		bool bIsIKSetup;
		bool bIsIKConnected;
	...
	TObjectPtr<UTLLRN_RetargetChainSettings> PrimaryChainSettingsObject = ChainSettingsObjects[0].Get();

	// get source chain options for combobox
	SourceChainOptions.Reset();
	SourceChainOptions.Add(MakeShareable(new FString(TEXT("None"))));
	if (const UTLLRN_IKRigDefinition* SourceIKRig = Controller->AssetController->GetIKRig(ETLLRN_RetargetSourceOrTarget::Source))
	{
	...
	[
		...
		.OnSelectionChanged_Lambda([this, Controller]( TSharedPtr<FString> InString, ESelectInfo::Type SelectInfo)
		{
			...
			for (TWeakObjectPtr<UTLLRN_RetargetChainSettings> ChainMapSettings : ChainSettingsObjects)
			{
	...
	AddSettingsSection(
		...
		Controller,
		SettingsCategory,
		GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetChainSettings, FK),
		FName("FK"),
		LOCTEXT("FKGroup_Label", "FK"),
		FTLLRN_TargetChainFKSettings::StaticStruct(),
		GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetChainFKSettings, EnableFK),
		bFKEditingEnabled,
		FKNotMappedWarning);
	...
	AddSettingsSection(
		...
		Controller,
		SettingsCategory,
		GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetChainSettings, IK),
		FName("IK"),
		LOCTEXT("IKGroup_Label", "IK"),
		FTLLRN_TargetChainIKSettings::StaticStruct(),
		GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetChainIKSettings, EnableIK),
	...
	AddSettingsSection(
		...
		Controller,
		SettingsCategory,
		GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetChainSettings, SpeedPlanting),
		FName("Speed Planting"),
		LOCTEXT("PlantingGroup_Label", "Speed Planting"),
		FTLLRN_TargetChainSpeedPlantSettings::StaticStruct(),
		GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetChainSpeedPlantSettings, EnableSpeedPlanting),
		bIKEditingEnabled,
		IKWarningToUse);
}
	...
	const FText& DisabledMessage) const
{
	...
	const FString SettingsPropertyPath = GET_MEMBER_NAME_STRING_CHECKED(UTLLRN_RetargetChainSettings, Settings);
...
void FRetargetRootSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	RootSettingsObject = Cast<UTLLRN_RetargetRootSettings>(ObjectsBeingCustomized[0].Get());
	...
	const FString StructPropertyPath = GET_MEMBER_NAME_STRING_CHECKED(UTLLRN_RetargetRootSettings, Settings);
	...
	AddPropertyRowToGroup(AlphaGroup,GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetRootSettings, RotationAlpha));
	AddPropertyRowToGroup(AlphaGroup,GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetRootSettings, TranslationAlpha));
	...
	AddPropertyRowToGroup(BlendToSourceGroup,GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetRootSettings, BlendToSource));
	AddPropertyRowToGroup(BlendToSourceGroup,GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetRootSettings, BlendToSourceWeights));

	// scale group
	IDetailGroup& ScaleGroup = SettingsCategory.AddGroup("Scale Translation", LOCTEXT("ScaleRoot_Label", "Scale Translation"), false, true);
	AddPropertyRowToGroup(ScaleGroup,GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetRootSettings, ScaleHorizontal));
	AddPropertyRowToGroup(ScaleGroup,GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetRootSettings, ScaleVertical));

	// offset group
	IDetailGroup& OffsetGroup = SettingsCategory.AddGroup("Offsets", LOCTEXT("OffsetRoot_Label", "Offsets"), false, true);
	AddPropertyRowToGroup(OffsetGroup,GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetRootSettings, TranslationOffset));
	AddPropertyRowToGroup(OffsetGroup,GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetRootSettings, RotationOffset));
	...
	AddPropertyRowToGroup(AffectIKGroup,GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetRootSettings, AffectIKHorizontal));
	AddPropertyRowToGroup(AffectIKGroup,GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_TargetRootSettings, AffectIKVertical));
}

void FTLLRN_RetargetGlobalSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	TArray<TWeakObjectPtr<UObject>> ObjectsBeingCustomized = DetailBuilder.GetSelectedObjects();
	if (ObjectsBeingCustomized.Num() < 0)
	{
	...
	GlobalSettingsObject = Cast<UTLLRN_IKRetargetGlobalSettings>(ObjectsBeingCustomized[0].Get());
	...
	{
		const FString EnableWarpingPropertyName = GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, bWarping);
		const FString PropertyPath = FString::Printf(TEXT("%s.%s"), *StructPropertyPath, *PropertyName);
	...
	AlphaGroup.AddPropertyRow(GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, bEnableRoot)));
	AlphaGroup.AddPropertyRow(GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, bEnableFK)));
	AlphaGroup.AddPropertyRow(GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, bEnableIK)));
	AlphaGroup.AddPropertyRow(GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, bEnablePost)));
	...
	const TSharedRef<IPropertyHandle> EnableWarpingPropertyHandle = GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, bWarping));
	...
	WarpingGroup.AddPropertyRow(GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, ForwardDirection))).EditCondition(EditCondition,nullptr);
	WarpingGroup.AddPropertyRow(GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, DirectionSource))).EditCondition(EditCondition,nullptr);

	// get target chain options for combobox
	TargetChainOptions.Reset();
	TargetChainOptions.Add(MakeShareable(new FString(TEXT("None"))));
	if (const UTLLRN_IKRigDefinition* TargetIKRig = Controller.Pin().Get()->AssetController->GetIKRig(ETLLRN_RetargetSourceOrTarget::Target))
	{
		const TArray<FTLLRN_BoneChain>& Chains = TargetIKRig->GetRetargetChains();
		for (const FTLLRN_BoneChain& BoneChain : Chains)
		{
	...
	const TSharedRef<IPropertyHandle> DirectionChainPropertyHandle = GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, DirectionChain));
	WarpingGroup.AddPropertyRow(DirectionChainPropertyHandle).CustomWidget()
	.IsEnabled(GlobalSettingsObject->Settings.DirectionSource == ETLLRN_WarpingDirectionSource::Chain)
	.NameContent()
	[
		SNew(SBox)
		.Content()
	...
	WarpingGroup.AddPropertyRow(GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, WarpForwards))).EditCondition(EditCondition,nullptr);
	WarpingGroup.AddPropertyRow(GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, WarpSplay))).EditCondition(EditCondition,nullptr);
	WarpingGroup.AddPropertyRow(GetPropertyHandle(GET_MEMBER_NAME_STRING_CHECKED(FTLLRN_RetargetGlobalSettings, SidewaysOffset))).EditCondition(EditCondition,nullptr);
}
...
void FRetargetOpStackCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	RetargetOpStackObject = Cast<UTLLRN_RetargetOpStack>(ObjectsBeingCustomized[0].Get());
	...
	.WholeRowWidget
	[
		SNew(TLLRN_SRetargetOpStack, Controller)
	];
	...
	for (TObjectPtr<UTLLRN_RetargetOpBase> Op : RetargetOpStackObject->RetargetOps)
	{
...
void FRetargetOpStackCustomization::AddNewRetargetOp(UClass* Class)
{
	...
	const UTLLRN_IKRetargeterController* AssetController = Controller.Pin().Get()->AssetController;
	if (!AssetController)
	{
		return;
	}

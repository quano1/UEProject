...
#define LOCTEXT_NAMESPACE "TLLRN_RetargetBatchOperation"

int32 UTLLRN_IKRetargetBatchOperation::GenerateAssetLists(const FTLLRN_IKRetargetBatchOperationContext& Context)
{
...
void UTLLRN_IKRetargetBatchOperation::DuplicateRetargetAssets(
	const FTLLRN_IKRetargetBatchOperationContext& Context,
...
void UTLLRN_IKRetargetBatchOperation::RetargetAssets(
	const FTLLRN_IKRetargetBatchOperationContext& Context,
	FScopedSlowTask& Progress)
{
	...
	TArray<FTLLRN_AdditiveRetargetSettings> SettingsToRestoreAfterRetarget;
	...
	for (UAnimationAsset* AssetToRetarget : AnimationAssetsToRetarget)
	{
		...
		if (UAnimSequence* AnimSequenceToRetarget = Cast<UAnimSequence>(AssetToRetarget))
		{
			...
			FTLLRN_AdditiveRetargetSettings SequenceSettings;
	...
	{
		for (FTLLRN_AdditiveRetargetSettings SettingsToRestore : SettingsToRestoreAfterRetarget)
		{
...
void UTLLRN_IKRetargetBatchOperation::ConvertAnimation(
	const FTLLRN_IKRetargetBatchOperationContext& Context,
	FScopedSlowTask& Progress)
{
	// initialize the retargeter
	UObject* TransientOuter = Cast<UObject>(GetTransientPackage());
	UTLLRN_IKRetargetProcessor* Processor = NewObject<UTLLRN_IKRetargetProcessor>(TransientOuter);
	...
	const FTLLRN_RetargetSkeleton& TargetSkeleton = Processor->GetSkeleton(ETLLRN_RetargetSourceOrTarget::Target);
	...
	const FTLLRN_RetargetSkeleton& SourceSkeleton = Processor->GetSkeleton(ETLLRN_RetargetSourceOrTarget::Source);
	...
	for (TPair<UAnimationAsset*, UAnimationAsset*>& Pair : DuplicatedAnimAssets)
	{
		...
		for (int32 FrameIndex=0; FrameIndex<NumFrames; ++FrameIndex)
		{
			...
			FTLLRN_RetargetProfile SettingsProfile;
...
void UTLLRN_IKRetargetBatchOperation::RemapCurves(const FTLLRN_IKRetargetBatchOperationContext& Context, FScopedSlowTask& Progress)
{
	...
	UTLLRN_RetargetOpStack* OpStack = Context.IKRetargetAsset->GetPostSettingsUObject();
	for (const TObjectPtr<UTLLRN_RetargetOpBase>& RetargetOp : OpStack->RetargetOps)
	{
		...
		bCopyAllSourceCurves &= TLLRN_CurveRemapOp->bCopyAllSourceCurves;
	}
...
void UTLLRN_IKRetargetBatchOperation::OverwriteExistingAssets(const FTLLRN_IKRetargetBatchOperationContext& Context, FScopedSlowTask& Progress)
{
...
void UTLLRN_IKRetargetBatchOperation::NotifyUserOfResults(
	const FTLLRN_IKRetargetBatchOperationContext& Context,
...
void UTLLRN_IKRetargetBatchOperation::GetNewAssets(TArray<UObject*>& NewAssets) const
{
...
void UTLLRN_IKRetargetBatchOperation::CleanupIfCancelled(const FScopedSlowTask& Progress) const
{
	if (!Progress.ShouldCancel())
	{
		return;
...
void FTLLRN_AdditiveRetargetSettings::PrepareForRetarget(UAnimSequence* InSequenceAsset)
{
...
void FTLLRN_AdditiveRetargetSettings::RestoreOnAsset() const
{
...
TArray<FAssetData> UTLLRN_IKRetargetBatchOperation::DuplicateAndRetarget(
	const TArray<FAssetData>& AssetsToRetarget,
	USkeletalMesh* SourceMesh,
	USkeletalMesh* TargetMesh,
	UTLLRN_IKRetargeter* IKRetargetAsset,
	...
	const bool bIncludeReferencedAssets)
{
	...
	FTLLRN_IKRetargetBatchOperationContext Context;
	...
	UTLLRN_IKRetargetBatchOperation* BatchOperation = NewObject<UTLLRN_IKRetargetBatchOperation>();
...
void UTLLRN_IKRetargetBatchOperation::RunRetarget(FTLLRN_IKRetargetBatchOperationContext& Context)
{
	Reset();
	...
	const UTLLRN_IKRigDefinition* SrcIKRig = Context.IKRetargetAsset->GetIKRig(ETLLRN_RetargetSourceOrTarget::Source);
	if (!SrcIKRig)
	{
	...
	const UTLLRN_IKRigDefinition* TgtIKRig = Context.IKRetargetAsset->GetIKRig(ETLLRN_RetargetSourceOrTarget::Target);
	if (!TgtIKRig)
	{
...
void UTLLRN_IKRetargetBatchOperation::Reset()
{
	AnimationAssetsToRetarget.Reset();
	AnimBlueprintsToRetarget.Reset();
